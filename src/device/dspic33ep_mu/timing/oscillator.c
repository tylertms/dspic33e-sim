...
static bool oscillator_pll_mode(uint16_t control_value) {
    const uint16_t requested_source = (uint16_t)((control_value & OSCILLATOR_REQUEST_MASK) >> 8u);
    return requested_source == 1u || requested_source == 3u;
}

uint8_t dspic33_device_internal_oscillator_current_source(uint16_t control_value) {
    return (uint8_t)((control_value & OSCILLATOR_CURRENT_MASK) >> 12u);
}

static uint8_t oscillator_requested_source(uint16_t control_value) {
    return (uint8_t)((control_value & OSCILLATOR_REQUEST_MASK) >> 8u);
}

static bool oscillator_source_available(const Dspic33* cpu, uint8_t requested_source) {
    return (requested_source != 2u && requested_source != 3u) ||
           (cpu->configuration[8u] & 0x03u) != 0x03u;
}

static bool oscillator_source_immediately_ready(uint8_t requested_source) {
    return requested_source == 0u || requested_source >= 5u;
}

static bool oscillator_configuration_locked(const Dspic33* cpu, uint16_t control_value) {
    return (control_value & OSCILLATOR_CLOCK_LOCK) != 0u &&
           (cpu->configuration[8u] & OSCILLATOR_CONFIGURATION_CLOCK_LOCK) != 0u;
}

static bool oscillator_direct_pll_transition(uint16_t control_value) {
    const uint8_t current_source = dspic33_device_internal_oscillator_current_source(control_value);
    const uint8_t requested_source = oscillator_requested_source(control_value);
    return (current_source == 1u && requested_source == 3u) ||
           (current_source == 3u && requested_source == 1u);
}
...
static void schedule_oscillator_readiness(Dspic33* cpu, uint16_t oscillator_control) {
    ...
    const uint64_t readiness_delay =
        oscillator_pll_mode(oscillator_control) ? OSCILLATOR_SOURCE_DELAY : OSCILLATOR_SWITCH_DELAY;
    schedule_oscillator_event(cpu, OSCILLATOR_EVENT_SWITCH, readiness_delay);
}
...
static bool main_pll_relock_required(const Dspic33* cpu, uint16_t address, uint16_t previous,
    ...
    const uint8_t configured_source = configured_main_pll_source(cpu, oscillator_control);
    const uint16_t changed_bits = (uint16_t)(previous ^ current);
    if (configured_source != 1u && configured_source != 3u) {
        return false;
    }
    if (address == MAIN_PLL_FEEDBACK) {
        return (changed_bits & MAIN_PLL_FEEDBACK_DIVISOR) != 0u;
    }
    if (address == MAIN_OSCILLATOR_TUNING) {
        return configured_source == 1u && (changed_bits & MAIN_FRC_TUNING) != 0u;
    }
    if (address != MAIN_CLOCK_DIVISOR) {
        return false;
    }
    if ((changed_bits & MAIN_PLL_PRESCALER) != 0u) {
        return true;
    }
    return configured_source == 1u && (changed_bits & MAIN_FRC_DIVISOR) != 0u;
}

static void restart_main_pll_lock(Dspic33* cpu) {
    const uint16_t oscillator_control = dspic33_device_internal_raw_word(cpu, OSCILLATOR_CONTROL);
    const bool source_ready = cpu->oscillator.active && cpu->oscillator.source_ready;
    cpu->oscillator.generation++;
...
void dspic33_device_internal_oscillator_configuration_changed(Dspic33* cpu,
    ...
    const uint16_t oscillator_control = dspic33_device_internal_raw_word(cpu, OSCILLATOR_CONTROL);
    const uint8_t requested_source = oscillator_requested_source(oscillator_control);
    ...
    if (!cpu->oscillator.active || oscillator_direct_pll_transition(oscillator_control) ||
        (requested_source != 2u && requested_source != 3u) ||
...
void dspic33_device_internal_oscillator_startup_configuration_changed(
    ...
    const uint8_t startup_source = (uint8_t)(cpu->configuration[6u] & 0x07u);
    if (((previous_configuration ^ cpu->configuration[6u]) & 0x07u) == 0u ||
        (cpu->configuration[8u] & OSCILLATOR_CONFIGURATION_SWITCH_DISABLE) == 0u) {
        return;
    }
    dspic33_device_abort_oscillator_switch(cpu);
    dspic33_device_internal_start_automatic_oscillator_switch(cpu, startup_source);
}
...
void dspic33_device_internal_start_automatic_oscillator_switch(Dspic33* cpu,
    ...
    oscillator_control =
        (uint16_t)((oscillator_control & ~OSCILLATOR_REQUEST_MASK & ~OSCILLATOR_PLL_LOCK &
                   ...
                   ((uint16_t)requested_source << 8u));
    dspic33_device_internal_raw_write_word(cpu, OSCILLATOR_CONTROL, oscillator_control);
    if (oscillator_source_immediately_ready(requested_source)) {
        oscillator_control = (uint16_t)((oscillator_control & ~OSCILLATOR_CURRENT_MASK) |
                                        ((uint16_t)requested_source << 12u));
        dspic33_device_internal_raw_write_word(cpu, OSCILLATOR_CONTROL, oscillator_control);
        return;
    }
    cpu->oscillator.generation++;
...
void dspic33_device_internal_reset_main_oscillator(Dspic33* cpu) {
    const uint8_t startup_source = (uint8_t)(cpu->configuration[6u] & 0x07u);
    uint16_t oscillator_control = (uint16_t)(startup_source << 8u);
    if ((cpu->configuration[6u] & 0x80u) == 0u) {
        oscillator_control |= (uint16_t)(startup_source << 12u);
        if (startup_source == 1u || startup_source == 3u) {
...
void dspic33_device_power_on_reset(Dspic33* cpu) {
    const uint8_t startup_source = (uint8_t)(cpu->configuration[6u] & 0x07u);
    if ((cpu->configuration[6u] & 0x80u) != 0u && startup_source != 0u) {
        dspic33_device_internal_start_automatic_oscillator_switch(cpu, startup_source);
    }
}
...
static void apply_oscillator_low(Dspic33* cpu, uint16_t previous_control,
    ...
    uint16_t writable_mask = OSCILLATOR_IO_LOCK;
    const bool was_io_locked = (previous_control & OSCILLATOR_IO_LOCK) != 0u;
    const bool requests_io_lock = (requested_control & OSCILLATOR_IO_LOCK) != 0u;
    ...
    if (!oscillator_configuration_locked(cpu, previous_control)) {
        writable_mask |= OSCILLATOR_LP_ENABLE;
    }
    oscillator_control =
        (uint16_t)((oscillator_control & ~writable_mask) | (requested_control & writable_mask));
...
bool dspic33_oscillator_failure_detected(Dspic33* cpu) {
    ...
    if ((cpu->configuration[8u] & 0xc0u) != 0u || cpu->power_state == DSPIC33_POWER_SLEEP ||
        (current_source != 2u && current_source != 3u && current_source != 4u)) {
        return false;
    }
    dspic33_device_abort_oscillator_switch(cpu);
...
bool dspic33_device_internal_protect_oscillator_write(Dspic33* cpu, uint16_t address,
    ...
    write_lane = (uint8_t)(address - OSCILLATOR_CONTROL);
    write_value = cpu->data[address];
    first_key = write_lane == 0u ? 0x46u : 0x78u;
    second_key = write_lane == 0u ? 0x57u : 0x9au;
    dspic33_device_internal_raw_write_word(cpu, OSCILLATOR_CONTROL, previous_control);
    if (!cpu->instruction_active || cpu->io.cpu_write_width != 1u) {
        cpu->oscillator.key_stage = 0u;
        return true;
    }
    if (oscillator_key_authorized(cpu, write_lane, 2u)) {
        cpu->oscillator.key_stage = 0u;
        if (write_lane == 0u) {
    ...
    if (oscillator_key_authorized(cpu, write_lane, 1u) && write_value == second_key) {
        cpu->oscillator.key_stage = 2u;
        cpu->oscillator.key_instruction = cpu->instructions;
        return true;
    }
    if (write_value == first_key) {
        oscillator_key_start(cpu, write_lane);
        return true;
    }
    cpu->oscillator.key_stage = 0u;
    return true;
