...
bool dspic33_device_internal_timer_power_enabled(const Dspic33* cpu, uint8_t timer_index,
    ...
    return external_clock && timer_index == 0u && (timer_control & TIMER_SYNC) == 0u;
}
...
static TimerAdvance advance_counter(uint64_t current_value, uint64_t period_value,
    ...
    TimerAdvance advance_result = {current_value, 0u};
    const uint64_t cycle_length = period_value + 1u;
    ...
    if (current_value == period_value) {
        first_match_ticks = cycle_length;
    ...
    if (tick_count >= first_match_ticks) {
        advance_result.period_matches = 1u + (tick_count - first_match_ticks) / cycle_length;
    }
    if (tick_count < first_reset_ticks) {
        advance_result.counter_value = (current_value + tick_count) & maximum_value;
        return advance_result;
    }
    remaining_ticks = tick_count - first_reset_ticks;
    advance_result.counter_value = remaining_ticks % cycle_length;
    return advance_result;
}
    ...
    if (is_paired) {
        ...
        const uint64_t period_value =
            ...
            ((uint64_t)dspic33_device_internal_raw_word(
             ...
             << 16u);
        const TimerAdvance advance_result =
        ...
        dspic33_device_internal_raw_write_word(cpu, dspic33_device_timer_registers[timer_index],
                                               (uint16_t)advance_result.counter_value);
        dspic33_device_internal_raw_write_word(cpu,
                                               dspic33_device_timer_registers[timer_index + 1u],
                                               (uint16_t)(advance_result.counter_value >> 16u));
        if (period_value != 0u) {
            signal_timer_period(cpu, (uint8_t)(timer_index + 1u), advance_result.period_matches,
                                is_gated, synchronization_sources);
        }
    } else {
        const uint16_t period_value =
            dspic33_device_internal_raw_word(cpu, dspic33_device_timer_periods[timer_index]);
        const TimerAdvance advance_result = advance_counter(
            dspic33_device_internal_raw_word(cpu, dspic33_device_timer_registers[timer_index]),
        ...
        dspic33_device_internal_raw_write_word(cpu, dspic33_device_timer_registers[timer_index],
                                               (uint16_t)advance_result.counter_value);
        if (period_value != 0u) {
            signal_timer_period(cpu, timer_index, advance_result.period_matches, is_gated,
...
void dspic33_device_internal_pulse_timer_synchronization_sources(Dspic33* cpu, uint16_t* pending) {
    uint16_t pending_sources = *pending;
    uint8_t timer_index = 0u;

    *pending = 0u;
    while (pending_sources != 0u) {
        if ((pending_sources & 1u) != 0u) {
...
void dspic33_device_internal_clock_timer(Dspic33* cpu, uint8_t timer_index, uint64_t clock_count,
    ...
    const uint16_t timer_control =
        dspic33_device_internal_raw_word(cpu, dspic33_device_timer_controls[timer_index]);
    const uint32_t prescale = dspic33_device_internal_timer_prescale(timer_control);
    const uint64_t accumulated_clocks = cpu->io.timer_fraction[timer_index] + clock_count;
    uint64_t remaining_ticks = accumulated_clocks / prescale;
    const uint8_t signal_timer = dspic33_device_internal_timer_pair_enabled(cpu, timer_index)
                                     ? (uint8_t)(timer_index + 1u)
                                     : timer_index;
    const uint8_t sync_source = (uint8_t)(INPUT_CAPTURE_SYNC_TIMER_FIRST + signal_timer);
    cpu->io.timer_fraction[timer_index] = (uint32_t)(accumulated_clocks % prescale);
    ...
    while (remaining_ticks != 0u) {
        ...
        if (signal_timer != timer_index && signal_timer < 5u) {
        ...
        if (timer_index < 5u) {
            static const uint16_t capture_sources[5] = {0x1000u, 0x0400u, 0x0000u, 0x0800u,
                                                        0x0c00u};
            dspic33_device_internal_input_capture_advance_clock(cpu, capture_sources[timer_index],
                                                                step_ticks);
            dspic33_device_internal_output_compare_advance_clock(cpu, timer_index, step_ticks);
            if (signal_timer != timer_index && signal_timer < 5u) {
                dspic33_device_internal_output_compare_advance_clock(cpu, signal_timer, step_ticks);
            }
        }
...
void dspic33_device_internal_set_timer_gate(Dspic33* cpu, uint8_t timer_index, bool gate_high) {
    uint16_t timer_bit;
    bool previous_gate_high;
    uint16_t timer_control;
    ...
    timer_bit = (uint16_t)(1u << timer_index);
    previous_gate_high = (cpu->io.timer_gate & timer_bit) != 0u;
    if (gate_high) {
    ...
    timer_control =
        dspic33_device_internal_raw_word(cpu, dspic33_device_timer_controls[timer_index]);
    if (!dspic33_device_internal_timer_pmd_disabled(cpu, timer_index) && previous_gate_high &&
        !gate_high && (cpu->io.timer_enabled & timer_bit) != 0u &&
