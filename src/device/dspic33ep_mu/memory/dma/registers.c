...
bool dspic33_device_internal_interrupt_control_write(Dspic33* cpu, uint16_t register_address,
    ...
    if (register_address == 0x08c0u) {
        current_word = (uint16_t)(requested_word & 0xfffeu);
        dspic33_device_internal_raw_write_word(cpu, register_address, current_word);
        if ((previous_word & 0x0020u) != 0u && (current_word & 0x0020u) == 0u) {
            dspic33_device_internal_raw_write_word(cpu, DMA_PWC, 0u);
            dspic33_device_internal_raw_write_word(cpu, DMA_RQC, 0u);
        }
        if ((current_word & 0x0010u) == 0u) {
            dspic33_set_math_error_source(cpu, false);
        }
        return true;
    }
    if (register_address == 0x08c2u) {
        current_word = (uint16_t)((previous_word & 0x4000u) | (requested_word & 0xa01fu));
        dspic33_device_internal_raw_write_word(cpu, register_address, current_word);
        if ((current_word & 0x2000u) != 0u) {
            dspic33_device_internal_raw_write_word(
                cpu, 0x08c6u, (uint16_t)(dspic33_device_internal_raw_word(cpu, 0x08c6u) | 0x0001u));
        }
        dspic33_set_generic_hard_trap_source(
            cpu, (current_word & 0x2000u) != 0u ||
                     (dspic33_device_internal_raw_word(cpu, 0x08c6u) & 0x0001u) != 0u);
        if ((current_word & 0x8000u) != 0u) {
            cpu->gie_disable_deferred = 0u;
            cpu->gie_disable_deferred_next = 0u;
        } else if ((previous_word & 0x8000u) != 0u) {
            cpu->gie_disable_deferred_next = 1u;
        }
        return true;
    }
    if (register_address == 0x08c4u) {
        current_word = (uint16_t)(requested_word & 0x0070u);
        dspic33_device_internal_raw_write_word(cpu, register_address, current_word);
        dspic33_set_generic_soft_trap_source(cpu, current_word != 0u);
        return true;
    }
    if (register_address == 0x08c6u) {
        current_word = (uint16_t)(requested_word & 0x0001u);
        dspic33_device_internal_raw_write_word(cpu, register_address, current_word);
        dspic33_set_generic_hard_trap_source(
            cpu,
            current_word != 0u || (dspic33_device_internal_raw_word(cpu, 0x08c2u) & 0x2000u) != 0u);
        return true;
    }
...
static uint32_t dma_register_address(const Dspic33* cpu, uint16_t channel_base,
    ...
    return ((uint32_t)(dspic33_device_internal_raw_word(
                           cpu, (uint16_t)(channel_base + address_offset + 2u)) &
                       0x00ffu)
            << 16u) |
           dspic33_device_internal_raw_word(cpu, (uint16_t)(channel_base + address_offset));
}
...
static void initialize_dma_channel(Dspic33* cpu, uint8_t channel_index) {
    ...
    const uint16_t channel_mask = dspic33_device_internal_dma_channel_bit(channel_index);
    ...
    cpu->io.dma_bank &= (uint16_t)~channel_mask;
    cpu->io.dma_half_raised &= (uint16_t)~channel_mask;
    cpu->io.dma_forced_pending &= (uint16_t)~channel_mask;
    cpu->io.dma_peripheral_pending &= (uint16_t)~channel_mask;
    cpu->io.dma_active &= (uint16_t)~channel_mask;
    cpu->io.dma_enabled |= channel_mask;
    dspic33_device_internal_dma_advance_generation(cpu, channel_index);
    dspic33_device_internal_raw_write_word(
        cpu, DMA_PPS, (uint16_t)(dspic33_device_internal_raw_word(cpu, DMA_PPS) & ~channel_mask));
}
...
void dspic33_device_internal_update_dma_control(Dspic33* cpu, uint8_t channel_index,
    ...
    const uint16_t channel_mask = dspic33_device_internal_dma_channel_bit(channel_index);
    const bool was_channel_enabled = (previous_control & DMA_CON_CHEN) != 0u;
    const bool is_channel_enabled = (current_control & DMA_CON_CHEN) != 0u;

    if (is_channel_enabled && !was_channel_enabled) {
        initialize_dma_channel(cpu, channel_index);
    } else if (!is_channel_enabled && was_channel_enabled) {
        cpu->io.dma_enabled &= (uint16_t)~channel_mask;
        cpu->io.dma_forced_pending &= (uint16_t)~channel_mask;
        cpu->io.dma_peripheral_pending &= (uint16_t)~channel_mask;
        cpu->io.dma_active &= (uint16_t)~channel_mask;
        cpu->io.dma_arbiter_waiting &= (uint16_t)~channel_mask;
        dspic33_device_internal_dma_advance_generation(cpu, channel_index);
        dspic33_device_internal_raw_write_word(
...
void dspic33_device_internal_update_dma_request(Dspic33* cpu, uint8_t channel_index,
    ...
    const uint16_t channel_base = dspic33_device_internal_dma_channel_base(channel_index);
    const uint16_t request_register = (uint16_t)(channel_base + 2u);
    uint16_t request_word = dspic33_device_internal_raw_word(cpu, request_register);
    const uint16_t channel_mask = dspic33_device_internal_dma_channel_bit(channel_index);

    if ((dspic33_device_internal_raw_word(cpu, channel_base) & DMA_CON_CHEN) == 0u) {
        dspic33_device_internal_raw_write_word(cpu, request_register,
                                               (uint16_t)(request_word & ~DMA_REQ_FORCE));
        return;
    }
    if ((previous_request & DMA_REQ_FORCE) != 0u) {
        dspic33_device_internal_raw_write_word(cpu, request_register,
                                               (uint16_t)(request_word | DMA_REQ_FORCE));
        return;
    }
    if ((request_word & DMA_REQ_FORCE) == 0u) {
        return;
    }
    if ((cpu->io.dma_active & channel_mask) != 0u) {
        dspic33_device_internal_dma_request_collision(cpu, channel_index);
        dspic33_device_internal_raw_write_word(cpu, request_register,
                                               (uint16_t)(request_word & ~DMA_REQ_FORCE));
        return;
    }
    if ((cpu->io.dma_peripheral_pending & channel_mask) != 0u) {
        dspic33_device_internal_dma_request_collision(cpu, channel_index);
        dspic33_device_internal_raw_write_word(cpu, request_register,
                                               (uint16_t)(request_word & ~DMA_REQ_FORCE));
        return;
    }
    if ((cpu->io.dma_forced_pending & channel_mask) == 0u &&
