#include "device/dspic33ep_mu/communication/i2c/internal.h"

static bool address_matches(const Dspic33* cpu, uint8_t channel, uint16_t address) {
    uint16_t control_word =
        dspic33_i2c_internal_raw_word(cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_CON));
    uint16_t configured_address =
        dspic33_i2c_internal_raw_word(cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_ADD));
    uint16_t address_mask =
        dspic33_i2c_internal_raw_word(cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_MSK));
    if ((control_word & I2C_IPMIEN) != 0u) {
        return true;
    }
    if ((control_word & I2C_A10M) != 0u) {
        return false;
    }
    if ((address <= 0x07u || address >= 0x78u) && address != (configured_address & 0x007fu)) {
        return false;
    }
    return ((address ^ configured_address) & (uint16_t)~address_mask & 0x007fu) == 0u;
}

static bool ten_bit_high_matches(const Dspic33* cpu, uint8_t channel, uint16_t address) {
    uint16_t control_word =
        dspic33_i2c_internal_raw_word(cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_CON));
    uint16_t configured_address =
        dspic33_i2c_internal_raw_word(cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_ADD));
    uint16_t address_mask =
        dspic33_i2c_internal_raw_word(cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_MSK));
    return (control_word & I2C_IPMIEN) != 0u ||
           ((control_word & I2C_A10M) != 0u &&
            ((address ^ configured_address) & (uint16_t)~address_mask & 0x0300u) == 0u);
}

static void reject_slave(Dspic33* cpu, uint8_t channel) {
    uint8_t channel_mask = (uint8_t)(1u << channel);

    cpu->io.i2c_slave_active &= (uint8_t)~channel_mask;
    cpu->io.i2c_slave_read &= (uint8_t)~channel_mask;
    cpu->io.i2c_slave_rejected |= channel_mask;
}

static void slave_start(Dspic33* cpu, uint8_t channel, uint16_t payload, bool schedule_ten_second,
                        bool raise_interrupt) {
    uint16_t register_base = dspic33_i2c_bases[channel];
    uint16_t address = payload & 0x03ffu;
    uint16_t control_word = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_CON));
    uint16_t status_word = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_STAT));
    bool is_read = (payload & I2C_EXTERNAL_READ) != 0u;
    bool is_ten_bit = (payload & I2C_EXTERNAL_TEN_BIT) != 0u;
    bool is_general_call = !is_ten_bit && address == 0u && !is_read;
    bool is_acknowledged = dspic33_i2c_internal_slave_acknowledges(status_word);
    uint8_t channel_mask = (uint8_t)(1u << channel);

    if (!dspic33_i2c_internal_module_enabled(cpu, channel)) {
        return;
    }
    if ((cpu->io.i2c_slave_rejected & channel_mask) != 0u) {
        return;
    }
    status_word = (uint16_t)((status_word | I2C_START_STATUS) &
                             ~(I2C_STOP_STATUS | I2C_GENERAL_CALL | I2C_TEN_BIT));
    dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_STAT), status_word);
    if (is_ten_bit && !ten_bit_high_matches(cpu, channel, address)) {
        dspic33_i2c_internal_record_slave_acknowledgement(cpu, channel, false);
        reject_slave(cpu, channel);
        return;
    }
    if (!is_ten_bit && (control_word & I2C_IPMIEN) == 0u &&
        ((is_general_call && (control_word & I2C_GCEN) == 0u) ||
         (!is_general_call && !address_matches(cpu, channel, address)))) {
        dspic33_i2c_internal_record_slave_acknowledgement(cpu, channel, false);
        reject_slave(cpu, channel);
        return;
    }
    cpu->io.i2c_slave_active |= channel_mask;
    if (is_read && !is_ten_bit) {
        cpu->io.i2c_slave_read |= channel_mask;
    } else {
        cpu->io.i2c_slave_read &= (uint8_t)~channel_mask;
    }
    cpu->io.i2c_slave_address[channel] = address;
    status_word &= (uint16_t)~(I2C_DATA | I2C_READ | I2C_GENERAL_CALL | I2C_TEN_BIT);
    if (is_read && !is_ten_bit) {
        status_word |= I2C_READ;
    }
    if (is_general_call) {
        status_word |= I2C_GENERAL_CALL;
    }
    if ((status_word & I2C_RBF) != 0u) {
        status_word |= I2C_OVERFLOW;
    } else {
        dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_RCV),
                                            is_ten_bit
                                                ? (uint16_t)(0x00f0u | ((address >> 7u) & 6u))
                                                : (uint16_t)((address << 1u) | is_read));
        status_word |= I2C_RBF;
    }
    control_word &= (uint16_t)~I2C_SCLREL;
    if (is_read && !is_ten_bit && (control_word & I2C_IPMIEN) != 0u) {
        cpu->io.i2c_slave_active &= (uint8_t)~channel_mask;
        cpu->io.i2c_slave_read &= (uint8_t)~channel_mask;
        control_word |= I2C_SCLREL;
    }
    dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_CON), control_word);
    dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_STAT), status_word);
    dspic33_i2c_internal_record_slave_acknowledgement(cpu, channel, is_acknowledged);
    if (raise_interrupt) {
        dspic33_i2c_internal_raise_slave(cpu, channel);
    }
    if (is_ten_bit && is_acknowledged && schedule_ten_second &&
        !dspic33_i2c_internal_schedule_external_event(cpu, channel, I2C_EVENT_SLAVE_TEN_SECOND,
                                                      address, 1u)) {
        cpu->stop_reason = DSPIC33_EVENT_QUEUE_ERROR;
    }
}

static void slave_ten_second(Dspic33* cpu, uint8_t channel, uint16_t address, bool interrupt) {
    uint16_t register_base = dspic33_i2c_bases[channel];
    uint16_t control_word = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_CON));
    uint16_t status_word = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_STAT));
    uint16_t configured_address =
        dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_ADD));
    uint16_t address_mask = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_MSK));
    bool is_acknowledged = dspic33_i2c_internal_slave_acknowledges(status_word);
    uint8_t channel_mask = (uint8_t)(1u << channel);

    if (!dspic33_i2c_internal_module_enabled(cpu, channel) ||
        (cpu->io.i2c_slave_active & channel_mask) == 0u ||
        cpu->io.i2c_slave_address[channel] != address) {
        return;
    }
    if ((control_word & I2C_SCLREL) == 0u) {
        dspic33_i2c_internal_schedule_external_event(cpu, channel, I2C_EVENT_SLAVE_TEN_SECOND,
                                                     address, 1u);
        return;
    }
    if ((control_word & I2C_IPMIEN) == 0u &&
        ((address ^ configured_address) & (uint16_t)~address_mask & 0x00ffu) != 0u) {
        dspic33_i2c_internal_record_slave_acknowledgement(cpu, channel, false);
        reject_slave(cpu, channel);
        return;
    }
    status_word = (uint16_t)((status_word | I2C_TEN_BIT) & ~(I2C_DATA | I2C_READ));
    if ((status_word & I2C_RBF) != 0u) {
        status_word |= I2C_OVERFLOW;
    } else {
        dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_RCV),
                                            address & 0x00ffu);
        status_word |= I2C_RBF;
    }
    control_word &= (uint16_t)~I2C_SCLREL;
    dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_CON), control_word);
    dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_STAT), status_word);
    dspic33_i2c_internal_record_slave_acknowledgement(cpu, channel, is_acknowledged);
    if (interrupt) {
        dspic33_i2c_internal_raise_slave(cpu, channel);
    }
}

static void slave_ten_restart(Dspic33* cpu, uint8_t channel, uint16_t address, bool interrupt) {
    uint16_t register_base = dspic33_i2c_bases[channel];
    uint16_t control_word = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_CON));
    uint16_t status_word = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_STAT));
    uint8_t channel_mask = (uint8_t)(1u << channel);
    bool is_acknowledged = dspic33_i2c_internal_slave_acknowledges(status_word);
    bool ipmi_enabled;

    if (!dspic33_i2c_internal_module_enabled(cpu, channel) ||
        (cpu->io.i2c_slave_active & channel_mask) == 0u ||
        cpu->io.i2c_slave_address[channel] != address || (status_word & I2C_TEN_BIT) == 0u) {
        return;
    }
    if (!ten_bit_high_matches(cpu, channel, address)) {
        dspic33_i2c_internal_record_slave_acknowledgement(cpu, channel, false);
        reject_slave(cpu, channel);
        return;
    }
    ipmi_enabled = (control_word & I2C_IPMIEN) != 0u;
    cpu->io.i2c_slave_read |= channel_mask;
    status_word =
        (uint16_t)((status_word | I2C_START_STATUS | I2C_READ) & ~(I2C_STOP_STATUS | I2C_DATA));
    if ((status_word & I2C_RBF) != 0u) {
        status_word |= I2C_OVERFLOW;
    } else {
        dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_RCV),
                                            (uint16_t)(0x00f1u | ((address >> 7u) & 6u)));
        status_word |= I2C_RBF;
    }
    if (ipmi_enabled) {
        cpu->io.i2c_slave_active &= (uint8_t)~channel_mask;
        cpu->io.i2c_slave_read &= (uint8_t)~channel_mask;
        control_word |= I2C_SCLREL;
    } else {
        control_word &= (uint16_t)~I2C_SCLREL;
    }
    dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_CON), control_word);
    dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_STAT), status_word);
    dspic33_i2c_internal_record_slave_acknowledgement(cpu, channel, is_acknowledged);
    if (interrupt) {
        dspic33_i2c_internal_raise_slave(cpu, channel);
    }
}

static void slave_write(Dspic33* cpu, uint8_t channel, uint8_t received_value,
                        bool raise_interrupt) {
    uint16_t register_base = dspic33_i2c_bases[channel];
    uint16_t control_word = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_CON));
    uint16_t status_word = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_STAT));
    bool is_acknowledged = dspic33_i2c_internal_slave_acknowledges(status_word);
    uint8_t channel_mask = (uint8_t)(1u << channel);

    if (!dspic33_i2c_internal_module_enabled(cpu, channel) ||
        (cpu->io.i2c_slave_active & channel_mask) == 0u ||
        (cpu->io.i2c_slave_read & channel_mask) != 0u) {
        return;
    }
    if ((control_word & I2C_SCLREL) == 0u) {
        dspic33_i2c_internal_schedule_external_event(cpu, channel, I2C_EVENT_SLAVE_WRITE,
                                                     received_value, 1u);
        return;
    }
    status_word |= I2C_DATA;
    if ((status_word & I2C_RBF) != 0u) {
        status_word |= I2C_OVERFLOW;
    } else {
        dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_RCV),
                                            received_value);
        status_word |= I2C_RBF;
    }
    if ((control_word & I2C_STREN) != 0u) {
        control_word &= (uint16_t)~I2C_SCLREL;
    }
    dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_CON), control_word);
    dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_STAT), status_word);
    dspic33_i2c_internal_record_slave_acknowledgement(cpu, channel, is_acknowledged);
    if (raise_interrupt) {
        dspic33_i2c_internal_raise_slave(cpu, channel);
    }
}

static void slave_read(Dspic33* cpu, uint8_t channel, bool is_acknowledged, bool require_buffer) {
    uint16_t register_base = dspic33_i2c_bases[channel];
    uint16_t control_word = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_CON));
    uint16_t status_word = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_STAT));
    uint8_t channel_mask = (uint8_t)(1u << channel);
    bool is_ten_bit = (status_word & I2C_TEN_BIT) != 0u;
    uint8_t transmit_value;

    if (!dspic33_i2c_internal_module_enabled(cpu, channel) ||
        (cpu->io.i2c_slave_active & channel_mask) == 0u ||
        (cpu->io.i2c_slave_read & channel_mask) == 0u ||
        (require_buffer && (status_word & I2C_TBF) == 0u)) {
        return;
    }
    if ((control_word & I2C_SCLREL) == 0u) {
        dspic33_i2c_internal_schedule_external_event(cpu, channel, I2C_EVENT_SLAVE_READ,
                                                     is_acknowledged ? 1u : 0u, 1u);
        return;
    }
    transmit_value =
        (uint8_t)dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_TRN));
    status_word &= (uint16_t)~I2C_TBF;
    status_word |= I2C_DATA;
    if (!is_acknowledged) {
        status_word |= I2C_NOT_ACKNOWLEDGED;
        cpu->io.i2c_slave_active &= (uint8_t)~channel_mask;
        cpu->io.i2c_slave_read &= (uint8_t)~channel_mask;
    } else {
        status_word &= (uint16_t)~I2C_NOT_ACKNOWLEDGED;
        control_word &= (uint16_t)~I2C_SCLREL;
    }
    dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_CON), control_word);
    dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_STAT), status_word);
    dspic33_i2c_internal_record_transfer(cpu, channel, DSPIC33_I2C_WRITE, transmit_value,
                                         is_acknowledged, false);
    if (is_acknowledged || !is_ten_bit) {
        dspic33_i2c_internal_raise_slave(cpu, channel);
    }
}

static void slave_stop(Dspic33* cpu, uint8_t channel) {
    uint16_t register_base = dspic33_i2c_bases[channel];
    uint16_t status_word = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_STAT));
    uint8_t channel_mask = (uint8_t)(1u << channel);

    if (!dspic33_i2c_internal_module_enabled(cpu, channel)) {
        return;
    }
    status_word = (uint16_t)((status_word | I2C_STOP_STATUS) &
                             ~(I2C_START_STATUS | I2C_TEN_BIT | I2C_GENERAL_CALL | I2C_TBF |
                               I2C_TRANSMIT_ACTIVE));
    dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_STAT), status_word);
    cpu->io.i2c_slave_active &= (uint8_t)~channel_mask;
    cpu->io.i2c_slave_read &= (uint8_t)~channel_mask;
    cpu->io.i2c_slave_rejected &= (uint8_t)~channel_mask;
}

void dspic33_i2c_internal_collide(Dspic33* cpu, uint8_t channel) {
    uint16_t register_base = dspic33_i2c_bases[channel];
    uint16_t control_word = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_CON));
    uint16_t status_word = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_STAT));
    uint8_t channel_mask = (uint8_t)(1u << channel);

    if (!dspic33_i2c_internal_module_enabled(cpu, channel)) {
        return;
    }
    control_word &= (uint16_t)~I2C_MASTER_MASK;
    status_word = (uint16_t)((status_word | I2C_BUS_COLLISION) & ~(I2C_TBF | I2C_TRANSMIT_ACTIVE));
    dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_CON), control_word);
    dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_STAT), status_word);
    dspic33_i2c_internal_pin_abort(cpu, channel);
    cpu->io.i2c_generation[channel]++;
    cpu->io.i2c_master_active &= (uint8_t)~channel_mask;
    dspic33_i2c_internal_record_transfer(cpu, channel, DSPIC33_I2C_COLLISION, 0u, false, true);
    dspic33_i2c_internal_raise_master(cpu, channel);
}

static bool slave_pin_operating(const Dspic33* cpu, uint8_t channel) {
    return dspic33_i2c_internal_module_enabled(cpu, channel) &&
           !(cpu->power_state == DSPIC33_POWER_IDLE &&
             (dspic33_i2c_internal_raw_word(cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_CON)) &
              0x2000u) != 0u);
}

static bool resolved_pin_high(const Dspic33* cpu, uint8_t channel,
                              const Dspic33I2cPinMapping* mapping, bool clock) {
    uint8_t channel_mask = (uint8_t)(1u << channel);

    if (((clock ? cpu->io.i2c_pin_clock_low : cpu->io.i2c_pin_data_low) & channel_mask) != 0u) {
        return false;
    }
    return dspic33_i2c_internal_pin_input_high(cpu, mapping, clock);
}

static void slave_pin_baseline(Dspic33* cpu, uint8_t channel, const Dspic33I2cPinMapping* mapping) {
    uint8_t channel_mask = (uint8_t)(1u << channel);

    if (resolved_pin_high(cpu, channel, mapping, true)) {
        cpu->io.i2c_pin_clock_high |= channel_mask;
    } else {
        cpu->io.i2c_pin_clock_high &= (uint8_t)~channel_mask;
    }
    if (resolved_pin_high(cpu, channel, mapping, false)) {
        cpu->io.i2c_pin_data_high |= channel_mask;
    } else {
        cpu->io.i2c_pin_data_high &= (uint8_t)~channel_mask;
    }
}

static void slave_pin_prepare_transmit(Dspic33* cpu, uint8_t channel) {
    uint16_t register_base = dspic33_i2c_bases[channel];
    uint16_t status_word = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_STAT));
    uint8_t transmit_bit_index = cpu->io.i2c_slave_pin_bits[channel];

    if (cpu->io.i2c_slave_pin_state[channel] != I2C_SLAVE_PIN_TRANSMIT) {
        return;
    }
    if (transmit_bit_index >= 8u || (status_word & I2C_TBF) == 0u) {
        dspic33_i2c_internal_pin_set_low(cpu, channel, false, false);
        return;
    }
    dspic33_i2c_internal_pin_set_low(
        cpu, channel, false,
        (dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_TRN)) &
         (uint16_t)(0x0080u >> transmit_bit_index)) == 0u);
}

static void slave_pin_start(Dspic33* cpu, uint8_t channel) {
    uint8_t channel_mask = (uint8_t)(1u << channel);

    cpu->io.i2c_pin_physical |= channel_mask;
    cpu->io.i2c_slave_pin_active |= channel_mask;
    cpu->io.i2c_slave_pin_acknowledge &= (uint8_t)~channel_mask;
    cpu->io.i2c_slave_pin_interrupt &= (uint8_t)~channel_mask;
    cpu->io.i2c_slave_pin_stretch &= (uint8_t)~channel_mask;
    cpu->io.i2c_slave_pin_state[channel] = I2C_SLAVE_PIN_ADDRESS;
    cpu->io.i2c_slave_pin_next[channel] = I2C_SLAVE_PIN_IDLE;
    cpu->io.i2c_slave_pin_bits[channel] = 0u;
    cpu->io.i2c_slave_pin_shift[channel] = 0u;
    dspic33_i2c_internal_pin_set_low(cpu, channel, true, false);
    dspic33_i2c_internal_pin_set_low(cpu, channel, false, false);
}

static void slave_pin_stop(Dspic33* cpu, uint8_t channel) {
    uint8_t channel_mask = (uint8_t)(1u << channel);

    slave_stop(cpu, channel);
    cpu->io.i2c_pin_physical &= (uint8_t)~channel_mask;
    cpu->io.i2c_slave_pin_active &= (uint8_t)~channel_mask;
    cpu->io.i2c_slave_pin_acknowledge &= (uint8_t)~channel_mask;
    cpu->io.i2c_slave_pin_interrupt &= (uint8_t)~channel_mask;
    cpu->io.i2c_slave_pin_stretch &= (uint8_t)~channel_mask;
    cpu->io.i2c_slave_pin_state[channel] = I2C_SLAVE_PIN_IDLE;
    cpu->io.i2c_slave_pin_next[channel] = I2C_SLAVE_PIN_IDLE;
    cpu->io.i2c_slave_pin_bits[channel] = 0u;
    dspic33_i2c_internal_pin_set_low(cpu, channel, true, false);
    dspic33_i2c_internal_pin_set_low(cpu, channel, false, false);
}

static void slave_pin_receive_rising(Dspic33* cpu, uint8_t channel, bool input_data_high) {
    cpu->io.i2c_slave_pin_shift[channel] =
        (uint8_t)((cpu->io.i2c_slave_pin_shift[channel] << 1u) | (input_data_high ? 1u : 0u));
    cpu->io.i2c_slave_pin_bits[channel]++;
    if (cpu->io.i2c_slave_pin_bits[channel] != 8u) {
        return;
    }
    cpu->io.i2c_slave_pin_next[channel] = cpu->io.i2c_slave_pin_state[channel];
    cpu->io.i2c_slave_pin_state[channel] = I2C_SLAVE_PIN_RECEIVED;
}

static void slave_pin_receive_falling(Dspic33* cpu, uint8_t channel) {
    uint16_t register_base = dspic33_i2c_bases[channel];
    uint16_t status_word = dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_STAT));
    uint8_t channel_mask = (uint8_t)(1u << channel);
    uint8_t next_state = cpu->io.i2c_slave_pin_next[channel];
    bool is_acknowledged = dspic33_i2c_internal_slave_acknowledges(status_word);
    bool raise_interrupt = false;
    uint16_t resulting_control_word;

    if (next_state == I2C_SLAVE_PIN_ADDRESS) {
        uint8_t address_byte = cpu->io.i2c_slave_pin_shift[channel];
        bool is_read = (address_byte & 1u) != 0u;

        if ((address_byte & 0xf8u) == 0xf0u) {
            uint16_t address = (uint16_t)((address_byte & 6u) << 7u);
            if (is_read) {
                address = cpu->io.i2c_slave_address[channel];
                slave_ten_restart(cpu, channel, address, false);
            } else {
                slave_start(cpu, channel, (uint16_t)(address | I2C_EXTERNAL_TEN_BIT), false, false);
            }
        } else {
            uint8_t address = (uint8_t)(address_byte >> 1u);
            slave_start(cpu, channel, (uint16_t)(address | (is_read ? I2C_EXTERNAL_READ : 0u)),
                        true, false);
        }
        is_acknowledged = is_acknowledged && (cpu->io.i2c_slave_active & channel_mask) != 0u &&
                          (cpu->io.i2c_slave_rejected & channel_mask) == 0u;
        raise_interrupt = is_acknowledged;
        if (is_acknowledged && (address_byte & 0xf8u) == 0xf0u && !is_read) {
            cpu->io.i2c_slave_pin_next[channel] = I2C_SLAVE_PIN_TEN_SECOND;
        } else {
            cpu->io.i2c_slave_pin_next[channel] =
                is_acknowledged ? (is_read ? I2C_SLAVE_PIN_TRANSMIT : I2C_SLAVE_PIN_RECEIVE)
                                : I2C_SLAVE_PIN_REJECTED;
        }
    } else if (next_state == I2C_SLAVE_PIN_TEN_SECOND) {
        uint16_t address = (uint16_t)((cpu->io.i2c_slave_address[channel] & 0x0300u) |
                                      cpu->io.i2c_slave_pin_shift[channel]);
        cpu->io.i2c_slave_address[channel] = address;
        slave_ten_second(cpu, channel, address, false);
        is_acknowledged = is_acknowledged && (cpu->io.i2c_slave_active & channel_mask) != 0u &&
                          (cpu->io.i2c_slave_rejected & channel_mask) == 0u;
        raise_interrupt = is_acknowledged;
        cpu->io.i2c_slave_pin_next[channel] =
            is_acknowledged ? I2C_SLAVE_PIN_RECEIVE : I2C_SLAVE_PIN_REJECTED;
    } else {
        slave_write(cpu, channel, cpu->io.i2c_slave_pin_shift[channel], false);
        is_acknowledged = is_acknowledged && (cpu->io.i2c_slave_active & channel_mask) != 0u;
        raise_interrupt = (cpu->io.i2c_slave_active & channel_mask) != 0u;
        cpu->io.i2c_slave_pin_next[channel] = I2C_SLAVE_PIN_RECEIVE;
    }
    if (is_acknowledged) {
        cpu->io.i2c_slave_pin_acknowledge |= channel_mask;
    } else {
        cpu->io.i2c_slave_pin_acknowledge &= (uint8_t)~channel_mask;
    }
    if (raise_interrupt) {
        cpu->io.i2c_slave_pin_interrupt |= channel_mask;
    } else {
        cpu->io.i2c_slave_pin_interrupt &= (uint8_t)~channel_mask;
    }
    resulting_control_word =
        dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_CON));
    if ((resulting_control_word & I2C_SCLREL) == 0u) {
        cpu->io.i2c_slave_pin_stretch |= channel_mask;
        dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_CON),
                                            (uint16_t)(resulting_control_word | I2C_SCLREL));
    } else {
        cpu->io.i2c_slave_pin_stretch &= (uint8_t)~channel_mask;
    }
    cpu->io.i2c_slave_pin_state[channel] = I2C_SLAVE_PIN_ACKNOWLEDGE;
    cpu->io.i2c_slave_pin_bits[channel] = 0u;
    cpu->io.i2c_slave_pin_shift[channel] = 0u;
    dspic33_i2c_internal_pin_set_low(cpu, channel, false, is_acknowledged);
    cpu->io.i2c_slave_pin_bits[channel] = 1u;
}

static void slave_pin_rising(Dspic33* cpu, uint8_t channel, bool input_data_high) {
    uint8_t pin_state = cpu->io.i2c_slave_pin_state[channel];

    if (pin_state == I2C_SLAVE_PIN_ADDRESS || pin_state == I2C_SLAVE_PIN_RECEIVE ||
        pin_state == I2C_SLAVE_PIN_TEN_SECOND) {
        slave_pin_receive_rising(cpu, channel, input_data_high);
    } else if (pin_state == I2C_SLAVE_PIN_ACKNOWLEDGE) {
        if (cpu->io.i2c_slave_pin_bits[channel] == 1u) {
            cpu->io.i2c_slave_pin_bits[channel] = 2u;
        }
    } else if (pin_state == I2C_SLAVE_PIN_TRANSMIT) {
        cpu->io.i2c_slave_pin_bits[channel]++;
        if (cpu->io.i2c_slave_pin_bits[channel] == 8u) {
            cpu->io.i2c_slave_pin_state[channel] = I2C_SLAVE_PIN_MASTER_ACKNOWLEDGE;
            cpu->io.i2c_slave_pin_bits[channel] = 0u;
        }
    } else if (pin_state == I2C_SLAVE_PIN_MASTER_ACKNOWLEDGE &&
               cpu->io.i2c_slave_pin_bits[channel] == 1u) {
        cpu->io.i2c_slave_pin_shift[channel] = input_data_high ? 0u : 1u;
        cpu->io.i2c_slave_pin_bits[channel] = 2u;
    }
}

static void slave_pin_falling(Dspic33* cpu, uint8_t channel) {
    uint8_t channel_mask = (uint8_t)(1u << channel);
    uint8_t pin_state = cpu->io.i2c_slave_pin_state[channel];

    if (pin_state == I2C_SLAVE_PIN_RECEIVED) {
        slave_pin_receive_falling(cpu, channel);
    } else if (pin_state == I2C_SLAVE_PIN_ACKNOWLEDGE) {
        if (cpu->io.i2c_slave_pin_bits[channel] == 0u) {
            dspic33_i2c_internal_pin_set_low(
                cpu, channel, false,
                (cpu->io.i2c_slave_pin_acknowledge & (uint8_t)(1u << channel)) != 0u);
            cpu->io.i2c_slave_pin_bits[channel] = 1u;
        } else if (cpu->io.i2c_slave_pin_bits[channel] == 2u) {
            dspic33_i2c_internal_pin_set_low(cpu, channel, false, false);
            cpu->io.i2c_slave_pin_state[channel] = cpu->io.i2c_slave_pin_next[channel];
            cpu->io.i2c_slave_pin_bits[channel] = 0u;
            if ((cpu->io.i2c_slave_pin_stretch & channel_mask) != 0u) {
                uint16_t register_base = dspic33_i2c_bases[channel];
                uint16_t control_word =
                    dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_CON));
                cpu->io.i2c_slave_pin_stretch &= (uint8_t)~channel_mask;
                dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_CON),
                                                    (uint16_t)(control_word & ~I2C_SCLREL));
                dspic33_i2c_internal_pin_set_low(cpu, channel, true, true);
            }
            if ((cpu->io.i2c_slave_pin_interrupt & channel_mask) != 0u) {
                cpu->io.i2c_slave_pin_interrupt &= (uint8_t)~channel_mask;
                dspic33_i2c_internal_raise_slave(cpu, channel);
            }
            slave_pin_prepare_transmit(cpu, channel);
        }
    } else if (pin_state == I2C_SLAVE_PIN_TRANSMIT) {
        slave_pin_prepare_transmit(cpu, channel);
    } else if (pin_state == I2C_SLAVE_PIN_MASTER_ACKNOWLEDGE) {
        if (cpu->io.i2c_slave_pin_bits[channel] == 0u) {
            uint16_t register_base = dspic33_i2c_bases[channel];
            uint16_t status_word =
                dspic33_i2c_internal_raw_word(cpu, (uint16_t)(register_base + I2C_STAT));
            dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_STAT),
                                                (uint16_t)(status_word & ~I2C_TBF));
            dspic33_i2c_internal_pin_set_low(cpu, channel, false, false);
            cpu->io.i2c_slave_pin_bits[channel] = 1u;
        } else if (cpu->io.i2c_slave_pin_bits[channel] == 2u) {
            bool acknowledge = cpu->io.i2c_slave_pin_shift[channel] != 0u;
            slave_read(cpu, channel, acknowledge, false);
            cpu->io.i2c_slave_pin_state[channel] =
                acknowledge ? I2C_SLAVE_PIN_TRANSMIT : I2C_SLAVE_PIN_REJECTED;
            cpu->io.i2c_slave_pin_bits[channel] = 0u;
            cpu->io.i2c_slave_pin_shift[channel] = 0u;
            slave_pin_prepare_transmit(cpu, channel);
        }
    }
}

void dspic33_i2c_refresh_pins(Dspic33* cpu) {
    uint8_t channel;

    for (channel = 0u; channel < DSPIC33_I2C_COUNT; channel++) {
        Dspic33I2cPinMapping mapping;
        uint8_t channel_mask = (uint8_t)(1u << channel);
        bool clock_high;
        bool data_high;
        bool previous_clock_high;
        bool previous_data_high;
        if (!dspic33_i2c_internal_pin_mapping(cpu, channel, &mapping)) {
            continue;
        }
        if ((cpu->io.i2c_pin_active & channel_mask) == 0u &&
            (cpu->io.i2c_slave_active & channel_mask) != 0u) {
            dspic33_i2c_internal_pin_set_low(
                cpu, channel, true,
                (dspic33_i2c_internal_raw_word(cpu,
                                               (uint16_t)(dspic33_i2c_bases[channel] + I2C_CON)) &
                 I2C_SCLREL) == 0u);
        }
        clock_high = resolved_pin_high(cpu, channel, &mapping, true);
        data_high = resolved_pin_high(cpu, channel, &mapping, false);
        previous_clock_high = (cpu->io.i2c_pin_clock_high & channel_mask) != 0u;
        previous_data_high = (cpu->io.i2c_pin_data_high & channel_mask) != 0u;
        if (!slave_pin_operating(cpu, channel) || (cpu->io.i2c_pin_active & channel_mask) != 0u ||
            (cpu->io.i2c_master_active & channel_mask) != 0u) {
            slave_pin_baseline(cpu, channel, &mapping);
            continue;
        }
        slave_pin_prepare_transmit(cpu, channel);
        clock_high = resolved_pin_high(cpu, channel, &mapping, true);
        data_high = resolved_pin_high(cpu, channel, &mapping, false);
        if (previous_data_high && !data_high && clock_high) {
            slave_pin_start(cpu, channel);
        } else if (!previous_data_high && data_high && clock_high &&
                   (cpu->io.i2c_slave_pin_active & channel_mask) != 0u) {
            slave_pin_stop(cpu, channel);
        } else if (!previous_clock_high && clock_high &&
                   (cpu->io.i2c_slave_pin_active & channel_mask) != 0u) {
            slave_pin_rising(cpu, channel, data_high);
        } else if (previous_clock_high && !clock_high &&
                   (cpu->io.i2c_slave_pin_active & channel_mask) != 0u) {
            slave_pin_falling(cpu, channel);
        }
        slave_pin_baseline(cpu, channel, &mapping);
    }
}

bool dspic33_i2c_write_register(Dspic33* cpu, uint16_t address, uint16_t previous_word,
                                uint16_t requested_value) {
    uint8_t channel;
    uint16_t offset;
    uint16_t register_base = (uint16_t)(address & 0xfffeu);

    if (!dspic33_i2c_internal_channel_for_address(register_base, &channel, &offset)) {
        return false;
    }
    if (dspic33_i2c_internal_module_disabled(cpu, channel)) {
        dspic33_i2c_internal_raw_write_word(cpu, register_base, previous_word);
        return true;
    }
    if (offset == I2C_RCV) {
        dspic33_i2c_internal_raw_write_word(cpu, register_base, previous_word);
    } else if (offset == I2C_TRN) {
        if ((address & 1u) == 0u || cpu->io.cpu_write_width == 2u) {
            dspic33_i2c_internal_write_transmit(cpu, channel, previous_word,
                                                (uint8_t)requested_value);
        } else {
            dspic33_i2c_internal_raw_write_word(cpu, register_base, previous_word);
        }
    } else if (offset == I2C_BRG) {
        dspic33_i2c_internal_raw_write_word(cpu, register_base, requested_value & 0x01ffu);
    } else if (offset == I2C_CON) {
        uint16_t control_word = requested_value & 0xbfffu;
        uint16_t active_operations = previous_word & I2C_MASTER_MASK;
        bool disabling = (previous_word & I2C_ENABLE) != 0u && (control_word & I2C_ENABLE) == 0u;

        if ((control_word & I2C_STREN) == 0u && (control_word & I2C_SCLREL) == 0u) {
            control_word |= previous_word & I2C_SCLREL;
        }
        if (disabling) {
            control_word = (uint16_t)((control_word & ~I2C_MASTER_MASK) | I2C_SCLREL);
        } else if (active_operations != 0u) {
            control_word = (uint16_t)((control_word & ~I2C_MASTER_MASK) | active_operations);
        }
        if ((control_word & I2C_ENABLE) == 0u) {
            control_word |= I2C_SCLREL;
        }
        dspic33_i2c_internal_raw_write_word(cpu, register_base, control_word);
        if (disabling) {
            dspic33_i2c_internal_raw_write_word(
                cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_STAT), 0u);
            dspic33_i2c_internal_reset_runtime(cpu, channel);
        } else if ((control_word & I2C_ENABLE) != 0u && active_operations == 0u) {
            dspic33_i2c_internal_begin_control(cpu, channel, control_word & I2C_MASTER_MASK);
        }
    } else if (offset == I2C_STAT) {
        dspic33_i2c_internal_raw_write_word(
            cpu, register_base,
            (uint16_t)((previous_word & ~0x04c0u) | (previous_word & requested_value & 0x04c0u)));
    } else if (offset == I2C_ADD || offset == I2C_MSK) {
        dspic33_i2c_internal_raw_write_word(cpu, register_base, requested_value & 0x03ffu);
    }
    dspic33_i2c_refresh_pins(cpu);
    return true;
}

bool dspic33_i2c_read_register(Dspic33* cpu, uint16_t address, uint8_t* read_value) {
    uint8_t channel;
    uint16_t offset;
    uint16_t register_base = (uint16_t)(address & 0xfffeu);

    if (!dspic33_i2c_internal_channel_for_address(register_base, &channel, &offset)) {
        return false;
    }
    if (offset == I2C_TRN) {
        *read_value = 0u;
    } else {
        *read_value = cpu->data[address];
    }
    if (offset == I2C_RCV) {
        dspic33_i2c_internal_raw_write_word(
            cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_STAT),
            (uint16_t)(dspic33_i2c_internal_raw_word(
                           cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_STAT)) &
                       ~I2C_RBF));
    }
    return true;
}

void dspic33_i2c_process_event(Dspic33* cpu, uint8_t channel, uint32_t event_value,
                               bool is_external) {
    uint8_t event_kind = (uint8_t)(event_value >> I2C_EVENT_KIND_SHIFT);
    uint8_t event_generation = (uint8_t)(event_value >> I2C_EVENT_GENERATION_SHIFT);
    uint16_t event_payload = (uint16_t)(event_value & I2C_EVENT_PAYLOAD_MASK);

    if (channel >= DSPIC33_I2C_COUNT) {
        return;
    }
    if (event_kind == I2C_EVENT_PMD) {
        uint8_t channel_mask = (uint8_t)(1u << channel);
        bool module_disabled = (event_payload & 1u) != 0u;

        if (event_generation != cpu->io.i2c_pmd_generation[channel]) {
            return;
        }
        if (module_disabled) {
            cpu->io.i2c_pmd_disabled |= channel_mask;
            dspic33_i2c_internal_pause_events(cpu, channel);
        } else {
            cpu->io.i2c_pmd_disabled &= (uint8_t)~channel_mask;
            dspic33_i2c_internal_resume_events(cpu, channel);
        }
        dspic33_i2c_refresh_pins(cpu);
        return;
    }
    if ((event_kind <= I2C_EVENT_TRANSMIT_SHIFT || event_kind == I2C_EVENT_PIN) &&
        event_generation != cpu->io.i2c_generation[channel]) {
        return;
    }
    if (cpu->power_state == DSPIC33_POWER_IDLE &&
        (dspic33_i2c_internal_raw_word(cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_CON)) &
         0x2000u) != 0u) {
        dspic33_i2c_internal_schedule_event(cpu, channel, event_value, 1u, is_external);
        return;
    }
    if (event_kind == I2C_EVENT_CONTROL) {
        dspic33_i2c_internal_complete_control(cpu, channel, event_payload);
    } else if (event_kind == I2C_EVENT_BUS_STATUS) {
        dspic33_i2c_internal_complete_bus_status(cpu, channel, event_payload);
    } else if (event_kind == I2C_EVENT_TRANSMIT) {
        dspic33_i2c_internal_complete_transmit(cpu, channel);
    } else if (event_kind == I2C_EVENT_TRANSMIT_SHIFT) {
        dspic33_i2c_internal_complete_transmit_shift(cpu, channel);
    } else if (event_kind == I2C_EVENT_SLAVE_START) {
        slave_start(cpu, channel, event_payload, true, true);
    } else if (event_kind == I2C_EVENT_SLAVE_WRITE) {
        slave_write(cpu, channel, (uint8_t)event_payload, true);
    } else if (event_kind == I2C_EVENT_SLAVE_READ) {
        slave_read(cpu, channel, event_payload != 0u, true);
    } else if (event_kind == I2C_EVENT_SLAVE_STOP) {
        slave_stop(cpu, channel);
    } else if (event_kind == I2C_EVENT_COLLISION) {
        dspic33_i2c_internal_collide(cpu, channel);
    } else if (event_kind == I2C_EVENT_SLAVE_TEN_SECOND) {
        slave_ten_second(cpu, channel, event_payload & 0x03ffu, true);
    } else if (event_kind == I2C_EVENT_SLAVE_TEN_RESTART) {
        slave_ten_restart(cpu, channel, event_payload & 0x03ffu, true);
    } else if (event_kind == I2C_EVENT_PIN) {
        dspic33_i2c_internal_pin_run(cpu, channel);
    }
}

bool dspic33_i2c_respond(Dspic33* cpu, uint8_t channel, uint8_t response_value, bool acknowledge,
                         uint64_t delay) {
    Dspic33I2cResponse response;
    if (channel >= DSPIC33_I2C_COUNT || delay > UINT64_MAX - cpu->device_cycles) {
        return false;
    }
    response.cycle = cpu->device_cycles + delay;
    response.value = response_value;
    response.acknowledge = acknowledge;
    return dspic33_i2c_internal_response_push(&cpu->io.i2c_response[channel], &response);
}

bool dspic33_i2c_status(Dspic33* cpu, uint8_t channel, uint16_t requested_status) {
    const uint16_t hardware_status = 0x84c0u;
    uint16_t current_status;

    if (channel >= DSPIC33_I2C_COUNT || (requested_status & (uint16_t)~hardware_status) != 0u) {
        return false;
    }
    current_status =
        dspic33_i2c_internal_raw_word(cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_STAT));
    dspic33_i2c_internal_raw_write_word(
        cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_STAT),
        (uint16_t)((current_status & ~hardware_status) | requested_status));
    return true;
}

bool dspic33_i2c_slave_start(Dspic33* cpu, uint8_t channel, uint16_t address, bool is_read,
                             bool is_ten_bit, uint64_t delay) {
    uint16_t payload;
    if (channel >= DSPIC33_I2C_COUNT || address > (is_ten_bit ? 0x03ffu : 0x007fu)) {
        return false;
    }
    if (is_ten_bit && is_read) {
        uint8_t channel_mask = (uint8_t)(1u << channel);

        if ((cpu->io.i2c_slave_active & channel_mask) == 0u ||
            cpu->io.i2c_slave_address[channel] != address ||
            (dspic33_i2c_internal_raw_word(cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_STAT)) &
             I2C_TEN_BIT) == 0u) {
            return false;
        }
        return dspic33_i2c_internal_schedule_external_event(
            cpu, channel, I2C_EVENT_SLAVE_TEN_RESTART, address, delay);
    }
    payload = (uint16_t)(address | (is_read ? I2C_EXTERNAL_READ : 0u) |
                         (is_ten_bit ? I2C_EXTERNAL_TEN_BIT : 0u));
    return dspic33_i2c_internal_schedule_external_event(cpu, channel, I2C_EVENT_SLAVE_START,
                                                        payload, delay);
}

bool dspic33_i2c_slave_write(Dspic33* cpu, uint8_t channel, uint8_t transmit_value,
                             uint64_t delay) {
    return channel < DSPIC33_I2C_COUNT &&
           dspic33_i2c_internal_schedule_external_event(cpu, channel, I2C_EVENT_SLAVE_WRITE,
                                                        transmit_value, delay);
}

bool dspic33_i2c_slave_read(Dspic33* cpu, uint8_t channel, bool is_acknowledged, uint64_t delay) {
    return channel < DSPIC33_I2C_COUNT &&
           dspic33_i2c_internal_schedule_external_event(cpu, channel, I2C_EVENT_SLAVE_READ,
                                                        is_acknowledged ? 1u : 0u, delay);
}

bool dspic33_i2c_slave_stop(Dspic33* cpu, uint8_t channel, uint64_t delay) {
    return channel < DSPIC33_I2C_COUNT && dspic33_i2c_internal_schedule_external_event(
                                              cpu, channel, I2C_EVENT_SLAVE_STOP, 0u, delay);
}

bool dspic33_i2c_collision(Dspic33* cpu, uint8_t channel, uint64_t delay) {
    return channel < DSPIC33_I2C_COUNT && dspic33_i2c_internal_schedule_external_event(
                                              cpu, channel, I2C_EVENT_COLLISION, 0u, delay);
}

bool dspic33_i2c_transmit(Dspic33* cpu, uint8_t channel, Dspic33I2cTransfer* transfer) {
    return channel < DSPIC33_I2C_COUNT && transfer != NULL &&
           dspic33_i2c_internal_transfer_pop(&cpu->io.i2c_tx[channel], transfer);
}

bool dspic33_i2c_pin(const Dspic33* cpu, uint8_t port, uint8_t pin, bool* high) {
    uint8_t channel;

    for (channel = 0u; channel < DSPIC33_I2C_COUNT; channel++) {
        Dspic33I2cPinMapping mapping;
        uint16_t control_word;
        uint16_t status_word;
        uint8_t channel_mask;
        bool is_clock_pin;

        if (!dspic33_i2c_internal_pin_mapping(cpu, channel, &mapping) || mapping.port != port ||
            (mapping.clock != pin && mapping.data != pin) ||
            !dspic33_i2c_internal_module_enabled(cpu, channel)) {
            continue;
        }
        if (high == NULL) {
            return false;
        }
        control_word =
            dspic33_i2c_internal_raw_word(cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_CON));
        status_word =
            dspic33_i2c_internal_raw_word(cpu, (uint16_t)(dspic33_i2c_bases[channel] + I2C_STAT));
        channel_mask = (uint8_t)(1u << channel);
        is_clock_pin = mapping.clock == pin;
        if ((cpu->io.i2c_pin_physical & channel_mask) != 0u) {
            if ((is_clock_pin ? cpu->io.i2c_pin_clock_low : cpu->io.i2c_pin_data_low) &
                channel_mask) {
                *high = false;
                return true;
            }
            return dspic33_device_gpio_input_high(cpu, port, pin, high);
        }
        if ((control_word & I2C_MASTER_MASK) != 0u || (status_word & I2C_TRANSMIT_ACTIVE) != 0u ||
            (((cpu->io.i2c_slave_active & cpu->io.i2c_slave_read) & channel_mask) != 0u &&
             (control_word & I2C_SCLREL) != 0u)) {
            return false;
        }
        if (is_clock_pin && (((cpu->io.i2c_slave_active & channel_mask) != 0u &&
                              (control_word & I2C_SCLREL) == 0u) ||
                             (cpu->io.i2c_master_active & channel_mask) != 0u)) {
            *high = false;
            return true;
        }
        if (!is_clock_pin && (cpu->io.i2c_master_active & channel_mask) != 0u) {
            return false;
        }
        return dspic33_device_gpio_input_high(cpu, port, pin, high);
    }
    return false;
}

void dspic33_i2c_reset(Dspic33* cpu) {
    uint8_t channel;

    for (channel = 0u; channel < DSPIC33_I2C_COUNT; channel++) {
        uint16_t register_base = dspic33_i2c_bases[channel];

        dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_RCV), 0u);
        dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_TRN), 0x00ffu);
        dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_BRG), 0u);
        dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_CON), I2C_SCLREL);
        dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_STAT), 0u);
        dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_ADD), 0u);
        dspic33_i2c_internal_raw_write_word(cpu, (uint16_t)(register_base + I2C_MSK), 0u);
    }
}
