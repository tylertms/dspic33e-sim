...
static void can_abort_transmissions(Dspic33* cpu, uint8_t channel) {
    ...
    for (buffer = 0u; buffer < 8u; buffer++) {
        ...
        if ((control & CAN_BUFFER_REQUEST) != 0u) {
            ...
            dspic33_device_internal_can_raise_event(cpu, channel, CAN_INTERRUPT_TRANSMIT, buffer,
                                                    0u);
        }
    }
...
static void can_clear_receive_flags(Dspic33* cpu, uint8_t channel, uint16_t address,
    ...
    uint8_t high = address == dspic33_device_can_bases[channel] + 0x22u ? 16u : 0u;
    uint8_t bit;
    dspic33_device_internal_raw_write_word(cpu, address, result);
...
static void can_update_transmit_control(Dspic33* cpu, uint8_t channel, uint16_t address,
                                        uint16_t previous) {
    uint8_t first = (uint8_t)(((address - dspic33_device_can_bases[channel]) - 0x30u));
    uint8_t half;
    for (half = 0u; half < 2u; half++) {
        ...
        uint16_t current = dspic33_device_internal_can_buffer_control(cpu, channel, buffer);
        if ((current & CAN_BUFFER_REQUEST) != 0u && (before & CAN_BUFFER_REQUEST) == 0u) {
            ...
            dspic33_device_internal_can_set_buffer_control(cpu, channel, buffer, current);
            ...
            dspic33_device_internal_can_set_buffer_control(
                cpu, channel, buffer, (uint16_t)(current | CAN_BUFFER_ABORTED));
            dspic33_device_internal_can_raise_event(cpu, channel, CAN_INTERRUPT_TRANSMIT, buffer,
                                                    0u);
            if (active) {
                dspic33_device_internal_can_remove_transmit_events(cpu, channel);
...
void dspic33_device_internal_update_can_register(Dspic33* cpu, uint16_t address, uint16_t previous,
    ...
    for (channel = 0u; channel < DSPIC33_CAN_COUNT; channel++) {
        ...
        window = (dspic33_device_internal_raw_word(cpu, base) & CAN_WINDOW) != 0u;
        if (offset == 0u) {
        ...
        if (window && offset >= 0x20u) {
            uint16_t prior = dspic33_device_internal_can_filter_word(cpu, channel, offset);
        ...
        if (offset == 6u) {
            ...
            dspic33_device_internal_raw_write_word(cpu, (uint16_t)(base + 8u),
