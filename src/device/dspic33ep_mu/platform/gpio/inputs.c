...
uint16_t dspic33_device_internal_gpio_pin_values(const Dspic33* cpu, uint8_t port) {
    uint16_t tris_mask =
        dspic33_device_internal_raw_word(cpu, dspic33_device_gpio_tris_addresses[port]);
    uint16_t latch_value =
        dspic33_device_internal_raw_word(cpu, dspic33_device_gpio_latch_addresses[port]);
    uint16_t open_drain_mask =
        dspic33_device_internal_raw_word(cpu, dspic33_device_gpio_open_drain_addresses[port]);
    uint16_t driven_mask = cpu->io.gpio_driven[port];
    uint16_t pull_up_mask =
        dspic33_device_internal_raw_word(cpu, dspic33_device_gpio_pull_up_addresses[port]);
    uint16_t pull_down_mask =
        dspic33_device_internal_raw_word(cpu, dspic33_device_gpio_pull_down_addresses[port]);
    uint16_t input_mask = (uint16_t)(tris_mask | dspic33_device_gpio_input_only_masks[port]);
    uint16_t analog_mask =
        dspic33_device_gpio_analog_addresses[port] != 0u
            ? dspic33_device_internal_raw_word(cpu, dspic33_device_gpio_analog_addresses[port])
            : 0u;
    ...
    uint16_t external_level = (uint16_t)((cpu->io.gpio[port] & driven_mask) |
                                         (pull_up_mask & ~driven_mask & ~pull_down_mask));
    uint16_t pin_values;
    if (port == 2u && (cpu->configuration[8u] & 0x04u) == 0u) {
        uint8_t source = dspic33_device_internal_oscillator_current_source(
            dspic33_device_internal_raw_word(cpu, OSCILLATOR_CONTROL));
        bool frc = source == 0u || source == 1u || source == 6u || source == 7u;
        bool external_clock =
            (source == 2u || source == 3u) && (cpu->configuration[8u] & 0x03u) == 0u;
        if ((frc || external_clock) &&
            (dspic33_device_internal_raw_word(cpu, dspic33_device_gpio_open_drain_addresses[0]) &
             0x0008u) != 0u) {
            open_drain_mask |= 0x8000u;
        }
    }
    if (port == 6u && (dspic33_device_internal_raw_word(cpu, USB_CON) & USB_ENABLE) != 0u) {
        analog_mask |= 0x000cu;
    }
    pin_values = (uint16_t)(((external_level & input_mask & ~analog_mask) |
                             (latch_value & ~input_mask & ~open_drain_mask) |
                             (external_level & ~input_mask & open_drain_mask & latch_value)) &
                            dspic33_device_internal_gpio_port_mask(cpu, port));
    for (uint8_t pin = 0u; pin < 16u; pin++) {
        bool high;
        uint16_t pin_bit = (uint16_t)(1u << pin);
        if (dspic33_device_internal_pwm_pin_value(cpu, port, pin, &high)) {
            pin_values =
                high ? (uint16_t)(pin_values | pin_bit) : (uint16_t)(pin_values & ~pin_bit);
        } else if (dspic33_device_internal_uart_pps_output_value(cpu, port, pin, &high)) {
            if (high && (open_drain_mask & pin_bit) != 0u) {
                pin_values = (external_level & pin_bit) != 0u ? (uint16_t)(pin_values | pin_bit)
                                                              : (uint16_t)(pin_values & ~pin_bit);
            } else {
                pin_values =
                    high ? (uint16_t)(pin_values | pin_bit) : (uint16_t)(pin_values & ~pin_bit);
            }
        } else if (dspic33_device_internal_qei_pps_output_value(cpu, port, pin, &high)) {
            if (high && (open_drain_mask & pin_bit) != 0u) {
                pin_values = (external_level & pin_bit) != 0u ? (uint16_t)(pin_values | pin_bit)
                                                              : (uint16_t)(pin_values & ~pin_bit);
            } else {
    ...
    if (port == 2u && dspic33_device_internal_oscillator_pin_owned(cpu)) {
        pin_values &= 0x7fffu;
    }
    return pin_values;
}
...
void dspic33_device_internal_refresh_gpio_change_notification(Dspic33* cpu) {
    ...
    for (uint8_t port = 0u; port < DSPIC33_GPIO_PORT_COUNT; port++) {
        ...
        uint16_t pin_values = dspic33_device_internal_gpio_pin_values(cpu, port);
        uint16_t qualified_mask = gpio_change_notification_qualified(cpu, port);
        uint16_t newly_qualified = (uint16_t)(qualified_mask & ~cpu->io.gpio_cn_qualified[port]);
        cpu->io.gpio_cn_reference[port] =
            (uint16_t)((cpu->io.gpio_cn_reference[port] & qualified_mask & ~newly_qualified) |
                       (pin_values & (~qualified_mask | newly_qualified)));
        cpu->io.gpio_cn_values[port] = pin_values;
        cpu->io.gpio_cn_qualified[port] = qualified_mask;
        changed =
...
static uint8_t external_interrupt_selection(const Dspic33* cpu, uint8_t channel) {
    ...
    return (
        uint8_t)((dspic33_device_internal_raw_word(cpu, addresses[channel]) >> shifts[channel]) &
                 0x007fu);
}
...
void dspic33_device_internal_refresh_external_interrupts(Dspic33* cpu) {
    ...
    for (channel = 0u; channel < DSPIC33_EXTERNAL_INTERRUPT_COUNT; channel++) {
        ...
        if (!reconfigured && qualified && high != previous_high) {
            bool falling = (dspic33_device_internal_raw_word(cpu, 0x08c2u) & mask) != 0u;
...
static uint8_t timer_input_selection(const Dspic33* cpu, uint8_t timer) {
    ...
    uint16_t mapping =
        dspic33_device_internal_raw_word(cpu, (uint16_t)(0x06a6u + ((timer - 1u) / 2u) * 2u));
...
void dspic33_device_internal_refresh_timer_inputs(Dspic33* cpu) {
    ...
    for (timer = 0u; timer < DSPIC33_TIMER_COUNT; timer++) {
        ...
        if (previous_qualified && qualified && !previous_high && high) {
            dspic33_device_internal_pulse_timer(cpu, timer, 1u);
        }
        if (previous_qualified && qualified && previous_high && !high) {
            dspic33_device_internal_set_timer_gate(cpu, timer, false);
        }
        if (qualified && (!previous_qualified || high != previous_high)) {
            if (high) {
                dspic33_device_internal_set_timer_gate(cpu, timer, true);
            }
        }
    }
}
...
void dspic33_device_internal_refresh_input_capture_pps_inputs(Dspic33* cpu) {
    for (uint8_t channel = 0u; channel < DSPIC33_INPUT_CAPTURE_COUNT; channel++) {
        ...
        uint8_t selection = dspic33_device_internal_input_capture_pps_pin(cpu, channel);
        bool previous_qualified = (cpu->io.input_capture.pps_qualified & bit) != 0u;
        bool high = false;
        bool qualified = selection != 0u &&
                         (channel != 1u || !dspic33_device_internal_can_capture_enabled(cpu)) &&
                         dspic33_device_internal_pps_physical_input_high(cpu, selection, &high);
        ...
        if (reconfigured) {
            ...
            dspic33_device_internal_input_capture_level(cpu, channel, high);
        }
    }
}
    ...
    for (uint8_t channel = 0u; channel < DSPIC33_QEI_COUNT; channel++) {
        ...
        for (uint8_t input = 0u; input < 4u; input++) {
            ...
            uint16_t mapping = dspic33_device_internal_raw_word(
                cpu, dspic33_device_qei_pps_registers[channel][input / 2u]);
        ...
        if (affected != 0u && values != cpu->qei_inputs[channel]) {
            dspic33_device_internal_qei_set_physical_inputs(cpu, channel, values);
        }
    }
}
...
void dspic33_device_internal_uart_refresh_pps_inputs(Dspic33* cpu) {
    ...
    for (channel = 0u; channel < DSPIC33_UART_COUNT; channel++) {
        uint16_t mapping =
            dspic33_device_internal_raw_word(cpu, dspic33_device_uart_pps_registers[channel]);
        uint8_t selection = (uint8_t)(mapping & 0x007fu);
        uint8_t cts_selection = (uint8_t)((mapping >> 8u) & 0x007fu);
        uint8_t mask = (uint8_t)(1u << channel);
        bool previous_high = (cpu->io.uart_rx_pin_high & mask) != 0u;
        bool previous_qualified = (cpu->io.uart_rx_qualified & mask) != 0u;
        bool high = false;
        bool qualified = dspic33_device_internal_uart_rx_logical_level(cpu, channel, &high);
        bool reconfigured =
        ...
        if (qualified) {
            ...
            cpu->io.uart_rx_qualified &= (uint8_t)~mask;
        }
        if (high) {
            cpu->io.uart_rx_pin_high |= mask;
        } else {
            cpu->io.uart_rx_pin_high &= (uint8_t)~mask;
        }
        if (!reconfigured && qualified && high != previous_high) {
            uint16_t mode =
                dspic33_device_internal_raw_word(cpu, dspic33_device_uart_bases[channel]);
            if ((mode & UART_MODE_AUTO_BAUD) != 0u) {
                dspic33_device_internal_uart_auto_baud_edge(cpu, channel, previous_high, high);
                ...
                if (previous_high && !high) {
                    ...
                    dspic33_device_internal_raw_write_word(cpu, dspic33_device_uart_bases[channel],
...
void dspic33_device_internal_refresh_physical_pin_inputs(Dspic33* cpu) {
    dspic33_device_internal_uart_refresh_pps_inputs(cpu);
    dspic33_device_internal_refresh_qei_pps_inputs(cpu);
    dspic33_device_internal_refresh_gpio_change_notification(cpu);
    dspic33_device_internal_refresh_external_interrupts(cpu);
    dspic33_device_internal_refresh_timer_inputs(cpu);
    dspic33_device_internal_refresh_input_capture_pps_inputs(cpu);
    dspic33_device_internal_output_compare_refresh_fault_pps_inputs(cpu);
    dspic33_device_internal_refresh_pwm_inputs(cpu);
    dspic33_device_internal_dci_refresh_pps_inputs(cpu);
...
uint8_t dspic33_device_read_byte(Dspic33* cpu, uint16_t address, uint8_t value) {
    ...
    for (timer = 0u; timer < 5u; timer++) {
        if (dspic33_device_internal_timer_pmd_disabled(cpu, timer) &&
            (base == dspic33_device_timer_controls[timer] ||
             ...
             base == dspic33_device_timer_periods[timer] ||
             (dspic33_device_internal_timer_is_type_b(timer) &&
              base == dspic33_device_timer_holding_registers[timer / 2u]))) {
            return 0u;
        }
    }
    ...
    if (address >= OUTPUT_COMPARE_BASE &&
        ...
        if (dspic33_device_internal_output_compare_pmd_disabled(cpu, channel)) {
            return 0u;
        }
    }
    ...
    dspic33_device_internal_pmp_read_register(cpu, address);
    if (dspic33_i2c_read_register(cpu, address, &value)) {
        return value;
    }
    if (dspic33_device_internal_qei_read_register(cpu, address, &value)) {
        return value;
    }
    if (dspic33_device_internal_dci_read_register(cpu, address, &value)) {
        return value;
    }
    if (base >= RTCC_ALARM_VALUE && base <= RTCC_CONTROL) {
        ...
        if (base == RTCC_ALARM_VALUE) {
            return dspic33_device_internal_rtcc_read_window(cpu, address, true);
        }
        if (base == RTCC_VALUE) {
            return dspic33_device_internal_rtcc_read_window(cpu, address, false);
        }
    }
    ...
    if (address >= INPUT_CAPTURE_BASE &&
        ...
        channel = (uint8_t)((address - INPUT_CAPTURE_BASE) / INPUT_CAPTURE_STRIDE);
        dspic33_device_internal_input_capture_read_complete(cpu, channel);
    }
    if (address == 0x08c3u) {
        return cpu->disicnt != 0u ? (uint8_t)(value | 0x40u) : (uint8_t)(value & ~0x40u);
    }
    for (channel = 0u; channel < DSPIC33_CAN_COUNT; channel++) {
        uint16_t channel_base = dspic33_device_can_bases[channel];
        uint16_t word_address = (uint16_t)(address & 0xfffeu);
        uint16_t offset = (uint16_t)(word_address - channel_base);
        if (offset <= 0x7eu &&
            (dspic33_device_internal_raw_word(cpu, channel_base) & CAN_WINDOW) != 0u &&
            offset >= 0x20u) {
            uint16_t word = dspic33_device_internal_can_filter_word(cpu, channel, offset);
    ...
    for (port = 0u; port < DSPIC33_GPIO_PORT_COUNT; port++) {
        if ((address & 0xfffeu) == dspic33_device_gpio_port_addresses[port]) {
            uint16_t pins = dspic33_device_internal_gpio_pin_values(cpu, port);
            acknowledge_gpio_change_notification(cpu, port, pins);
            return (uint8_t)(pins >> ((address & 1u) * 8u));
        }
    }
    for (channel = 0u; channel < DSPIC33_UART_COUNT; channel++) {
        uint16_t channel_base = dspic33_device_uart_bases[channel];
        bool high_byte = (address & 1u) != 0u;
        bool nine_bit = (dspic33_device_internal_raw_word(cpu, channel_base) &
                         UART_MODE_DATA_MASK) == UART_MODE_DATA_MASK;
        if ((address & 0xfffeu) == channel_base + 6u && high_byte == nine_bit) {
            dspic33_device_internal_uart_read_complete(cpu, channel);
        }
    }
    for (channel = 0u; channel < DSPIC33_SPI_COUNT; channel++) {
        uint16_t channel_base = dspic33_device_spi_bases[channel];
        if ((address & 0xfffeu) == channel_base + 8u &&
            dspic33_device_internal_spi_read_complete(cpu, address)) {
            uint16_t discarded;
            if (dspic33_device_internal_word_queue_pop(&cpu->io.spi_rx_fifo[channel], &discarded)) {
                if (dspic33_device_internal_word_queue_front(&cpu->io.spi_rx_fifo[channel],
                                                             &discarded)) {
                    dspic33_device_internal_raw_write_word(cpu, (uint16_t)(channel_base + 8u),
                                                           discarded);
                }
                ...
                if (dspic33_device_internal_spi_enhanced(cpu, channel) &&
                    ...
                    dspic33_device_internal_spi_raise_mode(cpu, channel, 0u);
                }
            } else {
                dspic33_device_internal_raw_write_word(
                    cpu, channel_base,
                    (uint16_t)(dspic33_device_internal_raw_word(cpu, channel_base) & ~SPI_RX_FULL));
            }
        }
    }
    return value;
