...
static void expect(WatchdogConformance* state, bool condition, const char* name) {
    state->cases++;
    if (condition) {
        state->passed++;
    } else {
        state->failed++;
        printf("[watchdog-failed] %s\n", name);
    }
}
...
static void period_cases(WatchdogConformance* state, Dspic33* cpu) {
    uint8_t prescaler;
    uint8_t postscaler;
...
static void software_enable_cases(WatchdogConformance* state, Dspic33* cpu) {
...
static void window_cases(WatchdogConformance* state, Dspic33* cpu) {
    uint8_t postscaler;
    for (postscaler = 0u; postscaler < 16u; postscaler++) {
...
static void power_cases(WatchdogConformance* state, Dspic33* cpu) {
    configure(cpu, (uint8_t)(FWDTEN | WINDIS));
    dspic33_reset(cpu, 0u);
...
static void lifecycle_cases(WatchdogConformance* state, Dspic33* source,
                            Dspic33* copy) {
...
static void nvm_cases(WatchdogConformance* state, Dspic33* cpu) {
    configure(cpu, (uint8_t)(FWDTEN | WINDIS));
    dspic33_reset(cpu, 0u);
    cpu->pc = 0x0020u;
...
int main(void) {
    Dspic33 source;
    Dspic33 copy;
    WatchdogConformance state = {0u, 0u, 0u};
