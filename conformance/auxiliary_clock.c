...
static void divisor_cases(AuxiliaryClockConformance* state, Dspic33* cpu) {
    uint32_t generation;
    dspic33_reset(cpu, 0u);
...
static void reconfiguration_cases(AuxiliaryClockConformance* state, Dspic33* cpu) {
...
static void source_admission_cases(AuxiliaryClockConformance* state, Dspic33* cpu) {
    uint8_t mode;
    dspic33_reset(cpu, 0u);
...
static void configuration_matrix_cases(AuxiliaryClockConformance* state, Dspic33* cpu) {
    uint16_t ordinal;
...
static void clock_lock_cases(AuxiliaryClockConformance* state, Dspic33* cpu) {
    uint16_t ordinal;
    dspic33_load_configuration_word(cpu, OSCILLATOR_CONFIGURATION, 0x005eu);
    for (ordinal = 0u; ordinal < 4096u; ordinal++) {
        uint16_t configuration = configuration_from_ordinal(ordinal);
