...
static void expect(ResetConformance* state, bool condition, const char* name) {
    state->cases++;
    if (condition) {
        state->passed++;
    } else {
...
static void master_clear_register_cases(ResetConformance* state, Dspic33* cpu) {
...
static void reset_flag_cases(ResetConformance* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
...
static void brown_out_clock_cases(ResetConformance* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
...
static void nvm_reset_cases(ResetConformance* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
...
static void copy_cases(ResetConformance* state, Dspic33* source, Dspic33* copy) {
...
static void reset_vector_cases(ResetConformance* state, Dspic33* cpu,
                               Dspic33* protected_cpu) {
    dspic33_reset(cpu, 0u);
    expect(state,
...
int main(void) {
    Dspic33 source;
    Dspic33 copy;
    ResetConformance state = {0u, 0u, 0u};
