...
static void expect(RtccConformance* state, bool condition, const char* name) {
    state->cases++;
    if (condition) {
        state->passed++;
    } else {
        state->failed++;
        printf("[rtcc-failed] %s\n", name);
    }
}
...
static void authorization_cases(RtccConformance* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
...
static void pointer_read_cases(RtccConformance* state, Dspic33* cpu) {
    uint8_t pointer;
...
static void pointer_write_cases(RtccConformance* state, Dspic33* cpu) {
    uint8_t pointer;
    for (pointer = 0u; pointer < 4u; pointer++) {
        uint16_t expected_pointer = pointer == 0u ? 0u : (uint16_t)(pointer - 1u);
        dspic33_reset(cpu, 0u);
...
static void alarm_cases(RtccConformance* state, Dspic33* cpu) {
    uint8_t mask;
...
static void lifecycle_cases(RtccConformance* state, Dspic33* cpu) {
    Dspic33 copy;
...
static void long_sequence_cases(RtccConformance* state, Dspic33* cpu) {
    uint8_t elapsed;
    dspic33_reset(cpu, 0u);
    enable_clock(cpu);
    set_calendar(cpu, 0x0000u, 0x0000u, 0x0101u, 0x0000u);
...
int main(void) {
    Dspic33 cpu;
    RtccConformance state = {0u, 0u, 0u};
