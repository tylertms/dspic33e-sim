...
static const uint16_t writable_pads[] = {0x0224u, 0x0234u, 0x0248u, 0x0254u, 0x0268u,
...
static void pad_membership_cases(DmaConformance* state) {
    static const uint16_t firmware_read_pads[] = {0x0236u, 0x0248u, 0x0256u, 0x0300u,
                                                  0x0608u};
    ...
    for (address = 0u; address <= UINT16_MAX; address++) {
        bool expected_read =
            pad_in_set((uint16_t)address, readable_pads,
                       sizeof(readable_pads) / sizeof(readable_pads[0]));
        bool expected_write =
            pad_in_set((uint16_t)address, writable_pads,
    ...
    exact = true;
    for (index = 0u; index < sizeof(firmware_read_pads) / sizeof(firmware_read_pads[0]);
         index++) {
        exact = exact && dspic33_device_dma_pad_valid(firmware_read_pads[index], false);
    }
    for (index = 0u;
         index < sizeof(firmware_write_pads) / sizeof(firmware_write_pads[0]);
         index++) {
...
static void direction_and_width_cases(DmaConformance* state, Dspic33* cpu) {
    ...
    expect(state, stored_word(cpu, DMA_TEST_WRITE_PAD) == 0x5aa5u,
           "word RAM to peripheral");
    ...
    expect(state, stored_word(cpu, DMA_TEST_WRITE_PAD) == 0x7e5au,
...
static void invalid_write_cases(DmaConformance* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
    dspic33_write_word(cpu, 0x2000u, 0x5aa5u);
    ...
    expect(state, dspic33_read_word(cpu, 0x0e34u) == 0u,
           "invalid low-byte PAD invokes no handler");
    ...
    expect(state,
           dspic33_device_advance(cpu, 1u) &&
               dspic33_read_word(cpu, 0x0e34u) == 0x7777u &&
...
static void operating_mode_cases(DmaConformance* state, Dspic33* cpu) {
    ...
    expect(state, stored_word(cpu, DMA_TEST_WRITE_PAD) == 0x1001u,
    ...
    configure_channel(cpu, 0u, 0x2002u, 0x61u, 0x2500u, 0x2520u, DMA_TEST_WRITE_PAD,
                      0u);
    ...
    configure_channel(cpu, 0u, 0x2003u, 0x62u, 0x2500u, 0x2520u, DMA_TEST_WRITE_PAD,
                      0u);
...
static void interrupt_and_null_cases(DmaConformance* state, Dspic33* cpu) {
    ...
    configure_channel(cpu, 0u, 0x0801u, 0x72u, 0x2700u, 0u, DMA_TEST_BIDIRECTIONAL_PAD,
                      0u);
    ...
    expect(state,
           dspic33_read_word(cpu, 0x2700u) == 0u &&
               dspic33_read_word(cpu, 0x2800u) == 0x7b7bu && interrupt_flag(cpu, 0u) &&
               (dspic33_read_word(cpu, 0x0b00u) & 0x8000u) == 0u,
...
static void force_and_collision_cases(DmaConformance* state, Dspic33* cpu) {
    ...
    for (channel = 0u; channel < DSPIC33_DMA_COUNT; channel++) {
        ...
        expect(state,
               stored_word(cpu, pad) == first && cpu->io.dma_index[channel] == 1u,
...
static void routing_and_status_cases(DmaConformance* state, Dspic33* cpu) {
    ...
    expect(state, stored_word(cpu, DMA_TEST_WRITE_PAD) == 0x55aau,
           "24-bit memory transfer");
    expect(state,
    ...
    configure_channel(cpu, 0u, 0x2001u, 0x92u, DSPIC33_DATA_SIZE, 0u,
