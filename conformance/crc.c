...
static void expect(CrcConformance* state, bool condition, const char* name) {
    state->cases++;
    if (condition) {
        state->passed++;
    } else {
...
static void reset_and_access_cases(CrcConformance* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
...
static void fifo_cases(CrcConformance* state, Dspic33* cpu) {
...
static void lane_cases(CrcConformance* state, Dspic33* cpu) {
...
static void run_vector(CrcConformance* state, Dspic33* cpu, const uint32_t* words,
...
static void known_vector_cases(CrcConformance* state, Dspic33* cpu) {
...
static void width_matrix_cases(CrcConformance* state, Dspic33* cpu) {
    uint8_t data_width;
...
static void interrupt_timing_cases(CrcConformance* state, Dspic33* cpu) {
    configure(cpu, 8u, 16u, 0x1021u, false, false);
...
static void lifecycle_cases(CrcConformance* state, Dspic33* cpu) {
    uint32_t partial;
    configure(cpu, 8u, 16u, 0x1021u, false, false);
    start(cpu);
    expect(state,
...
static void no_dma_cases(CrcConformance* state, Dspic33* cpu) {
    configure(cpu, 8u, 16u, 0x1021u, false, false);
...
static void copy_reset_failure_cases(CrcConformance* state, Dspic33* cpu) {
    Dspic33 copy;
