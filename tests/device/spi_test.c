...
static void split_buffer_cases(TestState* state, Dspic33* cpu) {
    uint8_t channel;
    for (channel = 0u; channel < DSPIC33_SPI_COUNT; channel++) {
        uint16_t base = bases[channel];
...
static void transmit_output_cases(TestState* state, Dspic33* cpu) {
    uint8_t channel;
    uint8_t value;
    for (channel = 0u; channel < DSPIC33_SPI_COUNT; channel++) {
        uint16_t base = bases[channel];
...
static void receive_only_cases(TestState* state, Dspic33* cpu) {
    uint8_t channel;
    for (channel = 0u; channel < DSPIC33_SPI_COUNT; channel++) {
        uint8_t master;
...
static void physical_slave_input_cases(TestState* state, Dspic33* cpu, Dspic33* copy) {
    uint8_t channel;
    for (channel = 0u; channel < DSPIC33_SPI_COUNT; channel++) {
        uint8_t mode16;
        for (mode16 = 0u; mode16 < 2u; mode16++) {
    ...
    for (channel = 0u; channel < DSPIC33_SPI_COUNT; channel++) {
        ...
        for (polarity = 0u; polarity < 2u; polarity++) {
            ...
            expect(state,
                   accepted && transfer_interrupt_after_cycle(cpu, irqs[channel]) &&
    ...
    expect(state,
           transfer_interrupt_after_cycle(copy, irqs[0]) &&
    ...
    dspic33_reset(cpu, 0u);
    configure_spi(cpu, 0u, 0x001bu, 0u, 0u);
...
static void pps_slave_input_cases(TestState* state, Dspic33* cpu) {
...
static void master_output_cases(TestState* state, Dspic33* cpu, Dspic33* copy) {
...
static void timing_matrix_cases(TestState* state, Dspic33* cpu) {
    uint8_t channel;
    uint8_t mode16;
    uint8_t primary;
    uint8_t secondary;
...
static void standard_buffer_cases(TestState* state, Dspic33* cpu) {
    uint8_t channel;
    for (channel = 0u; channel < DSPIC33_SPI_COUNT; channel++) {
        uint16_t base = bases[channel];
        uint16_t control = 0x043bu;
...
static void enhanced_fifo_cases(TestState* state, Dspic33* cpu) {
    uint8_t channel;
    for (channel = 0u; channel < DSPIC33_SPI_COUNT; channel++) {
        uint16_t base = bases[channel];
        uint16_t control = 0x043bu;
...
static void interrupt_mode_cases(TestState* state, Dspic33* cpu) {
...
static void selection_and_frame_cases(TestState* state, Dspic33* cpu) {
    uint8_t channel;
    for (channel = 0u; channel < DSPIC33_SPI_COUNT; channel++) {
        uint16_t base = bases[channel];
        dspic33_reset(cpu, 0u);
...
static void slave_select_retry_cases(TestState* state, Dspic33* cpu) {
...
static void b1_frame_output_cases(TestState* state, Dspic33* cpu, Dspic33* copy) {
    ...
    dspic33_reset(cpu, 0u);
    configure_spi(cpu, 0u, 0x043bu, 0xa002u, 0u);
    dspic33_device_advance(cpu, 1u);
