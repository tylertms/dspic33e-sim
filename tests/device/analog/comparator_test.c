...
static bool prepare_relation(Dspic33* cpu, uint8_t comparator, Dspic33ComparatorInput negative,
                             uint16_t positive, uint16_t negative_level) {
...
static void configure_comparator(Dspic33* cpu, uint8_t comparator, uint16_t channel, bool inverted,
    ...
    for (comparator = 0u; comparator < DSPIC33_COMPARATOR_COUNT; comparator++) {
        ...
        for (channel = 0u; channel < 3u; channel++) {
            ...
            for (inverted = 0u; inverted < 2u; inverted++) {
                dspic33_reset(cpu, 0u);
                expect(state,
                       prepare_relation(cpu, comparator, negative_inputs[channel], 100u, 150u),
    ...
    for (comparator = 0u; comparator < DSPIC33_COMPARATOR_COUNT; comparator++) {
        ...
        for (inverted = 0u; inverted < 2u; inverted++) {
            ...
            for (polarity = 0u; polarity < 4u; polarity++) {
                ...
                dspic33_reset(cpu, 0u);
                expect(state,
                       prepare_relation(cpu, comparator, DSPIC33_COMPARATOR_INPUT_NEGATIVE_2, 0u,
                                        100u),
                       "prepare comparator event baseline");
                configure_comparator(cpu, comparator, 0u, inverted != 0u, event_polarity);
...
static bool trigger_unread_rising_event(Dspic33* cpu, uint8_t comparator) {
    dspic33_reset(cpu, 0u);
    if (!prepare_relation(cpu, comparator, DSPIC33_COMPARATOR_INPUT_NEGATIVE_2, 0u, 100u)) {
        return false;
    }
    ...
    return dspic33_comparator_input(cpu, comparator, DSPIC33_COMPARATOR_INPUT_POSITIVE, 200u, 0u) &&
           dspic33_device_advance(cpu, 0u) && status_event(cpu, comparator) && interrupt_flag(cpu);
}
...
static void last_read_cout_cases(TestState* state, Dspic33* cpu) {
    ...
    for (comparator = 0u; comparator < DSPIC33_COMPARATOR_COUNT; comparator++) {
        ...
        clear_interrupt(cpu);
        expect(state,
               prepared && !status_event(cpu, comparator) && dspic33_device_advance(cpu, 1u) &&
                   status_event(cpu, comparator) && interrupt_flag(cpu),
        ...
        clear_interrupt(cpu);
        expect(state,
               prepared && dspic33_device_advance(cpu, 1u) && status_event(cpu, comparator) &&
                   interrupt_flag(cpu),
...
static void pps_case(TestState* state, Dspic33* cpu, uint8_t comparator, uint8_t pin,
    ...
    dspic33_reset(cpu, 0u);
    expect(state,
           prepare_relation(cpu, comparator, DSPIC33_COMPARATOR_INPUT_NEGATIVE_2, 200u, 100u),
...
static void pps_cases(TestState* state, Dspic33* cpu) {
    ...
    dspic33_write_word(cpu, comparator_base(0u), COMPARATOR_ENABLE | COMPARATOR_OUTPUT_ENABLE);
...
static void sleep_case(TestState* state, Dspic33* cpu, uint8_t comparator) {
    ...
    expect(state,
           output_is(cpu, comparator, true) && status_event(cpu, comparator) && interrupt_flag(cpu),
...
static void lifecycle_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state,
           output_is(cpu, 0u, true) && output_is(&copy, 0u, true) && status_event(cpu, 0u) &&
               status_event(&copy, 0u),
...
static void reference_selection_cases(TestState* state, Dspic33* cpu) {
    ...
    for (comparator = 0u; comparator < DSPIC33_COMPARATOR_COUNT; comparator++) {
        ...
        for (selection = 0u; selection < 4u; selection++) {
            ...
            dspic33_write_word(cpu, comparator_base(comparator), COMPARATOR_ENABLE | 0x0003u);
            expect(state, output_is(cpu, comparator, false), "IVREF equality stays low");
            expect(state,
                   dspic33_comparator_input(cpu, comparator, DSPIC33_COMPARATOR_INPUT_POSITIVE,
                                            (uint16_t)(level + 1u), 0u) &&
                       dspic33_device_advance(cpu, 0u) && output_is(cpu, comparator, true),
    ...
    dspic33_write_word(cpu, COMPARATOR_REFERENCE, COMPARATOR_REFERENCE_EXTERNAL);
    dspic33_write_word(cpu, comparator_base(0u), COMPARATOR_ENABLE | 0x0010u);
    expect(state, output_is(cpu, 0u, true), "default VREF positive level drives comparator");
    expect(state,
           dspic33_comparator_reference(cpu, DSPIC33_COMPARATOR_REFERENCE_VREF_POSITIVE, 500u, 2u),
           "schedule delayed comparator reference");
    dspic33_device_advance(cpu, 1u);
    expect(state, output_is(cpu, 0u, true), "delayed comparator reference waits for deadline");
    dspic33_device_advance(cpu, 1u);
    expect(state, output_is(cpu, 0u, false), "delayed comparator reference applies at deadline");
        ...
        for (inverted = 0u; inverted < 2u; inverted++) {
            ...
            for (selection = 0u; selection < 16u; selection++) {
                ...
                for (high = 0u; high < 2u; high++) {
                    ...
                    dspic33_write_word(cpu, (uint16_t)(comparator_base(0u) + 2u), source);
    ...
    for (comparator = 0u; comparator < DSPIC33_COMPARATOR_COUNT; comparator++) {
        ...
        for (source = 0u; source < 8u; source++) {
            ...
            for (divider_selection = 0u; divider_selection < 8u; divider_selection++) {
                ...
                dspic33_write_word(
                    cpu, (uint16_t)(comparator_base(comparator) + 6u),
...
static void filter_sequence_cases(TestState* state, Dspic33* cpu) {
    ...
    advance_filter_clock(cpu, 4u, 1u);
    expect(state, output_is(cpu, 0u, true) && interrupt_flag(cpu) && status_event(cpu, 0u),
           "qualified filter output raises comparator event");
    dspic33_write_word(cpu, comparator_base(0u), COMPARATOR_ENABLE | 0x00c0u);
    clear_interrupt(cpu);
    dspic33_device_advance(cpu, 1u);
    advance_filter_clock(cpu, 4u, 1u);
    expect(state, interrupt_flag(cpu), "filtered stable output rearms against last read value");
...
static void blanking_transition_cases(TestState* state, Dspic33* cpu) {
    ...
    dspic33_write_word(cpu, comparator_base(0u), COMPARATOR_ENABLE | 0x00c0u);
    expect(state, output_is(cpu, 0u, true), "inactive PWM mask passes comparator output");
    ...
    expect(state, output_is(cpu, 0u, true), "PWM output release restores comparator output");
    ...
    dspic33_write_word(cpu, (uint16_t)(comparator_base(0u) + 4u), 0x0200u);
    dspic33_write_word(cpu, comparator_base(0u), COMPARATOR_ENABLE);
    expect(state, output_is(cpu, 0u, true), "inactive FLT2 mask passes comparator output");
    ...
    expect(state, output_is(cpu, 0u, true), "filter qualifies unblanked comparator output");
    dspic33_pwm_fault(cpu, 1u, true, 0u);
    dspic33_device_advance(cpu, 0u);
    advance_filter_clock(cpu, 4u, 2u);
    expect(state, output_is(cpu, 0u, true),
    ...
    advance_filter_clock(cpu, 4u, 1u);
    expect(state, output_is(cpu, 0u, false), "filtered blanking applies after three samples");
}
...
static void filter_lifecycle_cases(TestState* state, Dspic33* cpu) {
    ...
    dspic33_comparator_input(cpu, 0u, DSPIC33_COMPARATOR_INPUT_POSITIVE, 0u, 0u);
    dspic33_device_advance(cpu, 0u);
    expect(state, output_is(&copy, 0u, true), "copied comparator filter input remains independent");
    ...
    expect(state, output_is(cpu, 0u, false), "PMD pause retains remaining filter phase");
    dspic33_device_advance(cpu, 1u);
    expect(state, output_is(cpu, 0u, true), "PMD resume completes retained filter samples");
    ...
    expect(state, output_is(cpu, 0u, true), "wake resumes retained FP comparator filter phase");
    ...
    expect(state,
           cpu->stop_reason == DSPIC33_EVENT_QUEUE_ERROR &&
               dspic33_read_word(cpu, comparator_base(0u)) == 0u && cpu->events.count == 0u,
...
static void filter_reconfiguration_cases(TestState* state, Dspic33* cpu) {
    ...
    dspic33_write_word(cpu, (uint16_t)(comparator_base(0u) + 6u), 0x000bu);
    dspic33_device_advance(cpu, 20u);
    expect(state, output_is(cpu, 0u, true), "same filter write preserves sampling phase");
    ...
    dspic33_write_word(cpu, (uint16_t)(comparator_base(0u) + 6u), 0x0008u);
    dspic33_device_advance(cpu, 2u);
    expect(state, output_is(cpu, 0u, false), "changed filter configuration invalidates old phase");
    dspic33_device_advance(cpu, 1u);
    expect(state, output_is(cpu, 0u, true), "changed filter configuration uses new deadline");
    dspic33_device_advance(cpu, 1u);
    ...
    dspic33_device_power_state_changed(cpu);
    dspic33_device_advance(cpu, 20u);
    expect(state, output_is(cpu, 0u, true), "filter FP clock continues in unrestricted Idle");
    ...
    expect(
        state,
        dspic33_step(cpu) == DSPIC33_RUNNING && dspic33_read_word(cpu, comparator_base(0u)) == 0u &&
...
static void byte_access_behavior_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, output_is(cpu, 0u, true), "byte writes configure external comparator reference");
    ...
    dspic33_write_byte(cpu, (uint16_t)(comparator_base(0u) + 1u), 0x80u);
    expect(state, output_is(cpu, 0u, false), "byte writes configure comparator blanking gate");
...
static void dma_and_completed_feature_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, dspic33_read_word(cpu, (uint16_t)(comparator_base(0u) + 2u)) == 0x0abcu,
           "CMMSKSRC retains completed source routing");
    dspic33_write_word(cpu, (uint16_t)(comparator_base(0u) + 4u), 0x8001u);
    expect(state, dspic33_read_word(cpu, (uint16_t)(comparator_base(0u) + 4u)) == 0x8001u,
           "CMMSKCON retains completed blanking configuration");
    dspic33_write_word(cpu, (uint16_t)(comparator_base(0u) + 6u), 0x007fu);
    expect(state, dspic33_read_word(cpu, (uint16_t)(comparator_base(0u) + 6u)) == 0x007fu,
