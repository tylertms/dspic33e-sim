...
static bool drive_serial_edge(Dspic33* cpu, bool high, bool rising, uint16_t clock_mask) {
...
static bool activate_serial_clock(Dspic33* cpu, bool rising, uint16_t clock_mask) {
    uint8_t edges = cpu->io.dci.serial_startup_bits;
    while (edges-- != 0u) {
        if (!drive_serial_edge(cpu, false, rising, clock_mask)) {
            return false;
        }
    }
    return true;
...
static bool prepare_serial_data(Dspic33* cpu, bool rising, uint16_t clock_mask) {
    uint16_t control;
    if (!activate_serial_clock(cpu, rising, clock_mask)) {
        return false;
    }
    ...
    if (cpu->io.dci.serial_delay ||
        ...
        return drive_serial_edge(cpu, false, rising, clock_mask);
    }
    return true;
}
...
static bool drive_serial_pin_bit(Dspic33* cpu, bool high, bool rising, uint16_t clock_mask) {
    return prepare_serial_data(cpu, rising, clock_mask) &&
           drive_serial_edge(cpu, high, rising, clock_mask);
}

static bool drive_mapped_serial_word(Dspic33* cpu, uint16_t value, uint8_t width, bool rising,
...
static bool drive_serial_word(Dspic33* cpu, uint16_t value, uint8_t width, bool rising) {
...
static void configure_external(Dspic33* cpu, uint16_t control, uint8_t width, uint8_t slots,
                               uint8_t buffers, uint16_t transmit, uint16_t receive) {
...
static void configure_internal(Dspic33* cpu, uint16_t control, uint8_t width, uint8_t slots,
                               uint8_t buffers, uint16_t transmit, uint16_t receive) {
    dspic33_write_word(cpu, DCI_CONTROL1, 0u);
    dspic33_write_word(cpu, DCI_CONTROL2, configuration(width, slots, buffers));
...
static bool drive_internal_pin_slot(Dspic33* cpu, uint16_t value, uint8_t width,
...
static void configure_dma(Dspic33* cpu, uint8_t channel, uint16_t control, uint32_t memory,
                          uint16_t pad, uint16_t count, uint8_t request) {
...
static void width_and_lane_cases(TestState* state, Dspic33* cpu) {
    uint8_t width;
...
static void slot_buffer_status_cases(TestState* state, Dspic33* cpu) {
    uint8_t index;
    dspic33_reset(cpu, 0u);
    ...
    expect(state, clock_word(cpu, 0x3000u, false), "mixed-slot startup loads active TXBUF0");
...
static void admission_and_clock_cases(TestState* state, Dspic33* cpu) {
    uint8_t width;
    ...
    expect(state, clock_word(cpu, 0x3333u, false), "clock external DCI while PMD is effective");
    ...
    expect(state, clock_word(cpu, 0x4444u, false) && dspic33_read_word(cpu, DCI_RECEIVE_BASE) == 0u,
           "DCISIDL stops external DCI in Idle");
    dspic33_write_word(cpu, DCI_CONTROL1, DCI_ENABLE | DCI_EXTERNAL_CLOCK);
    expect(state,
           clock_word(cpu, 0x5555u, false) && dspic33_read_word(cpu, DCI_RECEIVE_BASE) == 0x5555u,
           "external DCI continues in Idle when DCISIDL is clear");
    cpu->power_state = DSPIC33_POWER_SLEEP;
    expect(state,
           clock_word(cpu, 0x6666u, false) && dspic33_read_word(cpu, DCI_RECEIVE_BASE) == 0x6666u,
    ...
    expect(state, clock_word(cpu, 0xb000u, true) && dspic33_read_word(cpu, DCI_RECEIVE_BASE) == 0u,
    ...
    dspic33_write_word(cpu, DCI_RECEIVE_SLOTS, 1u);
    dspic33_write_word(cpu, DCI_CONTROL1, DCI_ENABLE | DCI_EXTERNAL_CLOCK);
    expect(state, clock_word(cpu, 0x9090u, false) && dspic33_read_word(cpu, DCI_RECEIVE_BASE) == 0u,
...
static void protocol_geometry_cases(TestState* state, Dspic33* cpu) {
    uint8_t mode;
        ...
        for (width = 4u; width <= 16u; width++) {
            ...
            for (frames = 1u; frames <= 16u; frames++) {
                ...
                for (options = 0u; options < 4u; options++) {
                    ...
                    expect(state, clock_word(cpu, receive, false), "clock protocol geometry word");
                    expect(state,
...
static void protocol_frame_cases(TestState* state, Dspic33* cpu) {
    ...
    for (justification = 0u; justification < 2u; justification++) {
        ...
        expect(state, clock_word(cpu, 0x1100u, false), "I2S slave waits for frame edge");
        expect(state, cpu->io.dci.receive_buffered == 0u && !cpu->io.dci.started,
               "I2S absent frame edge leaves transfer idle");
        expect(state, clock_word(cpu, 0x2200u, true), "I2S frame edge starts half-frame");
...
static void ac_link_cases(TestState* state, Dspic33* cpu) {
    Dspic33DciTransfer transfer;
    uint8_t slot;
    dspic33_reset(cpu, 0u);
...
static void protocol_integration_cases(TestState* state, Dspic33* cpu) {
    uint8_t mode;
    for (mode = 0u; mode < 4u; mode++) {
        uint16_t expected = mode >= DCI_MODE_AC_LINK_16 ? 0x5aa5u : 0x5a00u;
        dspic33_reset(cpu, 0u);
        configure_external(cpu, (uint16_t)(mode | DCI_EXTERNAL_FRAME), 8u, 2u, 1u, 0u, 1u);
        ...
        expect(state, clock_word(cpu, 0x2200u, true), "protocol slave accepts frame indication");
        expect(state, dspic33_read_word(cpu, DCI_RECEIVE_BASE) == 0x2200u,
               "protocol slave captures synchronized word");

        dspic33_reset(cpu, 0u);
        configure_dma(cpu, 0u, 0x0001u, (uint32_t)(0x4400u + mode * 2u), DCI_RECEIVE_BASE, 0u,
                      DCI_DMA_REQUEST);
        configure_external(cpu, mode, 8u, 1u, 1u, 0u, 1u);
        expect(state, clock_word(cpu, 0x5aa5u, false), "protocol raises receive DMA request");
...
static void pps_serial_input_cases(TestState* state, Dspic33* cpu) {
    Dspic33DciTransfer transfer;
    dspic33_reset(cpu, 0u);
    dspic33_write_word(cpu, GPIO_ANALOG_D,
    ...
    expect(state, drive_serial_word(cpu, 0xa500u, 8u, true), "clock default DCI VSS selection");
    expect(state,
    ...
    expect(state, drive_serial_word(cpu, 0xa500u, 8u, true), "clock PPS DCI on rising edges");
    expect(state,
    ...
    expect(state, drive_serial_word(cpu, 0x6b4bu, 16u, true), "clock PPS DCI receive DMA block");
    ...
    dspic33_write_word(cpu, DCI_TRANSMIT_BASE, 0x5a00u);
    expect(state, drive_serial_word(cpu, 0xa500u, 8u, true), "clock PPS DCI loopback word");
    expect(state,
    ...
    expect(state, drive_serial_bit(cpu, false, true) && interrupt_set(cpu, DCI_ERROR_IRQ),
           "PPS DCI transmit underflow raises error interrupt");

    dspic33_reset(cpu, 0u);
    configure_serial_pins(cpu);
    configure_external(cpu, 0u, 8u, 1u, 1u, 0u, 1u);
    expect(state, drive_serial_word(cpu, 0x3c00u, 8u, false), "clock PPS DCI on falling edges");
    ...
    dspic33_reset(cpu, 0u);
    configure_serial_pins(cpu);
    configure_external(cpu, DCI_MODE_AC_LINK_16 | DCI_SAMPLE_RISING, 4u, 1u, 1u, 0u, 1u);
    expect(state, drive_serial_word(cpu, 0xa55au, 20u, true), "clock PPS 16-bit AC-Link tag slot");
    ...
    dspic33_reset(cpu, 0u);
    configure_serial_pins(cpu);
    configure_external(cpu, DCI_MODE_AC_LINK_20 | DCI_SAMPLE_RISING, 4u, 1u, 1u, 0u, 1u);
...
static void pps_qualification_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
    configure_serial_pins(cpu);
    dspic33_write_word(cpu, DCI_PPS_INPUTS,
    ...
    expect(
        state,
        drive_mapped_serial_word(cpu, 0xf000u, 4u, true, GPIO_ANALOG_CLOCK_MASK, GPIO_CLOCK_MASK) &&
            dspic33_read_word(cpu, DCI_RECEIVE_BASE) == 0u,
    ...
    dspic33_write_word(cpu, GPIO_ANALOG_D,
                       (uint16_t)(dspic33_read_word(cpu, GPIO_ANALOG_D) | GPIO_ANALOG_CLOCK_MASK));
    configure_external(cpu, DCI_SAMPLE_RISING | DCI_EXTERNAL_FRAME | DCI_DATA_JUSTIFY, 4u, 1u, 1u,
                       0u, 1u);
    activate_serial_clock(cpu, true, GPIO_CLOCK_MASK);
...
static void pps_frame_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
    configure_serial_pins(cpu);
    configure_external(cpu, DCI_SAMPLE_RISING | DCI_EXTERNAL_FRAME, 4u, 1u, 1u, 0u, 1u);
    activate_serial_clock(cpu, true, GPIO_CLOCK_MASK);
    dspic33_gpio_drive(cpu, GPIO_PORT_D, GPIO_FRAME_MASK, GPIO_FRAME_MASK);
    expect(state, drive_serial_edge(cpu, true, true, GPIO_CLOCK_MASK),
    ...
    expect(state, drive_serial_bit(cpu, true, true), "sample same-cycle-justified DCI frame pulse");
    dspic33_gpio_drive(cpu, GPIO_PORT_D, 0u, GPIO_FRAME_MASK);
    expect(state,
    ...
    configure_external(cpu,
                       DCI_MODE_I2S | DCI_SAMPLE_RISING | DCI_EXTERNAL_FRAME | DCI_DATA_JUSTIFY, 4u,
                       1u, 1u, 0u, 1u);
    activate_serial_clock(cpu, true, GPIO_CLOCK_MASK);
    dspic33_gpio_drive(cpu, GPIO_PORT_D, GPIO_FRAME_MASK | GPIO_DATA_MASK,
                       GPIO_FRAME_MASK | GPIO_DATA_MASK);
...
static void pps_serial_matrix_cases(TestState* state, Dspic33* cpu) {
    uint8_t mode;
        ...
        for (width = 4u; width <= 16u; width++) {
            ...
            for (rising = 0u; rising < 2u; rising++) {
                ...
                for (immediate = 0u; immediate < 2u; immediate++) {
                    ...
                    activate_serial_clock(cpu, rising != 0u, GPIO_CLOCK_MASK);
                    dspic33_gpio_drive(cpu, GPIO_PORT_D, GPIO_FRAME_MASK, GPIO_FRAME_MASK);
                    if (immediate == 0u) {
                        expect(state, drive_serial_edge(cpu, false, rising != 0u, GPIO_CLOCK_MASK),
...
static void pps_startup_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
    configure_serial_pins(cpu);
    dspic33_write_word(cpu, DCI_CONTROL2, configuration(4u, 1u, 1u));
    dspic33_write_word(cpu, DCI_CONTROL3, 0u);
    ...
    expect(state,
           drive_serial_edge(cpu, false, true, GPIO_CLOCK_MASK) &&
    ...
    expect(state,
           drive_serial_edge(cpu, false, true, GPIO_CLOCK_MASK) &&
               ...
               cpu->io.dci.started && interrupt_set(cpu, DCI_TRANSFER_IRQ) &&
               (dspic33_read_word(cpu, DCI_STATUS) & DCI_TRANSMIT_EMPTY) != 0u,
           "third startup clock transfers buffers and raises DCI interrupt");
}

static void pps_internal_input_cases(TestState* state, Dspic33* cpu) {
    uint8_t mode;
    dspic33_reset(cpu, 0u);
    dspic33_write_word(cpu, DCI_PPS_INPUTS, 0u);
    dspic33_dci_input(cpu, UINT16_MAX);
    configure_internal(cpu, DCI_SAMPLE_RISING, 4u, 1u, 1u, 0u, 1u);
    expect(state,
           drive_internal_pin_slot(cpu, UINT16_MAX, 4u, 12u) &&
               dspic33_read_word(cpu, DCI_RECEIVE_BASE) == 0u,
    ...
    for (mode = DCI_MODE_I2S - 1u; mode <= DCI_MODE_I2S; mode++) {
        ...
        for (width = 4u; width <= 16u; width++) {
            ...
            for (rising = 0u; rising < 2u; rising++) {
                ...
                dspic33_reset(cpu, 0u);
                configure_serial_pins(cpu);
                configure_internal(cpu, control, width, 1u, 1u, 0u, 1u);
                expect(state,
                       drive_internal_pin_slot(cpu, value, width, 12u) &&
                           dspic33_read_word(cpu, DCI_RECEIVE_BASE) == value,
    ...
    for (mode = DCI_MODE_AC_LINK_16; mode <= DCI_MODE_AC_LINK_20; mode++) {
        ...
        for (rising = 0u; rising < 2u; rising++) {
            ...
            dspic33_reset(cpu, 0u);
            configure_serial_pins(cpu);
            configure_internal(cpu, control, 4u, 1u, 1u, 0u, 1u);
            expect(state,
                   drive_internal_pin_slot(cpu, 0x5aa5u, 16u, 12u) &&
...
static void pps_internal_frame_cases(TestState* state, Dspic33* cpu) {
    bool high;
    uint8_t mode;
    for (mode = 0u; mode < 4u; mode++) {
        uint8_t rising;
        for (rising = 0u; rising < 2u; rising++) {
            ...
            for (immediate = 0u; immediate < 2u; immediate++) {
                ...
                dspic33_gpio_drive(cpu, GPIO_PORT_D,
                                   ...
                                   GPIO_FRAME_MASK | GPIO_DATA_MASK);
                expect(state,
                       drive_internal_pin_slot(cpu, value, width, delay) &&
                           dspic33_read_word(cpu, DCI_RECEIVE_BASE) == value,
    ...
    configure_internal(cpu, DCI_SAMPLE_RISING | DCI_EXTERNAL_FRAME, 4u, 1u, 1u, 0u, 1u);
    expect(state,
...
static void pps_external_frame_output_cases(TestState* state, Dspic33* cpu) {
    bool high;
    uint8_t immediate;
    for (immediate = 0u; immediate < 2u; immediate++) {
        ...
        configure_external(cpu, (uint16_t)(DCI_SAMPLE_RISING | justify), 4u, 1u, 1u, 0u, 0u);
        activate_serial_clock(cpu, true, GPIO_CLOCK_MASK);
        ...
        configure_serial_pins(cpu);
        dspic33_write_word(cpu, DCI_PPS_CLOCK_FRAME_OUTPUT, 0x0d00u);
        configure_external(cpu, (uint16_t)(DCI_MODE_I2S | DCI_SAMPLE_RISING | justify), 4u, 1u, 1u,
                           0u, 0u);
        activate_serial_clock(cpu, true, GPIO_CLOCK_MASK);
        expect(state, dspic33_dci_pin(cpu, PPS_FRAME_OUTPUT_PIN, &high) && high,
        ...
        expect(state,
               drive_serial_word(cpu, 0u, 4u, true) &&
                   dspic33_dci_pin(cpu, PPS_FRAME_OUTPUT_PIN, &high) && !high,
...

    {
        ...
        for (mode = DCI_MODE_AC_LINK_16; mode <= DCI_MODE_AC_LINK_20; mode++) {
            for (immediate = 0u; immediate < 2u; immediate++) {
                ...
                configure_serial_pins(cpu);
                dspic33_write_word(cpu, DCI_PPS_CLOCK_FRAME_OUTPUT, 0x0d00u);
                configure_external(cpu, control, 4u, 1u, 1u, 0u, 0u);
                activate_serial_clock(cpu, true, GPIO_CLOCK_MASK);
                expect(state, dspic33_dci_pin(cpu, PPS_FRAME_OUTPUT_PIN, &high) && high,
                       "external CSCK AC-Link master asserts tag COFS");
                expect(state,
                       drive_serial_word(cpu, 0u, 16u, true) &&
                           dspic33_dci_pin(cpu, PPS_FRAME_OUTPUT_PIN, &high) && !high,
                       "AC-Link master negates COFS after sixteen clocks");
                expect(state,
                       drive_serial_word(cpu, 0u, 240u, true) &&
                           dspic33_dci_pin(cpu, PPS_FRAME_OUTPUT_PIN, &high) && high,
...
static void pps_internal_frame_output_cases(TestState* state, Dspic33* cpu) {
    bool high;
    dspic33_reset(cpu, 0u);
    dspic33_write_word(cpu, DCI_PPS_CLOCK_FRAME_OUTPUT, 0x0d00u);
    configure_internal(cpu, DCI_MODE_I2S, 4u, 1u, 1u, 0u, 0u);
    expect(state,
    ...
    dspic33_reset(cpu, 0u);
    dspic33_write_word(cpu, DCI_PPS_CLOCK_FRAME_OUTPUT, 0x0d00u);
    configure_internal(cpu, DCI_MODE_AC_LINK_16, 4u, 1u, 1u, 0u, 0u);
    expect(state,
...
static void pps_bcg_cases(TestState* state, Dspic33* cpu) {
    Dspic33 copy;
    bool high;
    bool copy_high;
    dspic33_reset(cpu, 0u);
...
static void pps_internal_sample_lifecycle_cases(TestState* state, Dspic33* cpu) {
    Dspic33 copy;
    uint8_t bit;
    dspic33_reset(cpu, 0u);
    configure_serial_pins(cpu);
...
static void pps_selection_cases(TestState* state, Dspic33* cpu) {
    ...
    for (selection = 0u; selection < 16u; selection++) {
        ...
        configure_external(cpu, DCI_SAMPLE_RISING | DCI_EXTERNAL_FRAME | DCI_DATA_JUSTIFY, 4u, 1u,
                           1u, 0u, 1u);
        activate_serial_clock(cpu, true, GPIO_CLOCK_MASK);
        dspic33_gpio_drive(cpu, GPIO_PORT_D, GPIO_FRAME_MASK, GPIO_FRAME_MASK);
        expect(state,
...
static void pps_output_cases(TestState* state, Dspic33* cpu) {
    bool high;
    expect(state,
    ...
    activate_serial_clock(cpu, true, GPIO_CLOCK_MASK);
    dspic33_gpio_drive(cpu, GPIO_PORT_D, GPIO_FRAME_MASK, GPIO_FRAME_MASK);
    expect(state, drive_serial_edge(cpu, false, true, GPIO_CLOCK_MASK),
...
static void pps_disable_timing_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
    configure_serial_pins(cpu);
...
static void pps_lifecycle_cases(TestState* state, Dspic33* cpu) {
    Dspic33 copy;
    bool high;
    dspic33_reset(cpu, 0u);
    configure_serial_pins(cpu);
    ...
    expect(state, drive_serial_word(cpu, 0xf000u, 4u, true) && cpu->io.dci.serial_bits == 2u,
    ...
    expect(state, drive_serial_bit(cpu, true, true), "advance PPS DCI before Idle stop");
    cpu->power_state = DSPIC33_POWER_IDLE;
    expect(state, drive_serial_word(cpu, 0xf000u, 4u, true) && cpu->io.dci.serial_bits == 1u,
    ...
    dspic33_write_word(cpu, DCI_PMD, 0u);
    expect(state,
           dspic33_device_advance(cpu, 1u) && drive_serial_word(cpu, 0xf000u, 4u, true) &&
    ...
    configure_serial_pins(cpu);
    configure_external(cpu,
                       DCI_SAMPLE_RISING | DCI_EXTERNAL_FRAME | DCI_DATA_JUSTIFY | DCI_STOP_IDLE,
                       4u, 1u, 1u, 0u, 1u);
    cpu->power_state = DSPIC33_POWER_IDLE;
    expect(state,
           dspic33_gpio_drive(cpu, GPIO_PORT_D, GPIO_FRAME_MASK, GPIO_FRAME_MASK) &&
...
static void mode_and_status_cases(TestState* state, Dspic33* cpu) {
    Dspic33DciTransfer transfer;
    uint8_t mode;
    for (mode = 0u; mode < 2u; mode++) {
        dspic33_reset(cpu, 0u);
        configure_external(cpu, mode != 0u ? DCI_UNDERFLOW_LAST : 0u, 16u, 1u, 1u, 1u, 0u);
...
static void interrupt_dma_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
    ...
    expect(state, interrupt_set(cpu, DCI_TRANSFER_IRQ), "sleeping external DCI sets IRQ60");
...
static void generation_and_frame_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
    dspic33_write_word(cpu, DCI_CONTROL2, configuration(4u, 1u, 1u));
    dspic33_write_word(cpu, DCI_CONTROL3, 1u);
    dspic33_write_word(cpu, DCI_RECEIVE_SLOTS, 1u);
...
static void disable_timing_cases(TestState* state, Dspic33* cpu) {
    Dspic33DciTransfer transfer;
    dspic33_reset(cpu, 0u);
    dspic33_write_word(cpu, DCI_CONTROL2, configuration(16u, 1u, 1u));
    dspic33_write_word(cpu, DCI_CONTROL3, 1u);
...
static void internal_clock_lifecycle_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
    dspic33_write_word(cpu, DCI_CONTROL2, configuration(4u, 1u, 1u));
    dspic33_write_word(cpu, DCI_CONTROL3, 1u);
    dspic33_write_word(cpu, DCI_RECEIVE_SLOTS, 1u);
...
int main(void) {
    ...
    if (initialized) {
        ...
        pps_startup_cases(&state, &cpu);
        pps_internal_input_cases(&state, &cpu);
        pps_internal_frame_cases(&state, &cpu);
        pps_external_frame_output_cases(&state, &cpu);
        pps_internal_frame_output_cases(&state, &cpu);
        pps_bcg_cases(&state, &cpu);
        pps_internal_sample_lifecycle_cases(&state, &cpu);
