...
static uint16_t capture_pmd_mask(uint8_t channel) { return (uint16_t)(1u << (8u + channel % 8u)); }
...
static void configure_capture(Dspic33* cpu, uint8_t channel, uint8_t interval, bool paired) {
    uint16_t base = dspic33_input_capture_test_capture_base(channel);
...
void dspic33_input_capture_test_configure_comparator_source(Dspic33* cpu, uint8_t comparator) {
    ...
    dspic33_comparator_input(cpu, comparator, DSPIC33_COMPARATOR_INPUT_NEGATIVE_2, 100u, 0u);
...
bool dspic33_input_capture_test_rising_edge(Dspic33* cpu, uint8_t channel) {
    return dspic33_input_capture_input(cpu, channel, false, 0u) &&
           dspic33_device_advance(cpu, 0u) && dspic33_input_capture_input(cpu, channel, true, 0u) &&
           dspic33_device_advance(cpu, 1u);
}

static bool set_level(Dspic33* cpu, uint8_t channel, bool high) {
    return dspic33_input_capture_input(cpu, channel, high, 0u) && dspic33_device_advance(cpu, 1u);
}

static void configure_dma(Dspic33* cpu, uint8_t request, uint16_t pad, uint16_t destination) {
...
void dspic33_input_capture_test_access_cases(TestState* state, Dspic33* cpu) {
    ...
    for (channel = 0u; channel < DSPIC33_INPUT_CAPTURE_COUNT; channel++) {
        ...
        expect(state, dspic33_read_word(cpu, (uint16_t)(base + 2u)) == 0x000du, "ICCON2 reset");
        ...
        expect(state, dspic33_read_word(cpu, (uint16_t)(base + 2u)) == 0x01dfu,
               "ICCON2 access mask");
        expect(state, dspic33_read_word(cpu, (uint16_t)(base + 4u)) == 0u, "ICBUF read only");
        expect(state, dspic33_read_word(cpu, (uint16_t)(base + 6u)) == 0u, "ICTMR read only");
    }
}
...
void dspic33_input_capture_test_fifo_cases(TestState* state, Dspic33* cpu) {
    ...
    for (channel = 0u; channel < DSPIC33_INPUT_CAPTURE_COUNT; channel++) {
        ...
        for (sample = 0u; sample < DSPIC33_INPUT_CAPTURE_FIFO_SIZE; sample++) {
            expect(state, dspic33_input_capture_test_rising_edge(cpu, channel),
                   "capture FIFO edge");
        }
        ...
        expect(
            state,
            cpu->io.input_capture.fifo[channel].words[cpu->io.input_capture.fifo[channel].head] ==
                1u,
            "capture FIFO exposes oldest word");
        expect(state, dspic33_input_capture_test_rising_edge(cpu, channel),
               "capture overflow edge");
        expect(state,
        ...
        expect(state, !interrupt_flag(cpu, channel), "fourth capture interrupt remains delayed");
        expect(state, dspic33_device_advance(cpu, 1u), "advance capture interrupt delay");
        expect(state, interrupt_flag(cpu, channel), "fourth capture raises channel interrupt");
        for (sample = 0u; sample < DSPIC33_INPUT_CAPTURE_FIFO_SIZE; sample++) {
        ...
        expect(state,
               cpu->io.input_capture.fifo[channel].count == 0u &&
                   (dspic33_read_word(cpu, base) & (CAPTURE_NOT_EMPTY | CAPTURE_OVERFLOW)) == 0u,
    ...
    for (interval = 0u; interval < 4u; interval++) {
        ...
        for (sample = 0u; sample <= interval; sample++) {
            expect(state, dspic33_input_capture_test_rising_edge(cpu, 0u),
                   "capture interrupt-rate edge");
            expect(state, !interrupt_flag(cpu, 0u), "capture interrupt not raised on FIFO write");
        }
        expect(state, dspic33_device_advance(cpu, 1u), "advance first interrupt-delay cycle");
        expect(state, !interrupt_flag(cpu, 0u), "capture interrupt waits two cycles");
        expect(state, dspic33_device_advance(cpu, 1u), "advance second interrupt-delay cycle");
        expect(state, interrupt_flag(cpu, 0u), "capture interrupt follows selected interval");
    }
}
...
void dspic33_input_capture_test_zero_interval_overflow_cases(TestState* state, Dspic33* cpu) {
    ...
    for (sample = 0u; sample < DSPIC33_INPUT_CAPTURE_FIFO_SIZE; sample++) {
        expect(state, dspic33_input_capture_test_rising_edge(cpu, 0u), "ICI zero FIFO edge");
    }
    ...
    expect(state, dspic33_input_capture_test_rising_edge(cpu, 0u), "ICI zero overflow edge");
    expect(state,
           cpu->io.input_capture.fifo[0].count == 3u &&
               (dspic33_read_word(cpu, 0x0140u) & CAPTURE_OVERFLOW) == 0u &&
               cpu->io.input_capture.fifo[0].words[cpu->io.input_capture.fifo[0].head] == 2u,
           "ICI zero fifth capture discards sample without overflow");
    expect(state, dspic33_read_word(cpu, CAPTURE_DMA_DESTINATION) == 1u && !interrupt_flag(cpu, 0u),
    ...
    expect(state, !interrupt_flag(cpu, 0u), "ICI zero overflow interrupt waits two cycles");
    expect(state, dspic33_device_advance(cpu, 1u), "complete ICI zero overflow interrupt delay");
    expect(state, interrupt_flag(cpu, 0u), "ICI zero overflow still raises capture interrupt");
}

void dspic33_input_capture_test_mode_cases(TestState* state, Dspic33* cpu) {
    uint16_t base = dspic33_input_capture_test_capture_base(0u);
    uint8_t edge;
    dspic33_reset(cpu, 0u);
    dspic33_write_word(cpu, (uint16_t)(base + 2u), CAPTURE_TRIGGER);
    dspic33_write_word(cpu, base, 0x1803u);
    expect(state, dspic33_input_capture_test_rising_edge(cpu, 0u), "non-FP edge advances");
    expect(state, cpu->io.input_capture.fifo[0].count == 0u, "non-FP timer source is excluded");
    dspic33_write_word(cpu, base, 0u);
    dspic33_write_word(cpu, (uint16_t)(base + 2u), 0x00c1u);
    dspic33_write_word(cpu, base, CAPTURE_FP_RISING);
    expect(state, dspic33_input_capture_test_rising_edge(cpu, 0u),
           "nonsoftware sync edge advances");
    ...
    dspic33_write_word(cpu, base, CAPTURE_FP_RISING);
    expect(state, dspic33_input_capture_test_rising_edge(cpu, 0u),
           "inactive trigger edge advances");
    expect(state,
           cpu->io.input_capture.fifo[0].count == 1u &&
               cpu->io.input_capture.fifo[0].words[0] == 0u && cpu->io.input_capture.timer[0] == 0u,
    ...
    expect(state, dspic33_device_advance(cpu, 2u), "every-edge interrupt delay advances");
    expect(state, interrupt_flag(cpu, 0u), "every-edge mode ignores capture interrupt interval");
    clear_interrupt(cpu, 0u);
    expect(state,
           set_level(cpu, 0u, false) && set_level(cpu, 0u, true) && set_level(cpu, 0u, false),
    ...
    configure_capture(cpu, 0u, 0u, false);
    expect(state, dspic33_input_capture_test_rising_edge(cpu, 0u),
           "supported rising edge advances");
    expect(state, cpu->io.input_capture.fifo[0].count == 1u,
    ...
    expect(state, set_level(cpu, 0u, true), "fourth-edge prescaler reaches next phase boundary");
    expect(state, cpu->io.input_capture.fifo[0].count == 1u,
    ...
    dspic33_write_word(cpu, base, 0u);
    dspic33_write_word(cpu, base, 0x1c04u);
    expect(state, set_level(cpu, 0u, true), "prescaler advances after module re-enable");
    expect(state, cpu->io.input_capture.fifo[0].count == 0u,
...
static void paired_firmware_case(TestState* state, Dspic33* cpu, uint8_t channel, uint8_t pin,
                                 uint16_t pps_register) {
    uint16_t first = dspic33_input_capture_test_capture_base(channel);
    uint16_t second = dspic33_input_capture_test_capture_base((uint8_t)(channel + 1u));
    dspic33_reset(cpu, 0u);
    ...
    expect(state, dspic33_input_capture_pin(cpu, pin, false, 0u), "queue mapped PPS low state");
    expect(state, dspic33_device_advance(cpu, 65534u), "advance paired counter toward wrap");
    expect(state, dspic33_input_capture_pin(cpu, pin, true, 0u), "queue first mapped PPS edge");
    ...
    expect(state, !interrupt_flag(cpu, channel), "first paired capture does not meet ICI interval");
    expect(state, dspic33_input_capture_pin(cpu, pin, false, 0u), "queue paired falling state");
    expect(state, dspic33_device_advance(cpu, 0u), "apply paired falling state");
    expect(state, dspic33_input_capture_pin(cpu, pin, true, 0u), "queue second mapped PPS edge");
    ...
    expect(state, dspic33_device_advance(cpu, 1u), "advance paired interrupt first cycle");
    expect(state, !interrupt_flag(cpu, channel), "paired interrupt waits second cycle");
    expect(state, dspic33_device_advance(cpu, 1u), "advance paired interrupt second cycle");
    expect(state, interrupt_flag(cpu, channel) && dspic33_device_interrupt_pending(cpu),
           "paired channel IRQ becomes pending");
    expect(state,
           dspic33_device_service_interrupt(cpu) && cpu->last_interrupt == capture_irqs[channel] &&
...
void dspic33_input_capture_test_dma_cases(TestState* state, Dspic33* cpu) {
    ...
    for (channel = 0u; channel < 4u; channel++) {
        ...
        configure_dma(cpu, capture_irqs[channel],
                      (uint16_t)(dspic33_input_capture_test_capture_base(channel) + 4u),
                      destination);
        configure_capture(cpu, channel, 0u, false);
        expect(state, dspic33_input_capture_test_rising_edge(cpu, channel), "DMA capture edge");
        expect(state,
    ...
    dspic33_reset(cpu, 0u);
    configure_dma(cpu, capture_irqs[0],
                  (uint16_t)(dspic33_input_capture_test_capture_base(0u) + 4u),
                  CAPTURE_DMA_DESTINATION);
    configure_capture(cpu, 0u, 1u, false);
    expect(state, dspic33_input_capture_test_rising_edge(cpu, 0u), "ICI nonzero DMA-negative edge");
    expect(state,
    ...
    dspic33_reset(cpu, 0u);
    configure_dma(cpu, capture_irqs[4],
                  (uint16_t)(dspic33_input_capture_test_capture_base(4u) + 4u),
                  CAPTURE_DMA_DESTINATION);
    configure_capture(cpu, 4u, 0u, false);
    expect(state, dspic33_input_capture_test_rising_edge(cpu, 4u), "IC5 DMA-negative edge");
    expect(state,
           dspic33_read_word(cpu, CAPTURE_DMA_DESTINATION) == 0u &&
...
void dspic33_input_capture_test_lifecycle_cases(TestState* state, Dspic33* cpu) {
    ...
    dspic33_reset(cpu, 0u);
    configure_capture(cpu, 0u, 0u, false);
    expect(state, dspic33_input_capture_input(cpu, 0u, true, 0u), "queue capture before copy");
    ...
    expect(state,
           cpu->io.input_capture.fifo[0].count == 1u && copy.io.input_capture.fifo[0].count == 1u &&
    ...
    expect(state, dspic33_copy(&copy, cpu), "copy preserves mapped physical capture state");
    expect(state,
           dspic33_gpio_drive(cpu, 3u, 1u, 1u) && dspic33_gpio_drive(&copy, 3u, 1u, 1u) &&
    ...
    dspic33_reset(cpu, 0u);
    configure_capture(cpu, 0u, 0u, false);
    expect(state, dspic33_input_capture_input(cpu, 0u, true, 0u), "queue capture before disable");
    ...
    dspic33_reset(cpu, 0u);
    configure_capture(cpu, 0u, 0u, false);
    expect(state, dspic33_input_capture_input(cpu, 0u, true, 0u), "queue capture before reset");
    dspic33_reset(cpu, 0u);
    expect(state, dspic33_device_advance(cpu, 3u), "advance reset capture queue");
    expect(state,
           cpu->io.input_capture.fifo[0].count == 0u && dspic33_read_word(cpu, 0x0140u) == 0u &&
               cpu->events.count == 0u,
    ...
    expect(state, !dspic33_input_capture_pin(cpu, 0u, true, 0u), "reject PPS ground source");
    expect(state, !dspic33_input_capture_pin(cpu, 128u, true, 0u), "reject invalid PPS source");
    ...
    expect(state, !dspic33_device_advance(cpu, 0u), "capture scheduling overflow stops advance");
    expect(state, cpu->stop_reason == DSPIC33_EVENT_QUEUE_ERROR && cpu->events.count == 0u,
...
void dspic33_input_capture_test_pmd_channel_cases(TestState* state, Dspic33* cpu) {
    ...
    for (channel = 0u; channel < DSPIC33_INPUT_CAPTURE_COUNT; channel++) {
        ...
        expect(state, dspic33_device_advance(cpu, 1u), "capture PMD disable transition advances");
        expect(state, (cpu->io.input_capture.pmd_disabled & (uint16_t)(1u << channel)) != 0u,
        ...
        dspic33_write_word(cpu, base, 0u);
        dspic33_write_word(cpu, (uint16_t)(base + 2u), 0u);
        expect(state, set_level(cpu, channel, true), "capture PMD disabled external edge advances");
        ...
        expect(state, dspic33_device_advance(cpu, 1u), "capture PMD enable transition advances");
        expect(state,
...
void dspic33_input_capture_test_pmd_lifecycle_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, dspic33_device_advance(cpu, 1u), "capture PMD disable pauses pending snapshot");
    expect(state,
    ...
    expect(state, dspic33_device_advance(cpu, 1u), "capture PMD enable resumes pending snapshot");
    ...
    dspic33_reset(cpu, 0u);
    configure_capture(cpu, 0u, 0u, false);
    expect(state, set_level(cpu, 0u, true), "capture before PMD interrupt pause advances");
    dspic33_write_word(cpu, CAPTURE_PMD_LOW, pmd_mask);
    expect(state, dspic33_device_advance(cpu, 1u), "capture PMD disable pauses delayed interrupt");
    expect(state,
           !interrupt_flag(cpu, 0u) && cpu->events.count == 1u && cpu->events.items[0].paused &&
               cpu->events.items[0].paused_remaining == 1u,
           "capture PMD preserves delayed interrupt remaining time");
    dspic33_write_word(cpu, CAPTURE_PMD_LOW, 0u);
    expect(state, dspic33_device_advance(cpu, 1u), "capture PMD enable resumes delayed interrupt");
    ...
    dspic33_reset(cpu, 0u);
    configure_capture(cpu, 0u, 0u, false);
    expect(state, dspic33_device_advance(cpu, 5u), "capture timer advances before PMD disable");
    dspic33_write_word(cpu, CAPTURE_PMD_LOW, pmd_mask);
    expect(state, dspic33_device_advance(cpu, 1u), "capture timer reaches PMD disable boundary");
    expect(state, cpu->io.input_capture.timer[0] == 6u, "capture timer includes delayed PMD cycle");
    expect(state, dspic33_device_advance(cpu, 10u) && cpu->io.input_capture.timer[0] == 6u,
           "capture timer stops while PMD disabled");
    dspic33_write_word(cpu, CAPTURE_PMD_LOW, 0u);
    expect(state, dspic33_device_advance(cpu, 1u) && cpu->io.input_capture.timer[0] == 6u,
           "capture timer remains stopped through PMD enable delay");
    expect(state, dspic33_device_advance(cpu, 4u) && cpu->io.input_capture.timer[0] == 10u,
    ...
    dspic33_write_word(cpu, CAPTURE_PMD_LOW, 0u);
    expect(state, dspic33_device_advance(cpu, 1u), "enable capture with preserved FIFO");
    expect(state,
    ...
    expect(state,
           cpu->io.input_capture.fifo[0].count == 0u && cpu->io.input_capture.fifo[1].count == 0u &&
    ...
    expect(state,
           cpu->io.input_capture.fifo[0].count == 1u && cpu->io.input_capture.fifo[1].count == 1u,
    ...
    expect(state, dspic33_device_advance(cpu, 1u), "dual PMD disable pauses paired snapshot");
    ...
    expect(state,
           cpu->io.input_capture.pmd_disabled == 1u && copy.io.input_capture.pmd_disabled == 1u,
    ...
    dspic33_write_word(cpu, CAPTURE_PMD_LOW, pmd_mask);
    dspic33_write_word(cpu, CAPTURE_PMD_LOW, 0u);
    expect(state, dspic33_device_advance(cpu, 1u), "advance replaced capture PMD transition");
    expect(state,
           cpu->io.input_capture.pmd_disabled == 0u &&
               dspic33_read_word(cpu, CAPTURE_PMD_LOW) == 0u && cpu->events.count == 0u,
    ...
    dspic33_write_word(cpu, CAPTURE_PMD_LOW, pmd_mask);
    expect(state, dspic33_device_advance(cpu, 1u), "complete capture PMD disable before reset");
    dspic33_reset(cpu, 0u);
    expect(state,
           cpu->io.input_capture.pmd_disabled == 0u &&
               cpu->io.input_capture.pmd_generation[0] == 0u &&
