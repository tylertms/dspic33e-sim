...
static void internal_counting_cases(TestState* state, Dspic33* cpu) {
    uint8_t timer;
    for (timer = 0u; timer < DSPIC33_TIMER_COUNT; timer++) {
        dspic33_reset(cpu, 0u);
...
static void prescaler_cases(TestState* state, Dspic33* cpu) {
...
static void interrupt_delay_cases(TestState* state, Dspic33* cpu) {
    uint8_t timer;
    for (timer = 0u; timer < DSPIC33_TIMER_COUNT; timer++) {
        dspic33_reset(cpu, 0u);
...
static void prescaler_reset_cases(TestState* state, Dspic33* cpu) {
    uint8_t timer;
    for (timer = 0u; timer < DSPIC33_TIMER_COUNT; timer++) {
        dspic33_reset(cpu, 0u);
...
static void gate_cases(TestState* state, Dspic33* cpu) {
    uint8_t timer;
    for (timer = 0u; timer < DSPIC33_TIMER_COUNT; timer++) {
        dspic33_reset(cpu, 0u);
...
static void external_clock_cases(TestState* state, Dspic33* cpu) {
    uint8_t timer;
    for (timer = 0u; timer < DSPIC33_TIMER_COUNT; timer++) {
...
static void delayed_event_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
...
static void pps_event_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
    dspic33_write_word(cpu, 0x0e3eu, 0u);
    dspic33_write_word(cpu, 0x06a6u, 64u);
...
static void paired_timer_cases(TestState* state, Dspic33* cpu) {
    uint8_t low;
...
static void paired_mode_cases(TestState* state, Dspic33* cpu) {
    uint8_t low;
    for (low = 1u; low < DSPIC33_TIMER_COUNT; low += 2u) {
        uint8_t high = (uint8_t)(low + 1u);
        uint16_t holding = timer_holding_registers[low / 2u];
...
static void power_mode_cases(TestState* state, Dspic33* cpu) {
    uint8_t timer;
    for (timer = 0u; timer < DSPIC33_TIMER_COUNT; timer++) {
        dspic33_reset(cpu, 0u);
...
static void doze_interrupt_delay_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
...
static void timer1_write_interlock_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
...
static void dma_trigger_cases(TestState* state, Dspic33* cpu) {
    uint8_t timer;
    for (timer = 0u; timer < DSPIC33_TIMER_COUNT; timer++) {
