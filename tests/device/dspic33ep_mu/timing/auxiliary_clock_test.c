...
static uint16_t control(Dspic33* cpu) { return dspic33_read_word(cpu, AUXILIARY_CLOCK_CONTROL); }
...
static void access_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, cpu->io.auxiliary_pll_generation == 0u, "POR clears auxiliary PLL generation");
    expect(state, cpu->events.count == 0u, "POR has no auxiliary PLL event");
    expect(state, dspic33_read_word(cpu, AUXILIARY_CLOCK_DIVISOR) == 0u, "POR clears ACLKDIV3");
    ...
    expect(state, (control(cpu) & AUXILIARY_PLL_LOCK) == 0u, "APLLCK rejects software set");
    ...
    expect(state, control(cpu) == 0xbe00u, "ACLKCON3 low-byte write preserves high fields");
    ...
    expect(state, control(cpu) == 0x00e7u, "ACLKCON3 high-byte write preserves low fields");
    ...
    expect(state,
           dspic33_read_word(cpu, AUXILIARY_CLOCK_DIVISOR) == AUXILIARY_CLOCK_DIVISOR_WRITABLE,
...
static void lock_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, cpu->io.auxiliary_pll_generation == 1u, "enable advances lock generation");
    ...
    expect(state, control(cpu) == (AUXILIARY_PLL_FRC_CONFIGURATION | AUXILIARY_PLL_LOCK),
           "auxiliary PLL locks at modeled deadline");
    expect(state, cpu->events.count == 0u, "lock event is consumed");
    expect(state, cpu->io.auxiliary_pll_generation == 1u, "lock completion preserves generation");

    dspic33_write_word(cpu, AUXILIARY_CLOCK_CONTROL, AUXILIARY_PLL_FRC_CONFIGURATION);
    expect(state, cpu->events.count == 0u, "same configuration does not relock");
    expect(state, (control(cpu) & AUXILIARY_PLL_LOCK) != 0u, "same configuration preserves lock");
    ...
    expect(state, cpu->io.auxiliary_pll_generation == 2u, "disable invalidates lock generation");
    ...
    expect(state, control(cpu) == (AUXILIARY_PLL_ENABLE | AUXILIARY_CLOCK_SOURCE_PRIMARY),
    ...
    expect(state, dspic33_device_advance(cpu, 31u) && (control(cpu) & AUXILIARY_PLL_LOCK) == 0u,
           "reconfigured PLL remains unlocked before deadline");
    expect(state, dspic33_device_advance(cpu, 1u) && (control(cpu) & AUXILIARY_PLL_LOCK) != 0u,
...
static void stale_event_cases(TestState* state, Dspic33* cpu) {
    ...
    dspic33_write_word(cpu, AUXILIARY_CLOCK_CONTROL, AUXILIARY_PLL_FRC_CONFIGURATION | 0x0001u);
    ...
    expect(state, dspic33_device_advance(cpu, 1u), "advance to current generation event");
    expect(state, (control(cpu) & AUXILIARY_PLL_LOCK) != 0u,
    ...
    expect(state, dspic33_device_advance(cpu, 1u), "advance after disabled stale deadline");
    expect(state, cpu->events.count == 0u, "disabled stale event is consumed");

    dspic33_write_word(cpu, AUXILIARY_CLOCK_CONTROL, 0x0001u);
    expect(state, cpu->events.count == 0u, "disabled configuration change schedules no lock");
...
static void divisor_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state,
           control(cpu) == 0x24c2u && dspic33_read_word(cpu, AUXILIARY_CLOCK_DIVISOR) == 0x0007u,
           "firmware auxiliary PLL configuration is retained");
    expect(state, cpu->events.count == 0u, "disabled firmware configuration schedules no lock");
    ...
    expect(state, cpu->io.auxiliary_pll_generation == generation + 1u && cpu->events.count == 1u,
           "live multiplier change schedules fresh lock generation");
    dspic33_write_word(cpu, AUXILIARY_CLOCK_DIVISOR, 0x0006u);
    expect(state, cpu->events.count == 1u, "same multiplier write schedules no duplicate lock");
    expect(state,
           dspic33_device_advance(cpu, 32u) &&
    ...
    expect(state,
           cpu->io.auxiliary_pll_generation == generation + 1u && cpu->events.count == 0u &&
               (control(cpu) & AUXILIARY_PLL_LOCK) == 0u,
    ...
    dspic33_device_advance(cpu, 1u);
    dspic33_write_word(cpu, AUXILIARY_CLOCK_DIVISOR, 0x0001u);
    expect(state, cpu->events.count == 2u, "multiplier change retains stale lock event");
    expect(state,
           dspic33_device_advance(cpu, 31u) && (control(cpu) & AUXILIARY_PLL_LOCK) == 0u &&
               cpu->events.count == 1u,
...
static void reconfiguration_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state,
           (control(cpu) & AUXILIARY_PLL_LOCK) != 0u &&
               cpu->io.auxiliary_pll_generation == generation && cpu->events.count == 0u,
    ...
    expect(state,
           (control(cpu) & AUXILIARY_PLL_LOCK) == 0u &&
               cpu->io.auxiliary_pll_generation == generation + 1u && cpu->events.count == 1u,
           "effective FRC to primary source change relocks");
    expect(state, dspic33_device_advance(cpu, 32u), "primary source change reaches lock");
    generation = cpu->io.auxiliary_pll_generation;
    ...
    configuration = (uint16_t)((configuration & ~AUXILIARY_CLOCK_OSCILLATOR_MODE) | 0x1000u);
    dspic33_write_word(cpu, AUXILIARY_CLOCK_CONTROL, configuration);
    expect(state,
           (control(cpu) & AUXILIARY_PLL_LOCK) != 0u &&
               cpu->io.auxiliary_pll_generation == generation && cpu->events.count == 0u,
    ...
    expect(state, dspic33_device_advance(cpu, 32u), "auxiliary source change reaches lock");
    generation = cpu->io.auxiliary_pll_generation;
    configuration = (uint16_t)((configuration & ~AUXILIARY_CLOCK_OSCILLATOR_MODE) | 0x0800u);
    dspic33_write_word(cpu, AUXILIARY_CLOCK_CONTROL, configuration);
    expect(state,
           (control(cpu) & AUXILIARY_PLL_LOCK) == 0u &&
               cpu->io.auxiliary_pll_generation == generation + 1u && cpu->events.count == 1u,
    ...
    dspic33_write_word(cpu, OSCILLATOR_TUNING, 0x0001u);
    expect(state, cpu->events.count == 1u, "same FRC tuning write schedules no duplicate lock");
}
...
static void source_admission_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, dspic33_device_advance(cpu, 32u) && (control(cpu) & AUXILIARY_PLL_LOCK) == 0u,
           "unavailable auxiliary input remains unlocked");
    dspic33_write_word(cpu, AUXILIARY_CLOCK_CONTROL, AUXILIARY_PLL_ENABLE | 0x0800u);
    expect(state, cpu->events.count == 1u, "configured auxiliary oscillator starts PLL lock");
    expect(state, dspic33_device_advance(cpu, 32u) && (control(cpu) & AUXILIARY_PLL_LOCK) != 0u,
    ...
    for (mode = 0u; mode < 4u; mode++) {
        ...
        dspic33_load_configuration_word(cpu, OSCILLATOR_CONFIGURATION, (uint16_t)(0x005cu | mode));
        dspic33_reset(cpu, 0u);
        dspic33_write_word(cpu, AUXILIARY_CLOCK_CONTROL,
                           AUXILIARY_PLL_ENABLE | AUXILIARY_CLOCK_SOURCE_PRIMARY);
...
static void configuration_matrix_cases(TestState* state, Dspic33* cpu) {
    ...
    for (ordinal = 0u; ordinal < 4096u; ordinal++) {
        ...
        for (multiplier = 0u; multiplier < 8u; multiplier++) {
            ...
            dspic33_reset(cpu, 0u);
            dspic33_write_word(cpu, AUXILIARY_CLOCK_DIVISOR, (uint16_t)(0xfff8u | multiplier));
            dspic33_write_word(cpu, AUXILIARY_CLOCK_CONTROL,
            ...
            expect(state,
                   cpu->events.count == (((configuration & AUXILIARY_PLL_ENABLE) != 0u &&
...
static void clock_lock_cases(TestState* state, Dspic33* cpu) {
    ...
    for (ordinal = 0u; ordinal < 4096u; ordinal++) {
        ...
        for (multiplier = 0u; multiplier < 8u; multiplier++) {
            ...
            dspic33_write_word(cpu, AUXILIARY_CLOCK_CONTROL, configuration);
            expect(state,
                   control(cpu) == 0u && dspic33_read_word(cpu, AUXILIARY_CLOCK_DIVISOR) == 0u &&
    ...
    dspic33_write_byte(cpu, AUXILIARY_CLOCK_CONTROL, 0xffu);
    expect(state,
           control(cpu) == 0u && cpu->io.auxiliary_pll_generation == 0u && cpu->events.count == 0u,
    ...
    dspic33_write_word(cpu, AUXILIARY_CLOCK_CONTROL, 0xa4c2u);
    expect(state,
           control(cpu) == 0xa4c2u && dspic33_read_word(cpu, AUXILIARY_CLOCK_DIVISOR) == 0x0007u &&
               cpu->events.count == 1u,
...
static void lifecycle_cases(TestState* state, Dspic33* source, Dspic33* copy) {
    ...
    dspic33_write_word(source, AUXILIARY_CLOCK_CONTROL, AUXILIARY_PLL_FRC_CONFIGURATION);
    dspic33_device_advance(source, 10u);
    expect(state, dspic33_copy(copy, source), "copy preserves auxiliary PLL state");
    expect(state, copy->io.auxiliary_pll_generation == source->io.auxiliary_pll_generation,
           "copy preserves auxiliary PLL generation");
    expect(state, copy->events.count == source->events.count, "copy preserves pending lock event");
    expect(state, dspic33_read_word(copy, AUXILIARY_CLOCK_DIVISOR) == 0x0007u,
           "copy preserves auxiliary PLL divisor");
    expect(state,
           dspic33_device_advance(source, 22u) && (control(source) & AUXILIARY_PLL_LOCK) != 0u,
           "source completes copied lock interval");
    expect(state, dspic33_device_advance(copy, 22u) && (control(copy) & AUXILIARY_PLL_LOCK) != 0u,
    ...
    dspic33_write_word(source, AUXILIARY_CLOCK_CONTROL, AUXILIARY_PLL_FRC_CONFIGURATION);
    dspic33_device_advance(source, 10u);
    expect(state, source->events.count == 1u, "cold reset starts with pending lock event");
    dspic33_reset(source, 0u);
    ...
    expect(state, source->io.auxiliary_pll_generation == 0u, "cold reset clears lock generation");
    ...
    expect(state,
           dspic33_device_advance(source, 20u) && (control(source) & AUXILIARY_PLL_LOCK) == 0u,
    ...
    expect(state,
           control(source) == (AUXILIARY_PLL_FRC_CONFIGURATION | AUXILIARY_PLL_LOCK) &&
               dspic33_read_word(source, AUXILIARY_CLOCK_DIVISOR) == 0x0005u &&
               source->io.auxiliary_pll_generation == generation && source->events.count == 0u,
    ...
    expect(state, source->events.count == 0u, "failed lock schedule leaves no pending event");
}
