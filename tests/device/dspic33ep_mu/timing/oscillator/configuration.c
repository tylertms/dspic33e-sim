...
Dspic33StopReason dspic33_oscillator_test_write_protected_byte(Dspic33* cpu, uint16_t address,
    ...
    dspic33_oscillator_test_load_sequence(cpu, OPCODE_MOV_BYTE_W2_W1, OPCODE_MOV_BYTE_W3_W1,
...
bool dspic33_oscillator_test_select_locked_main_pll(Dspic33* cpu, uint8_t source) {
    ...
    if (dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL + 1u, source) !=
            DSPIC33_RUNNING ||
        dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL,
                                                     OSCILLATOR_SWITCH_ENABLE) != DSPIC33_RUNNING ||
        !dspic33_device_advance(cpu, OSCILLATOR_SWITCH_DELAY)) {
        return false;
    }
    return dspic33_oscillator_test_control(cpu) ==
           (uint16_t)(source * 0x1100u + OSCILLATOR_PLL_LOCK);
}

void dspic33_oscillator_test_oscillator_pin_cases(TestState* state, Dspic33* source,
                                                  Dspic33* copy) {
    ...
    expect(state,
           dspic33_oscillator_pin(source, &clock_output, &initial_edges) && clock_output &&
               dspic33_device_advance(source, 7u) &&
    ...
    expect(state,
           dspic33_oscillator_pin(source, &clock_output, &advanced_edges) && clock_output &&
               advanced_edges == UINT64_MAX,
    ...
    expect(state,
           dspic33_oscillator_pin(source, &clock_output, &advanced_edges) && !clock_output &&
               advanced_edges == 0u && !dspic33_gpio_pin(source, 2u, 15u, &clock_output),
...
void dspic33_oscillator_test_reset_cases(TestState* state, Dspic33* cpu) {
    ...
    dspic33_reset(cpu, 0u);
    expect(state, dspic33_oscillator_test_control(cpu) == 0x7700u,
           "POR immediately selects ready internal FNOSC source");
    expect(state, (dspic33_oscillator_test_control(cpu) & OSCILLATOR_PLL_LOCK) == 0u,
    ...
    dspic33_reset(cpu, 0u);
    expect(state, dspic33_oscillator_test_control(cpu) == 0u,
           "firmware configuration starts from FRC");
}
...
void dspic33_oscillator_test_protection_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, dspic33_oscillator_test_control(cpu) == 0u, "word write cannot unlock OSCCON");
    dspic33_write_byte(cpu, OSCILLATOR_CONTROL + 1u, 0x03u);
    expect(state, dspic33_oscillator_test_control(cpu) == 0u,
           "direct byte write cannot unlock OSCCON");
    expect(state,
           dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL + 1u, 0x03u) ==
               DSPIC33_RUNNING,
           "high-byte unlock sequence executes");
    expect(state, dspic33_oscillator_test_control(cpu) == 0x0300u,
           "high-byte unlock writes NOSC only");
    expect(state,
           dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL, 0x42u) ==
               DSPIC33_RUNNING,
           "low-byte unlock sequence executes");
    expect(state, dspic33_oscillator_test_control(cpu) == 0x0342u,
           "low-byte unlock writes IOLOCK and LPOSCEN");
    expect(state,
           dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL, 0x80u) ==
               DSPIC33_RUNNING,
           "CLKLOCK set sequence executes");
    expect(state, dspic33_oscillator_test_control(cpu) == 0x0380u,
           "CLKLOCK sets and other controls clear");
    expect(state,
           dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL, 0u) ==
               DSPIC33_RUNNING,
           "CLKLOCK clear attempt executes");
    expect(state, dspic33_oscillator_test_control(cpu) == 0x0380u, "CLKLOCK is software set-only");

    dspic33_reset(cpu, 0u);
    dspic33_oscillator_test_load_sequence(cpu, OPCODE_MOV_BYTE_W2_W1, OPCODE_NOP,
    ...
    expect(state, dspic33_oscillator_test_control(cpu) == 0u,
           "intervening instruction invalidates key");
    ...
    expect(state, dspic33_oscillator_test_control(cpu) == 0u, "wrong second key blocks write");
    ...
    expect(state, dspic33_oscillator_test_control(cpu) == 0u, "interrupt entry invalidates key");
}
...
void dspic33_oscillator_test_switch_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state,
           dspic33_oscillator_test_write_protected_byte(
               cpu, OSCILLATOR_CONTROL, OSCILLATOR_SWITCH_ENABLE) == DSPIC33_RUNNING,
           "firmware low-byte switch request executes");
    expect(state, dspic33_oscillator_test_control(cpu) == 0x0301u,
    ...
    expect(state, cpu->oscillator.generation == 1u, "switch advances oscillator generation");
    ...
    expect(state, dspic33_device_advance(cpu, 1u), "advance to modeled switch deadline");
    expect(state, dspic33_oscillator_test_control(cpu) == 0x3320u,
    ...
    expect(state, dspic33_oscillator_test_control(cpu) == 0u && cpu->events.count == 0u,
           "redundant switch clears OSWEN without event");

    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL + 1u, 0x03u);
    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL, OSCILLATOR_SWITCH_ENABLE);
    expect(state, cpu->oscillator.active, "abort case starts active switch");
    expect(state,
           dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL, 0u) ==
               DSPIC33_RUNNING,
           "explicit switch abort executes");
    expect(state, !cpu->oscillator.active && (dspic33_oscillator_test_control(cpu) & 1u) == 0u,
    ...
    expect(state, (dspic33_oscillator_test_control(cpu) & 0x7021u) == 0u,
           "aborted event cannot change source or lock");

    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL + 1u, 0x03u);
    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL, OSCILLATOR_SWITCH_ENABLE);
    expect(state, cpu->oscillator.active, "replacement case starts first switch");
    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL + 1u, 0x02u);
    expect(state, !cpu->oscillator.active && dspic33_oscillator_test_control(cpu) == 0x0200u,
    ...
    expect(state,
           dspic33_device_advance(cpu, 25u) && dspic33_oscillator_test_control(cpu) == 0x0201u,
    ...
    expect(state,
           dspic33_device_advance(cpu, 6u) && dspic33_oscillator_test_control(cpu) == 0x2200u,
    ...
    cpu->pc = 0x0200u;
    expect(state, dspic33_step(cpu) == DSPIC33_SLEEPING, "Sleep executes during clock switch");
    expect(state, !cpu->oscillator.active && (dspic33_oscillator_test_control(cpu) & 1u) == 0u,
           "Sleep aborts clock switch");

    dspic33_reset(cpu, 0u);
    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL, OSCILLATOR_CLOCK_LOCK);
    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL + 1u, 0x03u);
    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL,
                                                 OSCILLATOR_CLOCK_LOCK | OSCILLATOR_SWITCH_ENABLE);
    expect(state, dspic33_oscillator_test_control(cpu) == 0x0080u,
           "CLKLOCK blocks subsequent source switch");
    ...
    expect(state, dspic33_oscillator_test_control(cpu) == 0x0080u,
           "CLKLOCK blocks LPOSCEN changes");
}
...
void dspic33_oscillator_test_failure_trap_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state,
           dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL,
                                                        OSCILLATOR_CLOCK_FAIL) == DSPIC33_RUNNING,
           "CF write one executes without a software-generated failure");
    expect(state,
           (dspic33_oscillator_test_control(cpu) & OSCILLATOR_CLOCK_FAIL) == 0u &&
               cpu->trap_count == 0u,
    ...
    expect(state, (dspic33_oscillator_test_control(cpu) & OSCILLATOR_CLOCK_FAIL) != 0u,
    ...
    for (fcksm = 0u; fcksm < 4u; fcksm++) {
        for (source = 0u; source < 8u; source++) {
            ...
            dspic33_load_configuration_word(cpu, CONFIGURATION_FOSC,
                                            (uint8_t)(0x001eu | (uint8_t)(fcksm << 6u)));
            dspic33_load_configuration_word(cpu, CONFIGURATION_FWDT, 0x00ffu);
            dspic33_reset(cpu, 0u);
            dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL + 1u, source);
            dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL,
            ...
            if (switching_enabled && source != 0u) {
                ...
                expect(state,
                       cpu->oscillator.active && cpu->events.count == 1u &&
                           (dspic33_oscillator_test_control(cpu) & OSCILLATOR_SWITCH_ENABLE) != 0u,
                       "enabled source request starts one switch lifecycle");
                expect(state,
                       dspic33_device_advance(cpu, OSCILLATOR_SWITCH_DELAY) &&
                           dspic33_oscillator_test_control(cpu) == expected &&
                           !cpu->oscillator.active && cpu->events.count == 0u,
    ...
    for (fcksm = 0u; fcksm < 4u; fcksm++) {
        for (source = 0u; source < 8u; source++) {
            ...
            dspic33_load_configuration_word(cpu, CONFIGURATION_FOSC,
                                            (uint8_t)(0x001eu | (uint8_t)(fcksm << 6u)));
            dspic33_reset(cpu, 0u);
            previous = dspic33_oscillator_test_control(cpu);
            expect(state, dspic33_oscillator_failure_detected(cpu) == expected,
                   "FSCM admission follows source and FCKSM configuration");
            expect(state,
                   expected ? dspic33_oscillator_test_control(cpu) ==
                                  (uint16_t)((uint16_t)source << 8u | OSCILLATOR_CLOCK_FAIL)
                            : dspic33_oscillator_test_control(cpu) == previous,
...
void dspic33_oscillator_test_configuration_admission_cases(TestState* state, Dspic33* cpu) {
    ...
    size_t index;
    for (index = 0u; index < sizeof(disabled_configurations) / sizeof(disabled_configurations[0]);
         index++) {
        dspic33_load_configuration_word(cpu, CONFIGURATION_FOSC, disabled_configurations[index]);
        dspic33_reset(cpu, 0u);
        dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL + 1u, 0x02u);
        dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL,
                                                     OSCILLATOR_SWITCH_ENABLE);
        expect(state, dspic33_oscillator_test_control(cpu) == 0x0200u,
    ...
    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL,
                                                 OSCILLATOR_CLOCK_LOCK | OSCILLATOR_SWITCH_ENABLE);
    expect(state, dspic33_oscillator_test_control(cpu) == 0x0281u && cpu->oscillator.active,
           "FCKSM zero permits switch with CLKLOCK set");
    expect(state, cpu->events.count == 1u, "conditional CLKLOCK permission schedules switch");
    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL,
                                                 OSCILLATOR_CLOCK_LOCK | OSCILLATOR_LP_ENABLE);
    expect(state, (dspic33_oscillator_test_control(cpu) & OSCILLATOR_LP_ENABLE) != 0u,
           "FCKSM zero permits LPOSCEN change with CLKLOCK set");

    dspic33_reset(cpu, 0u);
    dspic33_load_configuration_word(cpu, CONFIGURATION_FOSC, 0x9eu);
    dspic33_reset(cpu, 0u);
    expect(state, dspic33_oscillator_test_program_fosc(cpu, 0x1eu),
           "FOSC RTSP enables clock switching");
    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL + 1u, 0x02u);
    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL, OSCILLATOR_SWITCH_ENABLE);
    expect(state, dspic33_oscillator_test_control(cpu) == 0x0201u && cpu->oscillator.active,
           "immediate FCKSM update affects next switch");
    expect(state, cpu->events.count == 1u, "RTSP-enabled switch schedules completion");
    expect(state, dspic33_oscillator_test_program_fosc(cpu, 0x009eu),
           "FOSC RTSP disables active software switching");
    expect(state, !cpu->oscillator.active && dspic33_oscillator_test_control(cpu) == 0x0200u,
           "switch disable aborts active software request");
    expect(state,
           dspic33_device_advance(cpu, 32u) && dspic33_oscillator_test_control(cpu) == 0x0200u,
    ...
    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL + 1u, 0x03u);
    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL, OSCILLATOR_SWITCH_ENABLE);
    expect(state, dspic33_oscillator_test_control(cpu) == 0x1321u,
           "FRCPLL to POSCPLL direct switch remains pending");
    expect(state, cpu->oscillator.active && cpu->events.count == 0u,
           "first direct PLL request schedules no completion");
    expect(state, dspic33_oscillator_test_program_fosc(cpu, 0x005du),
           "POSCMD mode changes during direct PLL request");
    expect(state,
           dspic33_device_advance(cpu, 32u) && dspic33_oscillator_test_control(cpu) == 0x1321u &&
               cpu->events.count == 0u,
           "POSCMD mode change cannot complete direct PLL request");
    expect(state, dspic33_oscillator_test_program_fosc(cpu, 0x005fu),
           "POSCMD disables target during direct PLL request");
    expect(state, dspic33_oscillator_test_program_fosc(cpu, 0x005eu),
    ...
    expect(state, program_fwdt(cpu, 0x00ffu), "PLLK restores during direct PLL request");
    expect(state,
           dspic33_device_advance(cpu, 32u) && dspic33_oscillator_test_control(cpu) == 0x1321u &&
               cpu->events.count == 0u,
           "PLLK restore cannot complete direct PLL request");
    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL, 0u);
    expect(state, dspic33_oscillator_test_control(cpu) == 0x1320u && !cpu->oscillator.active,
           "explicit clear aborts unsupported direct PLL request");

    dspic33_load_configuration_word(cpu, CONFIGURATION_FOSCSEL, 0x007bu);
    dspic33_reset(cpu, 0u);
    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL + 1u, 0x01u);
    dspic33_oscillator_test_write_protected_byte(cpu, OSCILLATOR_CONTROL, OSCILLATOR_SWITCH_ENABLE);
    expect(state, dspic33_oscillator_test_control(cpu) == 0x3121u,
    ...
    expect(state, dspic33_oscillator_test_program_fosc(cpu, 0x005fu),
           "FOSC RTSP disables pending POSC");
    expect(state,
           dspic33_device_advance(cpu, 32u) && dspic33_oscillator_test_control(cpu) == 0x0201u,
           "disabled POSC invalidates scheduled completion");
    expect(state, dspic33_oscillator_test_program_fosc(cpu, 0x005eu),
           "FOSC RTSP restores pending POSC");
    expect(state,
           dspic33_device_advance(cpu, 32u) && dspic33_oscillator_test_control(cpu) == 0x2200u,
...
void dspic33_oscillator_test_hardware_failure_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, dspic33_oscillator_test_control(cpu) == 0x0208u,
    ...
    expect(state, dspic33_step(cpu) == DSPIC33_TRAPPED, "FSCM dispatches oscillator-fail trap");
    expect(state, cpu->pc == 0x000302u && cpu->last_trap == 0u && cpu->last_trap_return == 0u,
           "FSCM enters oscillator-fail vector");

    dspic33_load_configuration_word(cpu, CONFIGURATION_FOSC, 0x005eu);
    dspic33_reset(cpu, 0u);
    expect(state,
           !dspic33_oscillator_failure_detected(cpu) &&
               dspic33_oscillator_test_control(cpu) == 0x2200u,
    ...
    dspic33_reset(cpu, 0u);
    expect(state,
           !dspic33_oscillator_failure_detected(cpu) && dspic33_oscillator_test_control(cpu) == 0u,
...
void dspic33_oscillator_test_pll_lock_sequence_cases(TestState* state, Dspic33* source,
    ...
    dspic33_oscillator_test_write_protected_byte(source, OSCILLATOR_CONTROL,
                                                 OSCILLATOR_SWITCH_ENABLE);
    expect(state,
           dspic33_oscillator_test_control(source) == 0x3300u && !source->oscillator.active &&
    ...
    dspic33_oscillator_test_write_protected_byte(source, OSCILLATOR_CONTROL, OSCILLATOR_IO_LOCK);
    expect(state,
           dspic33_oscillator_test_control(source) == 0x3340u && source->oscillator.lock_pending &&
               source->events.count == 1u,
           "unrelated OSCCON write preserves pending PLL lock");
    expect(state,
           dspic33_device_advance(source, 27u) &&
               dspic33_oscillator_test_control(source) == 0x3340u,
           "PLLK zero remains unlocked before modeled lock deadline");
    expect(state,
           dspic33_device_advance(source, 1u) && dspic33_oscillator_test_control(source) == 0x3360u,
    ...
    expect(state, program_fwdt(source, 0x00dfu), "PLLK RTSP clears pending wait policy");
    expect(state,
           source->oscillator.generation == 1u && source->events.count == 1u &&
               dspic33_oscillator_test_control(source) == 0x3300u &&
               source->oscillator.lock_pending,
           "PLLK clear releases source without moving lock deadline");
    expect(state,
           dspic33_device_advance(source, 30u) &&
               dspic33_oscillator_test_control(source) == 0x3300u,
    ...
    expect(state, source->events.count == 0u, "unavailable source still schedules no PLL event");
    expect(state, dspic33_oscillator_test_program_fosc(source, 0x005eu),
    ...
    expect(state,
           dspic33_device_advance(copy, 21u) && dspic33_oscillator_test_control(copy) == 0x3320u,
    ...
    expect(state,
           !source->oscillator.lock_pending && dspic33_oscillator_test_control(source) == 0x3300u,
...
void dspic33_oscillator_test_two_speed_startup_cases(TestState* state, Dspic33* source,
    ...
    dspic33_reset(source, 0u);
    expect(state, dspic33_oscillator_test_control(source) == 0u && !source->oscillator.active,
           "Two-Speed FRC target is redundant");

    for (index = 0u; index < sizeof(internal_sources) / sizeof(internal_sources[0]); index++) {
        ...
        dspic33_reset(source, 0u);
        expect(state, dspic33_oscillator_test_control(source) == (uint16_t)(source_id * 0x1100u),
    ...
    dspic33_reset(source, 0u);
    expect(state,
           dspic33_oscillator_test_control(source) == 0x0200u && source->oscillator.active &&
    ...
    expect(state,
           dspic33_device_advance(source, 31u) &&
               dspic33_oscillator_test_control(source) == 0x0200u,
           "Two-Speed POSC remains pending before readiness");
    expect(state,
           dspic33_device_advance(source, 1u) && dspic33_oscillator_test_control(source) == 0x2200u,
           "Two-Speed transfers to ready POSC");

    dspic33_load_configuration_word(source, CONFIGURATION_FOSCSEL, 0x00fcu);
    dspic33_reset(source, 0u);
    expect(state,
           dspic33_oscillator_test_control(source) == 0x0400u && source->oscillator.automatic &&
               source->events.count == 1u,
           "Two-Speed begins SOSC transition without LPOSCEN");
    expect(state,
           dspic33_device_advance(source, 32u) &&
               dspic33_oscillator_test_control(source) == 0x4400u,
    ...
    expect(state,
           dspic33_device_advance(source, 1u) &&
               dspic33_oscillator_test_control(source) == 0x0300u &&
               source->oscillator.source_ready && source->oscillator.lock_pending &&
    ...
    dspic33_reset(source, 0u);
    expect(state,
           dspic33_oscillator_test_control(source) == 0x0100u && source->oscillator.automatic,
    ...
    expect(state,
           dspic33_device_advance(source, 29u) &&
    ...
    dspic33_oscillator_test_write_protected_byte(source, OSCILLATOR_CONTROL,
                                                 OSCILLATOR_CLOCK_LOCK | OSCILLATOR_SWITCH_ENABLE);
    expect(state,
           dspic33_oscillator_test_control(source) == 0x0280u && source->oscillator.active &&
               source->oscillator.automatic && source->oscillator.generation == 1u &&
               source->events.items[0].cycle == deadline,
           "CLKLOCK rejection preserves automatic startup deadline");
    expect(state,
           dspic33_device_advance(source, 26u) &&
               dspic33_oscillator_test_control(source) == 0x2280u,
    ...
    dspic33_oscillator_test_write_protected_byte(source, OSCILLATOR_CONTROL, OSCILLATOR_IO_LOCK);
    expect(state,
           dspic33_oscillator_test_control(source) == 0x0240u && source->oscillator.active &&
               source->oscillator.automatic,
           "OSCCON low write preserves automatic startup");
    expect(state,
           dspic33_device_advance(source, 29u) &&
               dspic33_oscillator_test_control(source) == 0x2240u,
    ...
    expect(state, !source->oscillator.active && dspic33_oscillator_test_control(source) == 0x0200u,
    ...
    expect(state, copy->oscillator.active && copy->oscillator.automatic && copy->events.count == 1u,
           "copy retains automatic startup phase");
    expect(
        state,
        dspic33_device_advance(source, 22u) && dspic33_oscillator_test_control(source) == 0x2200u &&
            dspic33_device_advance(copy, 22u) && dspic33_oscillator_test_control(copy) == 0x2200u,
    ...
    expect(state, dspic33_oscillator_test_control(source) == 0u && source->events.count == 0u,
           "enabled switching defers FNOSC until reset");
    dspic33_reset(source, 0u);
    expect(state, dspic33_oscillator_test_control(source) == 0x4400u,
           "reset applies deferred FNOSC configuration");
    expect(state, program_foscsel(source, 0x00fcu), "IESO RTSP changes startup policy");
    expect(state, dspic33_oscillator_test_control(source) == 0x4400u && source->events.count == 0u,
