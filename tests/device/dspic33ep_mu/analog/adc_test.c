...
static void format_cases(TestState* state, Dspic33* cpu) {
...
static void done_access_cases(TestState* state, Dspic33* cpu) {
    uint8_t module;
    for (module = 0u; module < DSPIC33_ADC_COUNT; module++) {
        dspic33_reset(cpu, 0u);
...
static void done_active_conversion_cases(TestState* state, Dspic33* cpu) {
    uint8_t module;
    for (module = 0u; module < DSPIC33_ADC_COUNT; module++) {
        dspic33_reset(cpu, 0u);
        set_input(cpu, 0u, 400u);
...
static void channel_cases(TestState* state, Dspic33* cpu) {
    uint8_t channel;
...
static void sequence_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
...
static void threshold_cases(TestState* state, Dspic33* cpu) {
    uint8_t module;
    uint8_t threshold;
    uint8_t sample;
    for (module = 0u; module < DSPIC33_ADC_COUNT; module++) {
...
static void dma_interrupt_rate_cases(TestState* state, Dspic33* cpu) {
    uint8_t module;
    uint8_t sample;
    for (module = 0u; module < DSPIC33_ADC_COUNT; module++) {
        dspic33_reset(cpu, 0u);
...
static void timer_trigger_cases(TestState* state, Dspic33* cpu) {
...
static void analog_pin_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
...
int main(void) {
    ...
    if (!initialized) {
        fprintf(stderr, "cannot initialize ADC test simulator\n");
        return 1;
    }
