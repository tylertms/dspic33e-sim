...
static void configure(Dspic33* cpu, uint16_t control) {
...
static void frame_admission_matrix(TestState* state, Dspic33* cpu) {
    configure(cpu, DCI_ENABLE | DCI_EXTERNAL_FRAME);
...
static void read_lane_matrix(TestState* state, Dspic33* cpu) {
    ...
    expect(state, dspic33_device_internal_dci_read_register(cpu, DCI_RECEIVE_BASE, &value),
    ...
    expect(state, dspic33_device_internal_dci_read_register(cpu, DCI_RECEIVE_BASE + 1u, &value),
