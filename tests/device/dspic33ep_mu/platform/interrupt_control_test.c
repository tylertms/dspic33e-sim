...
static void prepare_external_interrupts(Dspic33* cpu, uint16_t levels, uint16_t polarity) {
...
static void enable_interrupt(Dspic33* cpu, uint8_t irq, uint8_t priority, uint32_t vector) {
    ...
    dspic33_write_word(cpu, enable, (uint16_t)(dspic33_read_word(cpu, enable) | enable_mask));
    dspic33_write_word(
        cpu, priority_address,
        (uint16_t)((dspic33_read_word(cpu, priority_address) & ~(uint16_t)(7u << shift)) |
...
static void access_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, dspic33_read_word(cpu, INTCON1) == 0u, "software clears writable INTCON1 status");

    dspic33_device_latch_math_error(cpu, 0x0040u);
    expect(state, dspic33_read_word(cpu, INTCON1) == 0x0050u && pending_trap(cpu, 4u) != NULL,
           "hardware math source latches status and schedules trap");
    dspic33_write_word(cpu, INTCON1, 0x0040u);
    expect(state, dspic33_read_word(cpu, INTCON1) == 0x0040u && pending_trap(cpu, 4u) == NULL,
    ...
    expect(state, dspic33_read_word(cpu, INTCON3) == 0x0070u, "INTCON3 rejects reserved fields");
    ...
    expect(state, dspic33_read_word(cpu, INTCON4) == 0x0001u, "INTCON4 rejects reserved fields");
    ...
    expect(state, dspic33_read_word(cpu, INTTREG) == 0x0906u, "INTTREG word write is ignored");
...
static void generic_hard_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state,
           dspic33_read_word(cpu, INTCON2) == 0xa000u && dspic33_read_word(cpu, INTCON4) == 0x0001u,
    ...
    expect(state,
           dspic33_read_word(cpu, INTTREG) == 0x0d02u && (cpu->sr & 0x00e0u) == 0x00a0u &&
               (cpu->corcon & 0x0008u) != 0u,
           "generic hard trap enters priority thirteen");
    expect(state,
           dspic33_read_word(cpu, 0x2000u) == 2u && dspic33_read_word(cpu, 0x2002u) == 0x0500u &&
               cpu->w[15] == 0x2004u,
           "generic hard trap stacks completed instruction state");

    dspic33_write_word(cpu, INTCON2, 0u);
    expect(state,
           dspic33_read_word(cpu, INTCON2) == 0u && dspic33_read_word(cpu, INTCON4) == 1u &&
               pending_trap(cpu, 2u) != NULL,
    ...
    dspic33_write_word(cpu, INTCON4, 0u);
    expect(state, pending_trap(cpu, 2u) == NULL, "clearing final hard source cancels reentry");
    cpu->stop_reason = DSPIC33_RUNNING;
    expect(state,
           dspic33_step(cpu) == DSPIC33_RUNNING && cpu->pc == 2u && cpu->interrupt_depth == 0u,
...
static void generic_soft_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state,
           dspic33_read_word(cpu, INTTREG) == 0x0906u && (cpu->sr & 0x00e0u) == 0x0020u &&
               (cpu->corcon & 0x0008u) != 0u,
           "generic soft trap enters priority nine");
    expect(state,
           dspic33_read_word(cpu, 0x3000u) == 2u && dspic33_read_word(cpu, 0x3002u) == 0x0500u &&
               cpu->w[15] == 0x3004u,
           "generic soft trap stacks completed instruction state");

    dspic33_write_word(cpu, INTCON3, 0u);
    expect(state, pending_trap(cpu, 6u) == NULL, "clearing generic soft sources cancels reentry");
    cpu->stop_reason = DSPIC33_RUNNING;
    expect(state,
           dspic33_step(cpu) == DSPIC33_RUNNING && cpu->pc == 2u && cpu->interrupt_depth == 0u,
...
static void external_interrupt_edge_cases(TestState* state, Dspic33* cpu) {
    ...
    for (channel = 0u; channel < DSPIC33_EXTERNAL_INTERRUPT_COUNT; channel++) {
        ...
        dspic33_gpio_drive(cpu, 3u, pin, pin);
        expect(state, !interrupt_flag(cpu, irq), "stable high INT input does not retrigger");
        dspic33_gpio_drive(cpu, 3u, 0u, pin);
        expect(state, !interrupt_flag(cpu, irq), "falling edge does not match positive polarity");
    }
    ...
    for (channel = 0u; channel < DSPIC33_EXTERNAL_INTERRUPT_COUNT; channel++) {
        ...
        clear_interrupt(cpu, irq);
        dspic33_gpio_drive(cpu, 3u, 0u, pin);
        expect(state, !interrupt_flag(cpu, irq), "stable low INT input does not retrigger");
        dspic33_gpio_drive(cpu, 3u, pin, pin);
        expect(state, !interrupt_flag(cpu, irq), "rising edge does not match negative polarity");
    }

    prepare_external_interrupts(cpu, 0u, 0u);
    dspic33_write_byte(cpu, INTCON2, 0x1fu);
    expect(state,
           !interrupt_flag(cpu, 0u) && !interrupt_flag(cpu, 20u) && !interrupt_flag(cpu, 29u) &&
               !interrupt_flag(cpu, 53u) && !interrupt_flag(cpu, 54u),
...
static void external_interrupt_selection_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state,
           !interrupt_flag(cpu, 20u) && cpu->io.external_interrupt_selection[1] == 69u &&
    ...
    dspic33_gpio_drive(cpu, 3u, 0x0020u, 0x0020u);
    expect(state, !interrupt_flag(cpu, 20u) && (cpu->io.external_interrupt_qualified & 0x02u) == 0u,
           "unimplemented PPS selection cannot drive INT1");
    dspic33_write_word(cpu, RPINR0, 0u);
    expect(state,
           !interrupt_flag(cpu, 20u) && (cpu->io.external_interrupt_qualified & 0x02u) != 0u &&
               (cpu->io.external_interrupt_levels & 0x02u) == 0u,
    ...
    dspic33_write_word(cpu, TRISD, UINT16_MAX);
    expect(state, !interrupt_flag(cpu, 20u), "input qualification baselines INT1 level");
    dspic33_gpio_drive(cpu, 3u, 0u, 0x0002u);
    dspic33_gpio_drive(cpu, 3u, 0x0002u, 0x0002u);
    expect(state, interrupt_flag(cpu, 20u), "qualified digital input restores INT1 edges");
    ...
    dspic33_write_word(cpu, ANSELD, 0u);
    expect(state, !interrupt_flag(cpu, 20u), "digital qualification rebaselines INT1 level");
    dspic33_gpio_drive(cpu, 3u, 0x0002u, 0x0002u);
...
static void configure_output_compare_trigger(Dspic33* cpu, uint8_t source) {
    ...
    dspic33_write_word(cpu, OUTPUT_COMPARE16_CONTROL1, OUTPUT_COMPARE_FP | OUTPUT_COMPARE_EDGE_PWM);
    dspic33_write_word(cpu, OUTPUT_COMPARE16_CONTROL2, (uint16_t)(OUTPUT_COMPARE_TRIGGER | source));
}
...
static void external_interrupt_interaction_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, interrupt_flag(cpu, 0u) && (cpu->io.dma_peripheral_pending & 1u) != 0u,
           "INT0 edge raises its interrupt and DMA request");
    expect(state,
           dspic33_device_advance(cpu, 1u) && dspic33_read_word(cpu, DMA_TEST_PAD) == 0x5aa5u,
    ...
    dspic33_gpio_drive(cpu, 3u, 0x0001u, 0x0001u);
    expect(state,
           dspic33_device_advance(cpu, 0u) && (dspic33_read_word(cpu, 0x0320u) & 0x0002u) == 0u,
           "INT0 edge starts the selected ADC conversion");
    expect(state,
           dspic33_device_advance(cpu, 12u) && dspic33_read_word(cpu, 0x0300u) == 100u &&
    ...
    dspic33_gpio_drive(cpu, 3u, 0x0002u, 0x0002u);
    expect(state,
           interrupt_flag(cpu, 20u) && (dspic33_read_word(cpu, OUTPUT_COMPARE16_CONTROL2) &
    ...
    expect(state,
           interrupt_flag(cpu, 29u) && (dspic33_read_word(cpu, OUTPUT_COMPARE16_CONTROL2) &
                                        OUTPUT_COMPARE_TRIGGER_STATUS) != 0u,
...
static void external_interrupt_wake_lifecycle_cases(TestState* state, Dspic33* source,
    ...
    expect(state,
           dspic33_device_wake(source) && source->last_interrupt == 53u && source->pc == 0x0500u &&
               source->w[15] == 0x1804u,
    ...
    expect(state,
           interrupt_flag(source, 20u) && !interrupt_flag(copy, 20u) &&
               source->io.external_interrupt_levels != copy->io.external_interrupt_levels,
    ...
    source->pc = 0u;
    expect(state,
           dspic33_step(source) == DSPIC33_RUNNING && dspic33_read_word(source, RPINR0) == 0u &&
               source->io.external_interrupt_selection[1] == 0u && !interrupt_flag(source, 20u),
    ...
    expect(state, !interrupt_flag(source, 20u), "warm-reset remap baselines the retained high pin");
    dspic33_gpio_drive(source, 3u, 0u, 0x0002u);
    dspic33_gpio_drive(source, 3u, 0x0002u, 0x0002u);
    expect(state, interrupt_flag(source, 20u), "warm-reset external INT state detects later edges");
...
static void interrupt_entry_latency_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state,
           dspic33_step(cpu) == DSPIC33_RUNNING && cpu->pc == 0x0302u && cpu->cycles == 10u &&
               cpu->device_cycles == 10u && dspic33_read_word(cpu, TIMER2_COUNTER) == 10u,
    ...
    expect(state,
           dspic33_device_service_interrupt(cpu) && cpu->pc == 0x0300u && cpu->cycles == 9u &&
               cpu->device_cycles == 9u && interrupt_flag(cpu, 1u) && cpu->interrupt_depth == 1u,
           "entry latency processes a higher-priority request without premature service");
    clear_interrupt(cpu, 0u);
    expect(state,
           dspic33_device_service_interrupt(cpu) && cpu->pc == 0x0320u && cpu->cycles == 18u &&
               cpu->device_cycles == 18u && cpu->interrupt_depth == 2u,
    ...
    cpu->disicnt = 15u;
    dspic33_raise_interrupt(cpu, 0u);
    expect(state, dspic33_device_service_interrupt(cpu) && cpu->disicnt == 6u && cpu->cycles == 9u,
    ...
    expect(state,
           dspic33_device_service_interrupt(cpu) && cpu->stop_reason == DSPIC33_EVENT_QUEUE_ERROR &&
               cpu->cycles == UINT64_MAX - 8u && cpu->pc == 0x0200u && cpu->w[15] == 0x2500u &&
...
static void interrupt_entry_frame_timing_cases(TestState* state, Dspic33* cpu) {
    ...
    cpu->w[15] = 0x2700u;
    expect(state,
           schedule_interrupt_entry_dma(cpu, 0x2700u, 0xa55au, 6u) && service_interrupt_zero(cpu) &&
    ...
    cpu->w[15] = 0x2700u;
    expect(state,
           schedule_interrupt_entry_dma(cpu, 0x2700u, 0xa55au, 7u) && service_interrupt_zero(cpu) &&
    ...
    cpu->w[15] = 0x2700u;
    expect(state,
           schedule_interrupt_entry_dma(cpu, 0x2702u, 0xa55au, 9u) && service_interrupt_zero(cpu) &&
               dspic33_read_word(cpu, 0x2702u) == 0u,
           "high PC and SRL push completes after nine entry cycles");
}

static void interrupt_entry_overlap_cases(TestState* state, Dspic33* source, Dspic33* copy) {
    static const struct {
        uint32_t opcode;
        ...
        {OPCODE_TBLRDL_W4_W3, 5u, 1u, false, "fixed latency retains the documented TBLRD stall"},
        {OPCODE_MOV_W4_W3, 5u, 4u, true, "fixed latency overlaps the ordinary PSV stall"},
    ...
    dspic33_reset(source, 0u);
    expect(state, source->interrupt_entry_overlap == 0u, "POR clears fixed-latency overlap state");
}
...
static void priority_control_cases(TestState* state, Dspic33* cpu) {
    ...
    clear_interrupt(cpu, 0u);
    expect(state,
           dspic33_step(cpu) == DSPIC33_RUNNING && cpu->pc == 0x0200u && cpu->sr == 0x0105u &&
               cpu->w[15] == 0x2800u,
           "RETFIE restores priority and status before the pending peer interrupt");
    expect(state,
           dspic33_device_service_interrupt(cpu) && cpu->last_interrupt == 1u && cpu->pc == 0x0320u,
    ...
    cpu->disicnt = 1u;
    expect(state, !dspic33_device_service_interrupt(cpu), "DISI masks priorities below seven");
...
static void lifecycle_cases(TestState* state, Dspic33* source, Dspic33* copy) {
    ...
    expect(state,
           dspic33_read_word(source, INTCON1) == 0u &&
               ...
               dspic33_read_word(source, INTCON4) == 0u && dspic33_read_word(source, INTTREG) == 0u,
           "POR restores interrupt-control state");
}

static void variable_latency_erratum_cases(TestState* state, Dspic33* source, Dspic33* copy) {
    dspic33_reset(source, 0x0200u);
    ...
    expect(state,
           dspic33_step(source) == DSPIC33_RUNNING && dspic33_read_byte(source, 0x1200u) == 1u,
           "VAR mode records a data-variable write outside an ISR");
    source->program[0x6000u / 2u] = OPCODE_BSET_DATA_0;
    expect(state, dspic33_copy(copy, source), "copy preserves VAR write-domain history");
    dspic33_raise_interrupt(source, 0u);
