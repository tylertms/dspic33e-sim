...
static void cascade_pwm_cases(TestState* state, Dspic33* cpu) {
    ...
    for (low = 0u; low < DSPIC33_OUTPUT_COMPARE_COUNT; low += 2u) {
        ...
        configure_cascade(cpu, low, 6u, UINT32_C(0x00010004), UINT32_C(0x00010002), COMPARE_FP,
                          COMPARE_SELF_SYNC, false);
        expect(state,
               !dspic33_output_compare_output(cpu, low, &value) && output_is(cpu, high, false) &&
                   !interrupt_flag(cpu, low) && !interrupt_flag(cpu, high),
        ...
        expect(state, dspic33_device_advance(cpu, 1u), "advance cascade 32-bit compare");
        expect(state,
        ...
        expect(state, dspic33_device_advance(cpu, 2u), "advance cascade through period value");
        expect(state,
               compare_raw_word(cpu, (uint16_t)(low_base + 8u)) == 4u &&
                   compare_raw_word(cpu, (uint16_t)(high_base + 8u)) == 1u &&
                   output_is(cpu, high, true) && !interrupt_flag(cpu, high),
               "cascade period value is the final high output cycle");
        expect(state, dspic33_device_advance(cpu, 1u), "advance cascade period boundary");
        expect(state,
...
static void cascade_mode_cases(TestState* state, Dspic33* cpu) {
    ...
    for (mode = 1u; mode <= 7u; mode++) {
        ...
        dspic33_reset(cpu, 0u);
        configure_cascade(cpu, 0u, mode, secondary, primary, COMPARE_FP, COMPARE_SELF_SYNC, false);
        expect(state, output_is(cpu, 1u, initial_high), "cascade mode initializes the even output");
        ...
        expect(state,
               compare_raw_word(cpu, 0x0908u) == (uint16_t)primary &&
                   ...
                   output_is(cpu, 1u, mode == 2u || mode == 6u) && !interrupt_flag(cpu, 0u) &&
...
static void cascade_configuration_cases(TestState* state, Dspic33* cpu) {
    ...
    for (index = 0u; index < sizeof(invalid) / sizeof(invalid[0]); index++) {
        ...
        expect(state,
               !dspic33_output_compare_output(cpu, 0u, &high) &&
                   !dspic33_output_compare_output(cpu, 1u, &high) && cpu->events.count == 0u,
               "invalid cascade pairing remains inactive");
        expect(state, dspic33_device_advance(cpu, 8u), "advance invalid cascade pairing");
        expect(state,
    ...
    configure_cascade(cpu, 0u, 6u, UINT32_C(0x00010004), UINT32_C(0x00010002), COMPARE_FP,
                      COMPARE_SELF_SYNC, false);
    expect(state, dspic33_device_advance(cpu, 3u), "advance cascade before live mismatch");
    dspic33_write_word(cpu, 0x090au, 0x1806u);
    expect(state,
           compare_raw_word(cpu, 0x0908u) == 0u && compare_raw_word(cpu, 0x0912u) == 0u &&
               !output_is(cpu, 1u, true),
           "live cascade mismatch stops both timer halves");
    dspic33_write_word(cpu, 0x090au, 0x1c06u);
    expect(state,
           compare_raw_word(cpu, 0x0908u) == 0u && compare_raw_word(cpu, 0x0912u) == 0u &&
               output_is(cpu, 1u, false),
...
static void cascade_buffering_cases(TestState* state, Dspic33* cpu) {
    ...
    configure_cascade(cpu, 0u, 6u, UINT32_C(0x00010004), UINT32_C(0x00010002), COMPARE_FP,
                      COMPARE_SELF_SYNC, false);
    expect(state, dspic33_device_advance(cpu, 1u), "advance cascade before buffered writes");
    ...
    expect(state,
           cpu->io.output_compare.active_rs[0] == 4u && cpu->io.output_compare.active_rs[1] == 1u &&
               cpu->io.output_compare.active_r[0] == 2u && cpu->io.output_compare.active_r[1] == 1u,
    ...
    expect(state, dspic33_device_advance(cpu, 3u), "advance cascade through old buffered period");
    expect(state,
           compare_raw_word(cpu, 0x0908u) == 0u && compare_raw_word(cpu, 0x0912u) == 0u &&
    ...
    dspic33_reset(cpu, 0u);
    configure_cascade(cpu, 0u, 1u, UINT32_C(0x00030000), UINT32_C(0x00010002), COMPARE_FP,
                      COMPARE_SELF_SYNC, false);
    expect(state, dspic33_device_advance(cpu, 1u),
           "advance non-PWM cascade before immediate write");
    dspic33_write_word(cpu, 0x0910u, 2u);
    expect(state,
           cpu->io.output_compare.active_r[0] == 2u && cpu->io.output_compare.active_r[1] == 2u,
    ...
    expect(state,
           compare_raw_word(cpu, 0x0908u) == 2u && compare_raw_word(cpu, 0x0912u) == 2u &&
               output_is(cpu, 1u, false),
           "non-PWM cascade reschedules the full 32-bit compare");
    expect(state, dspic33_device_advance(cpu, 1u), "advance non-PWM cascade output pipeline");
    expect(state, output_is(cpu, 1u, true), "rewritten non-PWM cascade drives the even output");
}
...
static void cascade_trigger_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state,
           (compare_raw_word(cpu, 0x0916u) & COMPARE_TRIGGER_STATUS) == 0u &&
               compare_raw_word(cpu, 0x091cu) == 0u && compare_raw_word(cpu, 0x0926u) == 0u,
    ...
    expect(state, dspic33_device_advance(cpu, 1u), "advance first cascade clock after trigger");
    expect(state, compare_raw_word(cpu, 0x091cu) == 1u && compare_raw_word(cpu, 0x0926u) == 0u,
    ...
    expect(state, dspic33_device_advance(cpu, 1u), "advance synchronized cascade before source");
    expect(state, compare_raw_word(cpu, 0x091cu) == 1u,
           "synchronized cascade counts before the source event");
    expect(state, dspic33_device_advance(cpu, 1u), "advance synchronized cascade source event");
    expect(state,
    ...
    expect(state, dspic33_device_advance(cpu, 1u), "advance cascade synchronization boundary");
    expect(state,
           compare_raw_word(cpu, 0x091cu) == 0u && compare_raw_word(cpu, 0x0926u) == 0u &&
               (cpu->io.output_compare.sync_reset_pending & (1u << 2u)) == 0u,
...
static void cascade_power_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, compare_raw_word(cpu, 0x0908u) == 1u && compare_raw_word(cpu, 0x0912u) == 0u,
           "Sleep holds both cascade timer halves");
    cpu->power_state = DSPIC33_POWER_ACTIVE;
    dspic33_device_power_state_changed(cpu);
    expect(state, dspic33_device_advance(cpu, 1u) && compare_raw_word(cpu, 0x0908u) == 2u,
    ...
    for (member = 0u; member < 2u; member++) {
        ...
        configure_cascade(cpu, 0u, 6u, UINT32_C(0x00010004), UINT32_C(0x00010002), COMPARE_FP,
                          COMPARE_SELF_SYNC, false);
        expect(state, dspic33_device_advance(cpu, 1u), "advance cascade before pair OCSIDL");
        dspic33_write_word(cpu, base, (uint16_t)(dspic33_read_word(cpu, base) | 0x2000u));
        cpu->power_state = DSPIC33_POWER_IDLE;
        dspic33_device_power_state_changed(cpu);
        expect(state, dspic33_device_advance(cpu, 8u), "advance cascade pair through OCSIDL");
    ...
    for (member = 0u; member < 2u; member++) {
        ...
        configure_cascade(cpu, 0u, 6u, UINT32_C(0x00010004), UINT32_C(0x00010002), COMPARE_FP,
                          COMPARE_SELF_SYNC, false);
        expect(state, dspic33_device_advance(cpu, 1u), "advance cascade before pair PMD disable");
        dspic33_write_word(cpu, pmd_address,
                           (uint16_t)(dspic33_read_word(cpu, pmd_address) | pmd_mask));
        expect(state, dspic33_device_advance(cpu, 1u), "advance cascade pair PMD disable boundary");
        expect(state, dspic33_device_advance(cpu, 8u), "advance disabled cascade pair");
        expect(state,
               compare_raw_word(cpu, 0x0908u) == 2u && compare_raw_word(cpu, 0x0912u) == 0u &&
                   !dspic33_output_compare_output(cpu, 1u, &high),
               "disabling either cascade half pauses the pair");
        dspic33_write_word(cpu, pmd_address,
                           (uint16_t)(dspic33_read_word(cpu, pmd_address) & ~pmd_mask));
        expect(state, dspic33_device_advance(cpu, 1u), "advance cascade pair PMD enable boundary");
        expect(state,
               dspic33_device_advance(cpu, 1u) && compare_raw_word(cpu, 0x0908u) == 3u &&
                   output_is(cpu, 1u, false),
...
static void cascade_lifecycle_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, copy_initialized && dspic33_copy(&copy, cpu), "copy active cascade state");
    if (copy_initialized) {
        expect(state,
               dspic33_device_advance(cpu, 2u) && dspic33_device_advance(&copy, 3u) &&
                   compare_raw_word(cpu, 0x0908u) == 3u && compare_raw_word(&copy, 0x0908u) == 4u,
...
static void cascade_clock_cases(TestState* state, Dspic33* cpu) {
    ...
    for (index = 0u; index < sizeof(clocks) / sizeof(clocks[0]); index++) {
        ...
        configure_cascade(cpu, 0u, 6u, UINT32_C(0x00010004), UINT32_C(0x00010002), clocks[index],
                          COMPARE_SELF_SYNC, false);
        expect(state, dspic33_device_advance(cpu, UINT32_C(0x00010001)),
               "advance alternate-clock cascade before compare");
        expect(state,
               compare_raw_word(cpu, 0x0908u) == 1u && compare_raw_word(cpu, 0x0912u) == 1u &&
                   output_is(cpu, 1u, false),
               "alternate clock carries the cascade low timer into high timer");
        expect(state, dspic33_device_advance(cpu, 1u), "advance alternate-clock cascade compare");
        expect(state,
               compare_raw_word(cpu, 0x0908u) == 2u && compare_raw_word(cpu, 0x0912u) == 1u &&
                   output_is(cpu, 1u, true),
               "alternate clock drives the cascaded even output");
    }

    dspic33_reset(cpu, 0u);
    configure_cascade(cpu, 0u, 1u, 0u, UINT32_C(0xfffffffe), COMPARE_FP, COMPARE_NO_SYNC, false);
    expect(state, dspic33_device_advance(cpu, UINT32_C(0xfffffffe)),
           "advance free-running cascade to maximum compare");
    expect(state,
           compare_raw_word(cpu, 0x0908u) == 0xfffeu && compare_raw_word(cpu, 0x0912u) == 0xffffu &&
    ...
    expect(state, compare_raw_word(cpu, 0x0908u) == 0u && compare_raw_word(cpu, 0x0912u) == 0u,
    ...
    configure_cascade(cpu, 0u, 6u, 6u, 3u, COMPARE_FP, COMPARE_SELF_SYNC, false);
    {
        ...
        if (initialized) {
            uint8_t tick;
            expect(state, dspic33_device_advance(cpu, 7u), "advance cascade in one batch");
            for (tick = 0u; tick < 7u; tick++) {
                expect(state, dspic33_device_advance(&stepped, 1u),
                       "advance cascade one selected clock");
            }
            expect(state,
                   compare_raw_word(cpu, 0x0908u) == compare_raw_word(&stepped, 0x0908u) &&
                       compare_raw_word(cpu, 0x0912u) == compare_raw_word(&stepped, 0x0912u) &&
...
static void cascade_pipeline_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, dspic33_device_advance(cpu, 2u), "advance cascade single compare match");
    expect(state, output_is(cpu, 1u, false) && !interrupt_flag(cpu, 1u),
           "cascade single compare match precedes its output pipeline");
    expect(state, dspic33_device_advance(cpu, 1u), "advance cascade single output pipeline");
    expect(state, output_is(cpu, 1u, true) && !interrupt_flag(cpu, 1u),
           "cascade single output changes one clock after compare");
    expect(state, dspic33_device_advance(cpu, 2u), "advance cascade single interrupt pipeline");
    ...
    expect(state, dspic33_device_advance(cpu, 2u), "advance cascade dual primary compare");
    expect(state, dspic33_device_advance(cpu, 1u) && output_is(cpu, 1u, true),
           "cascade dual primary output is delayed one clock");
    expect(state, dspic33_device_advance(cpu, 7u), "advance cascade dual secondary compare");
    expect(state, output_is(cpu, 1u, true) && !interrupt_flag(cpu, 1u),
           "cascade dual secondary compare precedes its output pipeline");
    expect(state, dspic33_device_advance(cpu, 1u), "advance cascade dual secondary output");
    expect(state, output_is(cpu, 1u, false) && !interrupt_flag(cpu, 1u),
           "cascade dual secondary output is delayed one clock");
    expect(state, dspic33_device_advance(cpu, 2u), "advance cascade dual interrupt pipeline");
    expect(state, interrupt_flag(cpu, 1u) && !interrupt_flag(cpu, 0u),
...
static void cascade_short_pwm_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
    configure_cascade(cpu, 0u, 6u, 4u, UINT32_C(0x00010002), COMPARE_FP, COMPARE_SELF_SYNC, false);
    expect(state, dspic33_device_advance(cpu, 4u),
           "advance zero-even-period cascade through low RS");
    expect(state,
           compare_raw_word(cpu, 0x0908u) == 4u && compare_raw_word(cpu, 0x0912u) == 0u &&
               output_is(cpu, 1u, false) && !interrupt_flag(cpu, 1u),
    ...
    expect(state,
           compare_raw_word(cpu, 0x0908u) == UINT16_MAX &&
               compare_raw_word(cpu, 0x0912u) == UINT16_MAX && output_is(cpu, 1u, true) &&
               !interrupt_flag(cpu, 1u),
           "zero even RS holds high through the full timer range");
    expect(state, dspic33_device_advance(cpu, 1u), "advance zero-even-period cascade rollover");
    expect(state,
           compare_raw_word(cpu, 0x0908u) == 0u && compare_raw_word(cpu, 0x0912u) == 0u &&
    ...
    dspic33_reset(cpu, 0u);
    configure_cascade(cpu, 0u, 6u, UINT32_C(0x00010004), 2u, COMPARE_FP, COMPARE_SELF_SYNC, false);
    expect(state, dspic33_device_advance(cpu, 4u),
    ...
    expect(state, dspic33_device_advance(cpu, 1u), "advance all-short cascade low-period boundary");
    expect(state,
           compare_raw_word(cpu, 0x0908u) == 0u && compare_raw_word(cpu, 0x0912u) == 1u &&
               output_is(cpu, 1u, true) && interrupt_flag(cpu, 1u),
    ...
    expect(state, dspic33_device_advance(cpu, 3u), "advance all-short center PWM output pipeline");
    expect(state, output_is(cpu, 1u, true) && !interrupt_flag(cpu, 1u),
           "all-short center PWM raises only the even output");
    expect(state, dspic33_device_advance(cpu, 2u), "advance all-short center PWM period boundary");
    expect(state,
           compare_raw_word(cpu, 0x0908u) == 0u && compare_raw_word(cpu, 0x0912u) == 1u &&
               output_is(cpu, 1u, true) && interrupt_flag(cpu, 1u),
...
static void cascade_access_activation_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, dspic33_step(cpu) == DSPIC33_RUNNING && compare_raw_word(cpu, 0x0908u) == 0u,
    ...
    expect(state, dspic33_step(cpu) == DSPIC33_RUNNING && compare_raw_word(cpu, 0x0908u) == 1u,
    ...
    expect(state,
           !dspic33_output_compare_output(cpu, 0u, &value) && output_is(cpu, 1u, false) &&
               cpu->events.count == 1u,
    ...
    expect(state,
           output_is(cpu, 0u, true) && !dspic33_output_compare_output(cpu, 1u, &value) &&
               compare_raw_word(cpu, 0x0908u) == 0u && compare_raw_word(cpu, 0x0912u) == 0u,
    ...
    expect(state, pin_is(cpu, 65u, true), "cascaded even PPS output follows the 32-bit compare");
}
