...
static void configure_capture_source(Dspic33* cpu, uint8_t channel, uint16_t timer_source,
                                     bool triggered, bool running, uint8_t sync_source,
                                     bool paired) {
    uint16_t base = capture_base(channel);
...
static void interrupt_rate_cases(TestState* state, Dspic33* cpu) {
    uint8_t interval;
...
static void zero_interval_overflow_cases(TestState* state, Dspic33* cpu) {
    uint8_t sample;
    dspic33_reset(cpu, 0u);
...
static void paired_cases(TestState* state, Dspic33* cpu) {
...
static void dma_cases(TestState* state, Dspic33* cpu) {
    ...
    for (channel = 0u; channel < 4u; channel++) {
        ...
        dspic33_reset(cpu, 0u);
        configure_dma(cpu, capture_irqs[channel], (uint16_t)(capture_base(channel) + 4u),
                      destination);
    ...
    dspic33_reset(cpu, 0u);
    configure_dma(cpu, capture_irqs[0], (uint16_t)(capture_base(0u) + 4u), CAPTURE_DMA_DESTINATION);
    ...
    dspic33_reset(cpu, 0u);
    configure_dma(cpu, capture_irqs[4], (uint16_t)(capture_base(4u) + 4u), CAPTURE_DMA_DESTINATION);
...
static void timer_source_cases(TestState* state, Dspic33* cpu) {
    ...
    for (index = 0u; index < 5u; index++) {
        dspic33_reset(cpu, 0u);
        configure_capture_source(cpu, 0u, capture_timer_sources[index], true, true, 0u, false);
    ...
    for (index = 0u; index < sizeof(no_sources) / sizeof(no_sources[0]); index++) {
        dspic33_reset(cpu, 0u);
        configure_capture_source(cpu, 0u, 0x1c00u, false, false, no_sources[index], false);
    ...
    for (index = 0u; index < 2u; index++) {
        dspic33_reset(cpu, 0u);
        configure_capture_source(cpu, 0u, index == 0u ? 0x1400u : 0x1800u, true, true, 0u, false);
...
static void sync_trigger_cases(TestState* state, Dspic33* cpu) {
    ...
    for (source = 0u; source < 5u; source++) {
        dspic33_reset(cpu, 0u);
        configure_capture_source(cpu, 15u, 0x1c00u, true, false, (uint8_t)(11u + source), false);
    ...
    for (source = 0u; source < 9u; source++) {
        dspic33_reset(cpu, 0u);
        configure_capture_source(cpu, 15u, 0x1c00u, true, false, (uint8_t)(1u + source), false);
    ...
    for (source = 0u; source < 8u; source++) {
        ...
        configure_capture_source(cpu, 15u, 0x1c00u, true, false, (uint8_t)(16u + source), false);
    ...
    for (source = 0u; source < 3u; source++) {
        dspic33_reset(cpu, 0u);
        configure_capture_source(cpu, 15u, 0x1c00u, true, false, (uint8_t)(24u + source), false);
    ...
    expect(state, (dspic33_read_word(cpu, (uint16_t)(capture_base(0u) + 2u)) & 0x0040u) != 0u,
