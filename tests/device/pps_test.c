...
static void source_cases(TestState* state, Dspic33* cpu) {
    uint16_t source;
    uint8_t channel;
    uint8_t input;
...
static void physical_fanout_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
...
static void output_cases(TestState* state, Dspic33* cpu) {
    size_t index;
    bool high;
    dspic33_reset(cpu, 0u);
