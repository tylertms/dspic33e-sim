...
static void fifo_control_write_cases(TestState* state, Dspic33* cpu) {
...
static void register_access_cases(TestState* state, Dspic33* cpu) {
...
static void interrupt_flag_write_zero_cases(TestState* state, Dspic33* cpu) {
...
static void standard_filter_domain(TestState* state, Dspic33* cpu) {
    uint16_t identifier;
...
static void payload_and_remote_cases(TestState* state, Dspic33* cpu) {
    uint8_t channel;
    uint8_t length;
    uint8_t extended;
    uint8_t remote;
...
static void devicenet_cases(TestState* state, Dspic33* cpu) {
    uint8_t bits;
    uint8_t length;
...
static void direct_buffer_cases(TestState* state, Dspic33* cpu) {
    uint8_t channel;
    uint8_t buffer;
    for (channel = 0u; channel < DSPIC33_CAN_COUNT; channel++) {
...
static void fifo_overflow_advancement_cases(TestState* state, Dspic33* cpu) {
    uint8_t channel;
    for (channel = 0u; channel < DSPIC33_CAN_COUNT; channel++) {
        uint8_t mode;
...
static void receive_flag_write_zero_domain(TestState* state, Dspic33* cpu) {
...
static void receive_overflow_write_zero_prior_domain(TestState* state, Dspic33* cpu) {
...
static void receive_flag_read_pointer_cases(TestState* state, Dspic33* cpu) {
    static const uint8_t sizes[] = {4u, 6u, 8u, 12u, 16u, 24u, 32u};
    uint8_t channel;
    for (channel = 0u; channel < DSPIC33_CAN_COUNT; channel++) {
        uint8_t selection;
...
static void fifo_interrupt_boundary_cases(TestState* state, Dspic33* cpu) {
    uint8_t channel;
    for (channel = 0u; channel < DSPIC33_CAN_COUNT; channel++) {
        uint8_t wrap;
        for (wrap = 0u; wrap < 2u; wrap++) {
...
static void receive_flag_hardware_event_cases(TestState* state, Dspic33* cpu) {
    uint8_t channel;
    for (channel = 0u; channel < DSPIC33_CAN_COUNT; channel++) {
        uint8_t target;
...
static void overflow_and_fallback_cases(TestState* state, Dspic33* cpu) {
...
static void clock_timing_cases(TestState* state, Dspic33* cpu) {
...
static void stuffed_frame_timing_cases(TestState* state, Dspic33* cpu) {
...
static void transmit_abort_timing_cases(TestState* state, Dspic33* cpu) {
    for (uint8_t channel = 0u; channel < DSPIC33_CAN_COUNT; channel++) {
        uint16_t base = bases[channel];
...
static void overload_frame_cases(TestState* state, Dspic33* cpu) {
...
static void arbitration_field_cases(TestState* state, Dspic33* cpu) {
...
static void acknowledge_error_cases(TestState* state, Dspic33* cpu) {
    for (uint8_t channel = 0u; channel < DSPIC33_CAN_COUNT; channel++) {
        uint16_t base = bases[channel];
...
static void transmit_error_variant_cases(TestState* state, Dspic33* cpu) {
    bool high;
...
static void bus_off_recovery_cases(TestState* state, Dspic33* cpu) {
    for (uint8_t channel = 0u; channel < DSPIC33_CAN_COUNT; channel++) {
        uint16_t base = bases[channel];
...
static void error_counter_recovery_cases(TestState* state, Dspic33* cpu) {
    for (uint8_t channel = 0u; channel < DSPIC33_CAN_COUNT; channel++) {
        uint16_t base = bases[channel];
...
static void receive_pps_qualification_cases(TestState* state, Dspic33* cpu) {
...
static void priority_and_abort_cases(TestState* state, Dspic33* cpu) {
    Dspic33CanFrame output;
    uint8_t buffer;
    dspic33_reset(cpu, 0u);
...
static void mode_transition_cases(TestState* state, Dspic33* cpu) {
    uint64_t cycles;
    dspic33_reset(cpu, 0u);
...
static void physical_debug_mode_cases(TestState* state, Dspic33* cpu) {
...
static void capture_timestamp_cases(TestState* state, Dspic33* cpu) {
    for (uint8_t channel = 0u; channel < DSPIC33_CAN_COUNT; channel++) {
        uint16_t base = bases[channel];
...
static void interrupt_and_error_cases(TestState* state, Dspic33* cpu) {
    uint8_t channel;
...
static void invalid_message_cases(TestState* state, Dspic33* cpu) {
