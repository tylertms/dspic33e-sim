...
static void write_descriptor(Dspic33* cpu, uint8_t endpoint, uint8_t direction, uint8_t bank,
                             uint16_t status, uint16_t count, uint32_t buffer) {
...
static bool packet_data(const Dspic33UsbPacket* packet, const uint8_t* data, uint16_t size) {
...
static void register_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, memory_word(cpu, BDT) == 0xa55au, "USB power clear preserves BDT memory");
}
...
static void out_payload_domain(TestState* state, Dspic33* cpu) {
    ...
    for (endpoint = 0u; endpoint < DSPIC33_USB_ENDPOINT_COUNT; endpoint++) {
        ...
        for (size = 0u; size <= sizeof(data); size++) {
            ...
            write_descriptor(cpu, endpoint, 0u, bank, (uint16_t)(0x0088u | (data1 ? 0x0040u : 0u)),
                             size, buffer);
            expect(state,
            ...
            expect(state,
                   memory_word(cpu, descriptor_address(endpoint, 0u, bank)) ==
                       ...
                       memory_word(cpu, descriptor_address(endpoint, 0u, bank) + 2u) == size,
...
static void in_payload_domain(TestState* state, Dspic33* cpu) {
    ...
    for (endpoint = 0u; endpoint < DSPIC33_USB_ENDPOINT_COUNT; endpoint++) {
        ...
        for (size = 0u; size <= sizeof(data); size++) {
            ...
            write_descriptor(cpu, endpoint, 1u, bank, (uint16_t)(0x0088u | (data1 ? 0x0040u : 0u)),
                             size, buffer);
...
static void boundary_and_order_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state,
           memcmp(cpu->data + buffer, data, sizeof(data)) == 0 &&
               memory_word(cpu, descriptor_address(15u, 0u, 0u) + 2u) == DSPIC33_USB_PACKET_SIZE,
...
static void bus_access_error_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, (dspic33_read_word(cpu, IR) & 0x0002u) != 0u && interrupt_flag(cpu, USB_IRQ),
    ...
    expect(state, !interrupt_flag(cpu, USB_IRQ), "USB bus access software interrupt clear");
...
static void host_interrupt_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, (dspic33_read_word(cpu, IR) & 0x0040u) != 0u && interrupt_flag(cpu, USB_IRQ),
    ...
    expect(state, (dspic33_read_word(cpu, IR) & 0x0001u) != 0u && interrupt_flag(cpu, USB_IRQ),
    ...
    expect(state, (dspic33_read_word(cpu, IR) & 0x0080u) == 0u && interrupt_flag(cpu, USB_IRQ),
...
static void pmd_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state,
           dspic33_usb_bus(cpu, DSPIC33_USB_BUS_RESET, 0u, 0u) && dspic33_device_advance(cpu, 0u) &&
               (memory_word(cpu, IR) & 1u) == 0u && cpu->events.count == 0u,
    ...
    expect(state, dspic33_device_advance(cpu, USB_FRAME_CYCLES) && memory_word(cpu, FRML) == 0u,
...
static void power_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state,
           cpu->events.count == 1u && cpu->events.items[0].paused &&
               dspic33_device_advance(cpu, USB_FRAME_CYCLES) && memory_word(cpu, FRML) == 0u,
    ...
    expect(state,
           dspic33_usb_bus(cpu, DSPIC33_USB_BUS_RESUME, 0u, 0u) &&
               dspic33_device_advance(cpu, 0u) && (memory_word(cpu, OTGIR) & 0x0010u) != 0u &&
