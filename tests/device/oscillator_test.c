...
static uint16_t control(Dspic33* cpu) { return dspic33_read_word(cpu, OSCILLATOR_CONTROL); }
...
static Dspic33StopReason write_protected_byte(Dspic33* cpu, uint16_t address, uint8_t value) {
...
static void reset_cases(TestState* state, Dspic33* cpu) {
    ...
    dspic33_reset(cpu, 0u);
    expect(state, control(cpu) == 0x7700u, "POR immediately selects ready internal FNOSC source");
...
static void protection_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, write_protected_byte(cpu, OSCILLATOR_CONTROL + 1u, 0x03u) == DSPIC33_RUNNING,
           "high-byte unlock sequence executes");
    expect(state, control(cpu) == 0x0300u, "high-byte unlock writes NOSC only");
    expect(state, write_protected_byte(cpu, OSCILLATOR_CONTROL, 0x42u) == DSPIC33_RUNNING,
           "low-byte unlock sequence executes");
    expect(state, control(cpu) == 0x0342u, "low-byte unlock writes IOLOCK and LPOSCEN");
    expect(state, write_protected_byte(cpu, OSCILLATOR_CONTROL, 0x80u) == DSPIC33_RUNNING,
...
static void switch_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, control(cpu) == 0x0301u, "switch remains pending before modeled deadline");
    expect(state, dspic33_device_advance(cpu, 1u), "advance to modeled switch deadline");
    expect(state, control(cpu) == 0x3320u, "POSCPLL switch transfers COSC and sets LOCK");
    ...
    expect(state, (control(cpu) & 0x7021u) == 0u, "aborted event cannot change source or lock");
    ...
    write_protected_byte(cpu, OSCILLATOR_CONTROL + 1u, 0x03u);
    write_protected_byte(cpu, OSCILLATOR_CONTROL, OSCILLATOR_CLOCK_LOCK | OSCILLATOR_SWITCH_ENABLE);
    ...
    write_protected_byte(cpu, OSCILLATOR_CONTROL, OSCILLATOR_CLOCK_LOCK | OSCILLATOR_LP_ENABLE);
...
static void failure_trap_cases(TestState* state, Dspic33* cpu) {
    dspic33_reset(cpu, 0u);
    expect(state,
           write_protected_byte(cpu, OSCILLATOR_CONTROL, OSCILLATOR_CLOCK_FAIL) == DSPIC33_RUNNING,
...
static void fail_safe_matrix_cases(TestState* state, Dspic33* cpu) {
    ...
    for (fcksm = 0u; fcksm < 4u; fcksm++) {
        for (source = 0u; source < 8u; source++) {
            ...
            expect(state,
                   expected
                       ? control(cpu) == (uint16_t)((uint16_t)source << 8u | OSCILLATOR_CLOCK_FAIL)
...
static void configuration_admission_cases(TestState* state, Dspic33* cpu) {
    ...
    for (index = 0u; index < sizeof(disabled_configurations) / sizeof(disabled_configurations[0]);
        ...
        write_protected_byte(cpu, OSCILLATOR_CONTROL + 1u, 0x02u);
        write_protected_byte(cpu, OSCILLATOR_CONTROL, OSCILLATOR_SWITCH_ENABLE);
        expect(state, control(cpu) == 0x0200u, "FCKSM disabled mode rejects software switch");
        expect(state, !cpu->oscillator.active && cpu->events.count == 0u,
    ...
    expect(state, program_fosc(cpu, 0x009eu), "FOSC RTSP disables active software switching");
    expect(state, !cpu->oscillator.active && control(cpu) == 0x0200u,
    ...
    write_protected_byte(cpu, OSCILLATOR_CONTROL + 1u, 0x03u);
    write_protected_byte(cpu, OSCILLATOR_CONTROL, OSCILLATOR_SWITCH_ENABLE);
    expect(state, control(cpu) == 0x1321u, "FRCPLL to POSCPLL direct switch remains pending");
    expect(state, cpu->oscillator.active && cpu->events.count == 0u,
           "first direct PLL request schedules no completion");
    expect(state, program_fosc(cpu, 0x005du), "POSCMD mode changes during direct PLL request");
    expect(state,
           dspic33_device_advance(cpu, 32u) && control(cpu) == 0x1321u && cpu->events.count == 0u,
           "POSCMD mode change cannot complete direct PLL request");
    expect(state, program_fosc(cpu, 0x005fu), "POSCMD disables target during direct PLL request");
    expect(state, program_fosc(cpu, 0x005eu), "POSCMD restores target during direct PLL request");
    expect(state,
           dspic33_device_advance(cpu, 32u) && control(cpu) == 0x1321u && cpu->events.count == 0u,
    ...
    write_protected_byte(cpu, OSCILLATOR_CONTROL, OSCILLATOR_SWITCH_ENABLE);
    expect(state, control(cpu) == 0x3121u, "POSCPLL to FRCPLL direct switch remains pending");
    expect(state, cpu->oscillator.active && cpu->events.count == 0u,
...
static void hardware_failure_cases(TestState* state, Dspic33* cpu) {
    ...
    expect(state, control(cpu) == 0x0208u, "FSCM falls back to FRC and retains requested source");
...
static void pll_lock_sequence_cases(TestState* state, Dspic33* source, Dspic33* copy) {
    ...
    expect(state, program_fosc(source, 0x005eu), "POSCMD RTSP enables PLLK-one request");
...
static void two_speed_startup_cases(TestState* state, Dspic33* source, Dspic33* copy) {
    ...
    dspic33_reset(source, 0u);
    expect(state,
           control(source) == 0x0400u && source->oscillator.automatic && source->events.count == 1u,
    ...
    expect(state, program_fosc(source, 0x005eu), "POSCMD RTSP enables Two-Speed target");
    expect(state, source->events.count == 1u && source->events.items[0].source == 0u,
    ...
    write_protected_byte(source, OSCILLATOR_CONTROL, OSCILLATOR_IO_LOCK);
    expect(state,
           control(source) == 0x0240u && source->oscillator.active && source->oscillator.automatic,
    ...
    expect(state,
           control(source) == 0x0200u && source->oscillator.active && source->oscillator.automatic,
    ...
    dspic33_reset(source, 0u);
    expect(state, control(source) == 0x4400u, "reset applies deferred FNOSC configuration");
...
static void reference_clock_pin_cases(TestState* state, Dspic33* source, Dspic33* copy) {
    uint64_t edges = 0u;
    dspic33_reset(source, 0u);
...
static void main_pll_configuration_cases(TestState* state, Dspic33* source, Dspic33* copy) {
    uint32_t generation;
    uint64_t deadline;
    ...
    expect(state, select_locked_main_pll(source, 1u), "select stable FRCPLL for relock cases");
    generation = source->oscillator.generation;
    ...
    expect(state, select_locked_main_pll(source, 3u), "select stable POSCPLL for relock cases");
    dspic33_write_word(source, MAIN_PLL_FEEDBACK, 0x0031u);
    ...
    expect(state, select_locked_main_pll(source, 1u), "select FRCPLL for relock lifecycle");
    dspic33_write_word(source, MAIN_PLL_FEEDBACK, 0x0031u);
    dspic33_device_advance(source, 10u);
    ...
    expect(state, select_locked_main_pll(source, 1u), "select FRCPLL for relock schedule failure");
    source->device_cycles = UINT64_MAX;
    dspic33_write_word(source, MAIN_PLL_FEEDBACK, 0x0031u);
    expect(state,
           source->stop_reason == DSPIC33_EVENT_QUEUE_ERROR && control(source) == 0x1100u &&
               source->oscillator.lock_pending && source->events.count == 0u,
...
static void lifecycle_cases(TestState* state, Dspic33* source, Dspic33* copy) {
    ...
    dspic33_reset(source, 0u);
    write_protected_byte(source, OSCILLATOR_CONTROL + 1u, 0x03u);
    load_sequence(source, OPCODE_MOV_BYTE_W2_W1, OPCODE_MOV_BYTE_W3_W1, OPCODE_MOV_BYTE_W0_W1);
    source->pc = 0x0200u;
